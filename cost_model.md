...
## Appendix A: Worked Example, Single-Block Eltwise Add

Section 6 shows the shape the model targets in steady state. This appendix shows
a measured execution graph for a real kernel at the other extreme: one block on
one core, where the pipeline never fills. It is derived from the generated
TTKernel and LLK sources for `examples/eltwise_add_single_block.py`, and it is
useful as a validation oracle for the graph unit tests in section 9, because
every edge is checkable by hand.

Operation labels carry the generated-C++ statement they come from: `C`n is
statement n of the compute kernel, `R`n of the reader, `W`n of the writer.

```mermaid
%%{init: {"theme": "base", "themeVariables": {"primaryColor": "#1e3a8a", "primaryTextColor": "#ffffff", "primaryBorderColor": "#93c5fd", "lineColor": "#94a3b8", "textColor": "#cbd5e1", "labelTextColor": "#cbd5e1", "edgeLabelBackground": "transparent", "fontSize": "14px"}}}%%
sequenceDiagram
    participant Rd as Reader (NCRISC, NoC0)
    participant Un as Unpacker (TRISC0)
    participant MM as Matrix engine (FPU, TRISC1)
    participant Pk as Packer (TRISC2)
    participant Wr as Writer (BRISC, NoC1)
    Note over Rd,Wr: all five start together, four stall immediately on an empty resource
    activate Rd
    Rd->>Rd: reserve cb0, cb1 (R1, R2)
    Rd->>Rd: async_read a, b, 4 pages (R3, R4)
    Rd->>Rd: read_barrier (R5)
    deactivate Rd
    Rd->>Un: D1 cb0/cb1 pages ready (R6, R7 push_back)
    activate Un
    Un->>Un: unpack_hw_config, AB_init (C4, C6)
    Un->>Un: unpack_AB tile 0, tile 1 (C7, C8)
    Un->>Rd: D6 cb0/cb1 pages free (C14, C15 pop_front)
    deactivate Un
    Un->>MM: D2 SrcA/SrcB dvalid, one edge per tile (C7, C8)
    activate MM
    MM->>MM: add_tiles into DST[0..1] (C7, C8)
    deactivate MM
    MM->>Pk: D3 DST half committed (C9 dest_section_done)
    activate Pk
    Pk->>Pk: pack_tile_block, DST to L1, 2 tiles (C11)
    Pk->>MM: D7 DST half released (C12)
    deactivate Pk
    Pk->>Wr: D4 cb2 pages ready (C13 push_tiles)
    activate Wr
    Wr->>Wr: async_write 2 pages (W2)
    Wr->>Wr: write_barrier (W3)
    Wr->>Pk: D5 cb2 pages free (W4 pop_front)
    deactivate Wr
    Note over Rd,Wr: D5, D6 and D7 fire but nothing is waiting on them, so one block means no overlap
```

Three properties of this graph are worth carrying into the model.

**The forward edges form a chain, not a pipeline.** D1 through D4 are the only
edges that gate anything, and they run strictly left to right, so the runtime is
the sum of all five participants' intervals. Four of the five participants are
idle for most of it. Any model that sums local work would report roughly the
right number here for entirely the wrong reason.

**The back-pressure edges fire with no waiter.** D5, D6 and D7 are the
capacity-return edges the section 2.4 initiation interval depends on. With one
block they are pure slack. They only bind once the trip count exceeds the buffer
depth, which is exactly why a candidate ranking that ignores buffer capacity can
misrank a loop while agreeing on the straight-line case.

**Participants here are engines, and one of them hides a decoupled processor.**
Following section 6, `Un` is the unpacker engine rather than the TRISC0 control
processor. Those two are not one interval: `_llk_unpack_AB_` calls
`wait_for_next_context(2)`, a two-credit counter posted by the RISC and released
by the backend, so the processor returns before the engine has moved anything and
may run one unpack ahead. Folding them, as section 2.2 permits, asserts a credit
of one. That is safe only while the processor is not on the limiting chain, and
it is the concrete case where the section 2.2 assumption needs recording.

The same graph also carries the two synchronization mechanisms noted as missing
from the section 2.3 edge list: the unpacker config-context credit above, and the
SrcA/SrcB bank credit behind D2, where the unpacker sets dvalid and the math MOP
end op `SETRWC(CLR_AB, ...)` clears it. Both are per-tile credits on the
unpack-to-math path, structurally the same as `t6` on the matrix-to-vector path.
