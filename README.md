...
cd /path/to/tt-lang
export TTMLIR_TOOLCHAIN_DIR=/opt/ttmlir-toolchain  # Optional
...
The CMake configure step generates a `build/env/activate` script that uses the Python environment and toolchain from the tt-mlir build. If `TTMLIR_TOOLCHAIN_DIR` is set as an environment variable, it will be used; otherwise, CMake will derive it from the tt-mlir build configuration.
...
```bash
export TTMLIR_TOOLCHAIN_DIR=/path/to/ttmlir-toolchain  # Set as environment variable
cmake -GNinja -Bbuild .
source build/env/activate
cmake --build build
```

Or set it as a CMake variable:
...
The CMake configure step generates a `build/env/activate` script that finds the installed tt-mlir and uses its toolchain and Python environment.
