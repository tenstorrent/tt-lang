...
precision during unpack (truncated to bf16) but the FPU/SRCA strategy
remains available.
...
SFPU-strategy when it has `TTLDSTInputsTrait` and is not `isCBInputOp`.

FPU-eligible `tile_add`/`sub`/`mul` ops carry `TTLDSTInputsTrait` by default,
so a separate pass `TTLMarkFPUBinaries` (scheduled before this pass) walks
each `ttl.compute` body and sets `kFPUBinaryAttrName` on every op for which
`isFpuBinaryEligible` holds. `isCBInputOp` returns true for both
`TTLCBInputTileOpTrait` ops and ops carrying that attribute. This means the
SFPU classification here is a structural check on the IR — no duplicated
predicate, no shared option.
