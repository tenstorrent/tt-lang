...
movement op:  cyc = latency(locality) + (tiles x bytes_per_tile) / R_noc(locality)
```
...
```
T_kernel = max(sum cyc_compute, sum cyc_movement)
```
...
```
memory_floor = (sum memory_bytes) / memory_aggregate_bw
...
```
node_time  = max_i(C_i) + (sum_i C_i - max_i C_i) / N     # fill/drain-inclusive per node
...
The report emits this per board when the profile sets `tensix_cores` (see [Output](#output)). Two caveats: FLOP counts **matmul only** (the SFPU rate is an ideal-floor assumption), and the compute roof is the fixed **bf16/HiFi4** reference - a BFP8 (LoFi) kernel runs ~4x faster, so its `compute util` can exceed 100%.
...
| `bytes_per_tile` | 2048 | bf16 32x32 tile (32*32*2 B) |
...
| `memory_aggregate_gbps` | 288 GB/s | 12 GDDR6 ch x 24 B/cyc @ 12 Gbps (tt-metal `Saturating_DRAM_bandwidth.md`); the **spec upper bound** is used. |
| matmul rate (default) | 1/64 | `16 x fidelity` cyc/tile; tt-lang sets no MathFidelity -> tt-metal default **HiFi4**, fixed. |
...
Known simplifications: `noc_bw` uses one measured asymptote for all localities (local L1 ~ 2x remote, DRAM ~ 24 B/cyc per channel); fidelity is fixed at HiFi4.
...
Program-level accuracy is checked against device cycles on Wormhole N300 (grid 8x8, 64 cores) across the roofline's three movement regimes - memory-bound re-read, compute-bound reuse, and NoC-bound multicast - plus a Blackhole P100a compute spot-check. `measured >= estimate` at every point (the estimate is an ideal-peak lower bound); how *tight* the bound is depends on which resource the kernel actually stresses.

![Four matmul movement regimes on the Wormhole N300 roofline: estimate markers vs device (red x). Memory, blocked, and compute regimes sit tight (~1.5x the estimate); fine-grained multicast sits far below (5.7x).](roofline-regimes.png)
...
|---|---|---|---|---|
| Memory | pure re-read (no blocking) | 15 | memory | ~1.5x |
| Memory | blocked re-read (`step_4`) | 43-63 | memory | ~1.4x |
| **Compute** | full reuse (per-node big block, cube M=N=K) | up to **241** | **compute** | **1.59x** |
| Movement | 2D multicast (fine-grained) | 85 | movement | **5.7x** |

- **The memory and compute roofs are both tight.** The DRAM roof (pure re-read, AI 15) and the compute roof (full reuse, AI 241 - a cube M=N=K sweep whose per-node block grows until AI crosses the ridge) both land at **1.4-1.7x the estimate**, so the ideal-peak model is a *tight* lower bound once a kernel saturates a roof.
- **Multicast exposes the movement model's gap.** A fine-grained (1-tile) 2D multicast runs at **5.7x the estimate** (a stable multiplier across sizes): the movement term prices the broadcast's bytes but not the fixed **per-transfer latency** that dominates thousands of small NoC transfers  (see [Limitations](#current-limitations--deferred-work)).
...
- **Multicast under-counted (~5.7x,** [Hardware Validation](#hardware-validation)**)** - `pipe_recv` uses the flat unicast latency, missing fan-out latency and a shared-NoC ceiling. Not a value swap: needs a size x fan-out *table lookup* (the model uses a scalar), `pipe_recv` must start emitting the fan-out (producer-side change), and it's WH-only in-tree + needs device re-validation.
*Source:* tt-metal `noc_estimator/noc_latencies.yaml` (a 55-way mcast of a 2 KB tile is ~10x unicast)
...
*Source:* derivable per-op from LLK instruction sequences x Tensix SFPU ISA issue rate (`llk_sfpu` + SFPU ISA, per board)
