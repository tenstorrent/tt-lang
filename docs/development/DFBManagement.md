...
## Storage identity through block shape views
...
`tensor.expand_shape`, respectively, with explicit reassociation indices. An
empty dimension list returns the original value. Public dimension normalization
and diagnostic rules remain unchanged.
...
Inserting or removing a size-one dimension does not change the row-major linear
index of any tile. The accepted operations therefore preserve tile order,
...
DFB provenance follows checked singleton-dimension views, identity casts, and
one-to-one conversions between TTL DFB values and TTKernel circular-buffer
handles when the source and result element types match. Arbitrary
...
initialization, and waited-buffer mutation analysis use this shared contract.
Unsupported expand/collapse operations terminate provenance tracing before the
generic `ViewLikeOpInterface` fallback because that interface alone does not
prove the block-view contract.
