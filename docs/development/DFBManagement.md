...
The lifetime proof assumes that every DFB accessed by an external callee or a
transitive helper appears as a direct DFB operand of `ttl.opaque_call`. A DFB
index passed only as an integer or template argument does not satisfy this
contract. The compiler cannot inspect the C++ body for hidden constants, global
state, or helper dependencies, so validity of the declared access set is an
external-code assumption.
...
buildPhysicalAllocationPlan(module, logicalIdentities, lifetimes, coloring):
  ...
  if reuseUserDFBs:
    ...
    compilerAssignments = colorConcurrentLifetimes(
        compilerDFBs, firstPhysicalIndex=compactedUserIndices.size)
    assignments = compactedUserIndices + compilerAssignments
...
- **Structured control flow and index reuse.** Lifecycle operations inside `scf.if`,
  ...
  `insideMutuallyExclusiveRegions` proves branch exclusivity. PipeNet schedule
  verification also expands structured and interprocedural event occurrences
  without imposing an order on mutually exclusive regions. The missing work is
  to integrate these facts into the cross-kernel DFB happens-before graph and
  prove bounded region lifetimes.
  MLIR
...
- **Launch-node domains.** Reuse currently requires the same producer and
  ...
  domains. Reusing `LaunchNodeDomainAnalysis` could permit domain-local reuse
  when no physical node observes both lifetimes; it already provides
  launch-coordinate-specific domains and execution-count queries to the
  PipeNet verifiers.
