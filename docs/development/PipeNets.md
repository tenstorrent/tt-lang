...
capacity counters. It allocates completion counters first. It then allocates
readiness counters locally only if the entire per-kernel readiness allocation
fits in the remaining local ids; otherwise all readiness counters use
...
Completion colors consume the first local semaphore ids. If every readiness
color fits in the remaining local ids, readiness counters use those ids and the
same color is reused on different source nodes. Otherwise all readiness
counters use GlobalSemaphore storage, with one allocation per source-node
color. This all-global readiness rule keeps one runtime descriptor form for a
kernel's readiness counters.
