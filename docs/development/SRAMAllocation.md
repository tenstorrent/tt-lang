...
SRAM placement is a reusable, compiler-internal C++ library and API. Callers provide an immutable allocation problem containing payload extents, a symmetric conflict matrix, target alignment, the payload base after all control records, and the SRAM budget. A strategy implementation returns one byte offset per region and the payload high-water mark through the common validated interface. New strategies can be added without changing MLIR analysis, target code generation, or result validation. Storage-owner construction produces one region for each owner with a compiler-owned payload. Tensor-backed owners consume control records but add no placement region.
