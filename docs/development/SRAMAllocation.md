...
```text
buildStorageConflicts(lifetimes, storageMode, selectedNodes = all nodes):
    ...
    for each unordered pair (left, right):
        if either launch domain or access completion is unproved:
            ...
            continue
        for each selected node where both may be active:
...
### Worker-Node Allocation Domains
...
buildDomains(operation):
    ...
    for each multicast receiver set:
        union all receiver nodes
    for each resulting domain, in stable node order:
        include every storage owner that may be active on any member node
        build storage conflicts using completion ordering on member nodes
...
bindDomains(invocation):
    validate finalized domain metadata and specialized kernel node sets
    allocate one zero-initialized SRAM tensor per domain
        singleton: use TT-Metal per-core allocation
        multiple nodes: use TT-Metal lockstep allocation
    for each selected device and specialized kernel:
        bind that node's arena base and payload offsets
        bind computed receiver bases using destination device and node identities
...
Domain placement avoids reserving the busiest node's layout everywhere. In the two-node 16-tile/one-tile regression, uniform allocation reserves 65,664 bytes for BF16 and 131,200 for FP32. Per-node allocation reserves 34,944 and 69,760 respectively, including control prefixes. These are measured backing extents, not execution-speed results or a claim of globally optimal host placement.
...
| Domain-specific reuse | [Domain tests](../../test/python/sram_domains.py) cover uneven demand, per-node lifetime differences, multicast address equality, and reported reservation. Host tests cover distinct device addresses and malformed bindings. |
...
3. Unified host reservation. Obtain complete occupied ranges from the owning host allocator, reserve a prepared requirement set transactionally, and construct owner-retaining tensor views. Preserve caller-owned addresses and reject stale occupancy snapshots before publication. Reuse TTNN/TT-Metal host facilities where their contracts suffice; extend host APIs where required.
