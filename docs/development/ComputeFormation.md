# Compute Formation and Fusion

## Purpose

Compute formation converts tensor operations and their output stores into
`ttl.compute` operations containing tile-level recipes. Fusion absorbs a
supported tensor expression into one formed compute so intermediate values can
remain in DST instead of being stored to dataflow buffers (DFBs).

Formation changes both evaluation position and output publication. A tensor
operation may be defined before a DFB release, used in another region, stored
through several reserves, or shared by several consumers. Correctness therefore
requires more than following SSA definitions. The compiler must prove:

- every input DFB remains available where the formed compute executes;
...
- every `ttl.store` is assigned to a formation or a DFB-to-DFB passthrough
  before conversion modifies IR.

The implementation separates analysis from mutation. Each planning pass builds
a complete immutable kernel plan, validates the plan, and then applies only the
recorded recipes. This follows the same analyze-then-apply principle as
One-Shot Bufferize and LLVM VPlan while adding TTL-specific DFB and tile
semantics.
...
A **formation source** is a tensor operation considered for replacement by
`ttl.compute`.

A **direct formation** lowers one source operation with an operation-specific
tile recipe.

A **fused formation** traces from a source result through supported producer
operations and emits their tile recipes in one `ttl.compute` body. A producer
included in this expression is **absorbed** by the formation.
...
formation position through the DFB availability analysis.
...
An **elision** replaces an operation without forming a compute or changing its
evaluation position. Identity `ttl.typecast` is the current elision.

## Pipeline Contract

Compute formation and intermediate materialization use this order:
...
`ttl-form-producer-compute` forms candidates that are legal in the original
kernel. Rejected candidates remain unchanged so intermediate DFB insertion can
...
selected consumer operands. `convert-ttl-to-compute` then rebuilds formation
...
available before an operation. `ComputeFormationPlanner` constructs typed
...
new storage before final formation.
...
Candidate construction records the source operands and result uses before
selecting a construction strategy:
...
- **rejected**: valid IR cannot use this formation, but the candidate or its
...
Direct recipes define the complete iteration and tile-level construction for
...
`rootInputs` defines the eventual formed compute inputs. A materialized operand
...
contraction roles. A root used in multiple roles appears in the formed
compute's input list once per distinct role so each occurrence receives the
correct affine map.
...
Formation replaces tensor stores with tile stores inside `ttl.compute` and may
...
formation unless each lifetime root is definitely available there and the
anchor dominates every result use not removed by the formation.

Existing pushes are resolved again during application because an earlier
formation may relocate a shared push. The reserve and store identities remain
...
output store. A complete signpost scope may remain around the formed compute.
A partially overlapping or unmatched scope rejects fusion. An intervening
operation that cannot move with the fused expression rejects instrumented
fusion because relocating the observation across it would change the observed
program.
...
expressions may overlap, and applying one formation may erase a source needed
...
Both `ttl.exp` formations may absorb `ttl.add`, but `%sum` must also be formed
for its independent publication. The absorbing formations execute before the
independent `%sum` formation. Each earlier formation removes only its own
consumer; another use keeps `ttl.add` present until all absorbers have run.
...
buildKernelPlan(kernel):
  ...
  repeat:
    accept a result-use-dominance rejection when every offending use is
      removed by an already legal fused formation
  ...
  for each legal candidate:
    ...
    omit an absorbed candidate erased by another selected formation
  ...
  topologically schedule the remaining formation dependencies
...
number. It also records uses that preceding formations must remove and one use
...
a fixed point before mutation. A formation trace stops at those operands and
...
Producer formation elides the identity typecast at its original position. The
...
replacement compiler DFB is a root of the eventual reduce formation, but the
...
`ttl-form-producer-compute` and `convert-ttl-to-compute` use the same planner
and application patterns with different completeness policies. Producer
formation permits unassigned stores that intermediate DFB insertion may
...
2. **Input storage.** Every current-storage input is definitely available at
   the formation anchor. A planned materialization replaces its exact consumer
...
4. **Publication order.** Each tile store retains its reserve transaction.
   ...
   transactions of one DFB reject combined formation.

5. **SSA uses.** The insertion anchor dominates every surviving result use.
   Uses that a preceding formation must erase are recorded and verified.

6. **Overlapping candidates.** An absorbed source is either erased by one
   selected formation or retained until every preceding absorber executes. A
   recorded surviving use proves the latter condition during application.

7. **Analysis stability.** No mutation occurs until the whole kernel plan is
   valid. Application checks the source operands, result uses,
   instrumentation, and output transactions used by the proof.

8. **Conversion completeness.** Final conversion assigns every `ttl.store` to
   a selected compute formation or passthrough plan before modifying IR.
...
compiler DFB; it never proves an unsafe formation legal.
...
- `TilingInterface`, `DestinationStyleOpInterface`, and
  `IndexingMapOpInterface` to expose the formed compute's planned iteration
  semantics to later transformations;
...
- DFB value availability at a formation position;
...
`ttl-print-compute-formation-plans` prints candidate kinds, recipes, iteration
...
disabled compiler DFBs. Runtime tests validate representative formation and
...
- Output stores for one formation source must share a block. Conditional
  routing to different output DFBs is diagnosed instead of asserting, but it
  is not yet formed.
- Tensor formation candidates currently require one result. Existing
...
- `lib/Dialect/TTL/Transforms/ComputeFormationPlanning.{h,cpp}` defines typed
...
- `lib/Dialect/TTL/Transforms/TTLPrintComputeFormationPlans.cpp` prints
