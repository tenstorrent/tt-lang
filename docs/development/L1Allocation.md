...
| Generated fabric PipeNet execution | Compile-only full-pipeline coverage preserves generated routes, routing-plane operations, compiler-owned receiver offsets, and descriptor independence; runtime-unit coverage verifies per-device route binding for compiler-owned and tensor-backed receiver addresses; Blackhole multi-device correctness covers BF16/FP32, both allocator strategies, both receiver storage forms, repeated invocation, and descriptor rejection |
