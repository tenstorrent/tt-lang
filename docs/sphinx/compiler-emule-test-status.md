...
- **132 dynamic-buffer tests:** low-level generated code is incompatible with
  the x86 host JIT. Diagnostics include rejected RISC-V inline assembly and
  missing identifiers such as `p_gpr_unpack`, `LO_16`, and `sync_regfile_write`.
...
These are failure groups rather than 165 independent bugs. The assembly,
missing-identifier, and missing-header diagnostics identify immediate JIT
failures; ownership of the incorrect-output and DPRINT failures still requires
isolated reproducers. They are not all established tt-emule defects. The MLIR,
bindings, and packaging groups had no failures; end-to-end tests had no
unexpected failures.
