...
Migration-only `thread=` accepts `"trisc"`, `"ncrisc"`, or `"brisc"` and maps
the call to the corresponding logical kernel. It is mutually exclusive with
`kernel=` and is removed before AST lowering.
