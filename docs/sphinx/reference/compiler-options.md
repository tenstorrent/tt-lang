...
| `--ttl-l1-budget N` | target-dependent | Override the per-core L1 budget used for target-aligned DFB allocation, synchronized reset and reconfiguration state, PipeNet resources, and final combined validation. |
...
| `l1-budget-override` | uint32_t | `0` (target default) | Override the per-core L1 budget used for target-aligned DFB allocation, synchronized reset and reconfiguration state, PipeNet resources, and final combined validation. |
...
- `ttl-form-pipe-transports` -- group eligible repeated PipeNet transfers and select bounded receiver storage while reserving synchronized-reset state and conservative PipeNet and reconfiguration resources
...
- `ttl-validate-cb-budget` -- verify target-aligned static DFB storage, synchronized-reset state, and reconfiguration tensors fit the per-core L1 budget
- `convert-ttl-to-ttkernel` -- lower TTL DMA, PipeNet, synchronized-reset, and reconfiguration operations to TTKernel, select their runtime resources, and validate the exact combined per-core L1 allocation
...
| `l1-budget-override` | uint32_t | `0` (target default) | Override the per-core L1 budget used by target-aligned physical DFB allocation, reset and reconfiguration state, and the conservative PipeNet reservation. |
...
Validate target-aligned finalized static DFB storage, allocator-rounded
synchronized-reset state, and one target-aligned configuration tensor per
reconfiguration boundary against the per-core L1 budget. Tensor-backed DFB
storage is excluded because the tensor allocator owns it. Exact PipeNet scratch
and GlobalSemaphore allocations are added by `convert-ttl-to-ttkernel` after
transport planning.

| Option | Type | Default | Description |
|---|---|---|---|
| `l1-budget-override` | uint32_t | `0` (target default) | Override the per-core L1 budget used for finalized DFB and fixed-state validation. |
