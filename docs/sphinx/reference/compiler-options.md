...
| `--ttl-specialize-cores` / `--no-ttl-specialize-cores` | disabled | Clone each TTKernel function whose control flow branches on a core coordinate once per launch coordinate (`ttkernel-specialize-cores`), replace `my_logical_x_` / `my_logical_y_` with constants, remove branches whose conditions become exact, and tag clones with `ttl.core_coord` for per-core dispatch. Opt-in. |
