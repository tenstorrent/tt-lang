...
| `--ttl-l1-budget N` | target-dependent | Override the per-core L1 budget used for target-aligned DFB allocation, PipeNet resources, reconfiguration state, and final combined validation. |
...
| `l1-budget-override` | uint32_t | `0` (target default) | Override the per-core L1 budget used for target-aligned DFB allocation, PipeNet resources, reconfiguration state, and final combined validation. |
...
- `ttl-validate-cb-budget` -- verify target-aligned finalized DFB storage and reconfiguration tensors fit the per-core L1 budget
- `convert-ttl-to-ttkernel` -- lower TTL DMA and PipeNet operations to TTKernel, select addressing and synchronization resources, and validate the exact combined per-core L1 allocation
...
| `exact-coloring-search-limit` | uint64 | `1000000` | Examine at most this many states during deterministic exact DFB allocation. Required index or authoritative weighted-L1 searches report an inconclusive diagnostic at the limit. A reservation-only weighted search retains an authoritative-fit assignment when inconclusive. |
| `l1-budget-override` | uint32 | `0` (target default) | Override the per-core L1 budget used by target-aligned physical DFB allocation, reconfiguration state, and the conservative PipeNet reservation. |
...
Validate the target-aligned allocation for finalized physical DFBs and one
configuration tensor per synchronized reconfiguration boundary. Tensor-backed
DFB storage is excluded because the tensor allocator owns it. Exact PipeNet
scratch and GlobalSemaphore allocations are added during
`convert-ttl-to-ttkernel`.

| Option | Type | Default | Description |
|---|---|---|---|
| `l1-budget-override` | uint32_t | `0` (target default) | Override the per-core L1 budget used for finalized DFB and reconfiguration-state validation. |
...
record-selected callback resources, and reconfiguration state. A group size of
one records the reservation without grouping. Exact combined validation occurs
...
| `l1-budget-override` | uint32_t | `0` (target default) | Override the exact combined per-core budget for target-aligned finalized DFBs, reconfiguration tensors, PipeNet scratch, and GlobalSemaphore allocations. |
