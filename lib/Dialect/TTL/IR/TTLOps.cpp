...
verifySelectedPipeRecords(mlir::Operation *op,
  ...
  bool recordsAreMulticast = records.getPipes().front().getIsMulticast();
  if (recordsAreMulticast != isMulticast) {
    return op->emitOpError()
           << "isMulticast must match the uniform records kind";
  }
  return mlir::success();
}
...
static int64_t getPipeNetIdFromReference(mlir::Value pipe) {
  pipe = mlir::tt::ttl::traceUnrealizedCasts(pipe);
  ...
  assert(selectedDst && "selected pipe direct definition verified earlier");
...
selectedPipeKindMatchesTransfer(mlir::Value pipe,
  ...
  if (auto selectedSrc = pipe.getDefiningOp<mlir::tt::ttl::SelectPipeSrcOp>()) {
    ...
    isMulticast = selectedDst.getIsMulticast();
  } else {
    return true;
  }
...
mlir::LogicalResult mlir::tt::ttl::PipeTransferPostOp::verify() {
  ...
  Type pipeType = traceUnrealizedCasts(createOp.getPipe()).getType();
  if (mlir::isa<SelectedPipeSrcType>(pipeType)) {
    return emitOpError()
...
mlir::LogicalResult mlir::tt::ttl::PipeTransferSendOp::verify() {
  ...
  Type pipeType = traceUnrealizedCasts(createOp.getPipe()).getType();
  if (mlir::isa<SelectedPipeDstType>(pipeType)) {
    return emitOpError()
           << "requires a static pipe or source-selected pipe transfer";
  }
