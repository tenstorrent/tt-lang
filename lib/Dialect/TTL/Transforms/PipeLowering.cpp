...
static Value buildSelectedReadyCounterAddress(
  ...
  SmallVector<int64_t> addressIndices;
  addressIndices.reserve(resources.size());
  for (const PipeResourceInfo &resource : resources) {
    ReadyCounterAddressInfo addressInfo =
        getReadyCounterAddressInfo(op, resource, pipeResourcePlan);
    assert(addressInfo.storage == storage &&
           "selected resource ready-counter kind mismatch");
    addressIndices.push_back(addressInfo.index);
  }

  Value selectedIndex =
      loadIndexTableEntry(op, loc, addressIndices, recordIndex, rewriter);
  switch (storage) {
  case ReadyCounterAddressStorage::LocalSemaphore:
...
lowerSelectedPipeTransferSend(PipeTransferSendOp op, Value srcCB,
      ...
      scf::IfOp::create(rewriter, loc, isCollective, /*withElseRegion=*/true);
  {
    ...
    ttk::NocAsyncWriteOp::create(
        rewriter, loc, srcAddr, ValueRange{dstStartXVal, dstStartYVal},
        /*dstBankId=*/ValueRange{}, dstAddr, totalSizeVal);
  }
