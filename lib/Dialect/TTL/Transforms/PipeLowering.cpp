...
public:
  ...
  LogicalResult emitPayloadPageWrites(Value srcAddr, Value dstAddr,
                                      ...
                                      int64_t payloadPageSizeBytes) override {
    assert(pipeType.hasSingleReceiver() &&
           "fabric page writes require a unicast transport");
    assert(payloadPageCount > 1 && payloadPageSizeBytes > 0 &&
           "fabric page writes require a multi-page payload");
    sourceAddress = srcAddr;
    destinationAddress = dstAddr;
    sizeBytes = {};
    pageCount = payloadPageCount;
    pageSizeBytes = payloadPageSizeBytes;
    return success();
  }
  ...
  LogicalResult emitReceiverCompletionIncrement(
      Value receiverCompletionCounterAddr) override {
    assert(sourceAddress && destinationAddress &&
           (sizeBytes || pageCount > 0) &&
    ...
    if (pageCount > 0) {
      Value lowerBound = arith::ConstantIndexOp::create(rewriter, loc, 0);
      Value upperBound =
          arith::ConstantIndexOp::create(rewriter, loc, pageCount);
      Value step = arith::ConstantIndexOp::create(rewriter, loc, 1);
      Value pageSize =
          arith::ConstantIntOp::create(rewriter, loc, pageSizeBytes, 32);
      auto pageLoop =
      ...
      Value pageOffset =
          arith::MulIOp::create(rewriter, loc, pageIndex, pageSize);
      Value pageSourceAddress =
          arith::AddIOp::create(rewriter, loc, sourceAddress, pageOffset);
      Value pageDestinationAddress =
          arith::AddIOp::create(rewriter, loc, destinationAddress, pageOffset);
      Value remotePageDestinationAddress = buildRemoteNocAddress(
          pipeType.getDstStartX(), pipeType.getDstStartY(),
          pageDestinationAddress);
      Value lastPage =
          arith::ConstantIndexOp::create(rewriter, loc, pageCount - 1);
      Value isLastPage =
          arith::CmpIOp::create(rewriter, loc, arith::CmpIPredicate::eq,
                                pageLoop.getInductionVar(), lastPage);
      Value zeroIncrement = arith::ConstantIntOp::create(rewriter, loc, 0, 32);
      Value oneIncrement = arith::ConstantIntOp::create(rewriter, loc, 1, 32);
      Value completionIncrement = arith::SelectOp::create(
          rewriter, loc, isLastPage, oneIncrement, zeroIncrement);
      ttk::RoutingPlaneFusedWriteAtomicIncOp::create(
          rewriter, loc, runtime.manager, runtime.routeId,
          buildConnectionIndex(), target.destinationDeviceId,
          target.destinationMeshId, pageSourceAddress, pageSize,
          remotePageDestinationAddress, remoteCompletionSemaphoreAddress,
          completionIncrement);
      return success();
    }
