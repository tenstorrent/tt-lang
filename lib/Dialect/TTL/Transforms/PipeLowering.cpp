...
getDeviceTransfer(PipeTransferCreateOp transferCreate) {
  return transferCreate.getDeviceTransferAttr();
}
...
LogicalResult buildFabricRoutePlan(ModuleOp mod, ValueOriginAnalysis &analysis,
  ...
  mod.walk([&](Operation *operation) {
    ...
    if (pipeReference.isStatic()) {
      ...
      plan.transferOps.insert(operation);
      if (!isSend) {
        return;
      }
      std::optional<std::size_t> routeIndex =
          recordRoute(operation, transfer, transfer.getEdge().getSource(),
                      transfer.getEdge().getDestination(),
                      getPipeSourceKey(pipeReference.getStaticPipeType()));
      if (routeIndex) {
      ...
      if (isSend) {
        ...
        for (int64_t destinationX = record.getDstStartX();
          ...
          for (int64_t destinationY = record.getDstStartY();
            ...
            std::optional<std::size_t> currentRouteIndex =
...
public:
  SelectedFabricPipeTransportEmitter(Operation *op, SelectedPipeFields fields,
    ...
    for (std::size_t routeIndex : routeIndices) {
      routeIndexTable.push_back(static_cast<int64_t>(routeIndex));
    }
    routeIndex =
