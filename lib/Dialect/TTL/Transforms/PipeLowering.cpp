...
static FailureOr<PipeReference>
...
lookupSelectedPipeResources(Operation *protocolOp,
                            const PipeResourcePlan &pipeResourcePlan) {
  auto it = pipeResourcePlan.selectedResources.find(protocolOp);
...
lowerSelectedPipeTransferSend(PipeTransferSendOp op, Value srcCB,
                              bool isConsumerCB, const PipeReference &pipeRef,
                              const PipeResourcePlan &pipeResourcePlan,
                              ConversionPatternRewriter &rewriter) {
  auto loc = op.getLoc();
  SelectedPipeFields fields = getSelectedPipeFields(pipeRef);
  ArrayRef<PipeResourceInfo> resources =
      lookupSelectedPipeResources(op.getOperation(), pipeResourcePlan);
...
LogicalResult lowerPipeTransferSend(
  ...
  if ((*pipeRef).isSelected()) {
    return lowerSelectedPipeTransferSend(op, srcCB, isConsumerCB, *pipeRef,
                                         pipeResourcePlan, rewriter);
  }
  ...
  bool usesCapacityProtocol = pipeCapacityPlan.usesCapacityProtocol(op);
...
static LogicalResult lowerSelectedPipeTransferPost(
    PipeTransferPostOp op, Value dst, const PipeReference &pipeRef,
...
LogicalResult lowerPipeTransferPost(
    PipeTransferPostOp op, Value dst, ValueOriginAnalysis &analysis,
  ...
  if ((*pipeRef).isSelected()) {
    return lowerSelectedPipeTransferPost(op, dst, *pipeRef,
  ...
  if (!pipeCapacityPlan.usesCapacityProtocol(op)) {
    assert(pipeResource.readyCounter &&
           "sender-ready protocol selected without a sender-ready counter");
...
LogicalResult lowerPipeTransferWait(PipeTransferWaitOp op, Value tokenSequence,
  ...
  auto selectedResourceIt =
      pipeResourcePlan.selectedResources.find(op.getOperation());
  if (selectedResourceIt != pipeResourcePlan.selectedResources.end()) {
    FailureOr<PipeReference> pipeRef =
        getPipeReferenceForProtocolOp(op.getOperation(), analysis);
    if (failed(pipeRef)) {
      return failure();
    }
    assert((*pipeRef).isSelected() &&
           "selected resource table requires a selected pipe");
    SelectedPipeFields fields = getSelectedPipeFields(*pipeRef);
    SmallVector<PipeCounterInfo> completionCounters = llvm::map_to_vector(
        selectedResourceIt->second, [](const PipeResourceInfo &resource) {
          return resource.completion.counter;
        });
...
getPipeReferenceForProtocolOp(Operation *protocolOp,
                              ValueOriginAnalysis &analysis) {
  FailureOr<PipeTransferCreateOp> createOp = failure();
  if (auto sendOp = dyn_cast<PipeTransferSendOp>(protocolOp)) {
    createOp =
        getPipeTransferCreate(protocolOp, sendOp.getTransfer(), analysis);
  } else if (auto postOp = dyn_cast<PipeTransferPostOp>(protocolOp)) {
    createOp =
        getPipeTransferCreate(protocolOp, postOp.getTransfer(), analysis);
  } else if (auto waitOp = dyn_cast<PipeTransferWaitOp>(protocolOp)) {
    FailureOr<PipeTransferPostOp> postOp =
  ...
  if (failed(createOp)) {
    return protocolOp->emitError(
        "pipe protocol operation has no transfer declaration");
  }
  return getPipeReference(protocolOp, createOp->getPipe());
}
...
collectPipeTransferAllocationUnits(
  ...
  WalkResult provenanceWalkResult = mod.walk([&](Operation *op) {
    ...
    if (auto waitOp = dyn_cast<PipeTransferWaitOp>(op)) {
      ...
      if (!pipeGraph.hasPipeTransferNodeForProtocolOp(postOp.getOperation())) {
        staticallyInactiveOps.insert(op);
        return WalkResult::advance();
      }
  ...
  auto recordSelectedProtocolRow = [&](PipeTransferAllocationUnit &unit,
    ...
    FailureOr<PipeReference> pipeRef =
        getPipeReferenceForProtocolOp(protocolOp, analysis);
    if (failed(pipeRef)) {
      return failure();
    }
    if ((*pipeRef).isStatic()) {
...
static bool usesSenderReadyCounter(const PipeTransferAllocationUnit &unit,
  ...
  if (!pipeCapacityPlan || isSelectedTransferUnit(unit)) {
    return true;
  }
