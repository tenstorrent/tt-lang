...
LogicalResult lowerPipeTransferSend(
  ...
  // Completion uses non-posted atomics. The send ttl.wait lowers to a no-op, so
  // this barrier is the only flush before kernel exit; without it receivers can
  // observe stale completion counts.
