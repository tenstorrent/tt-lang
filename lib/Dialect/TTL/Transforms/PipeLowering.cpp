...
protected:
  ...
  LogicalResult emitUnicastPayloadPageWrites(Value srcAddr, Value dstAddr,
                                             int64_t pageCount,
                                             int64_t pageSizeBytes,
                                             TranslatedCore dstCore) {
    ...
    ttk::NocAsyncWriteOp::create(rewriter, loc, pageSrcAddr,
                                 ValueRange{dstCore.x, dstCore.y}, ValueRange{},
                                 pageDstAddr, pageSize, nocVal);
    return success();
  }
...
public:
  ...
  LogicalResult emitPayloadPageWrites(Value srcAddr, Value dstAddr,
    ...
    return emitUnicastPayloadPageWrites(srcAddr, dstAddr, pageCount,
                                        pageSizeBytes, getDstStartCore());
  }
...
public:
  ...
  LogicalResult emitPayloadPageWrites(Value srcAddr, Value dstAddr,
                                      int64_t pageCount,
                                      int64_t pageSizeBytes) override {
    assert(!fields.isCollective &&
           "selected page writes require a unicast transport");
    return emitUnicastPayloadPageWrites(srcAddr, dstAddr, pageCount,
                                        pageSizeBytes, getDstStartCore());
  }
...
public:
  ...
  LogicalResult emitPayloadPageWrites(Value, Value, int64_t, int64_t) override {
    assert(false && "fabric transport does not use NoC page writes");
    return failure();
  }
...
LogicalResult lowerStaticPipeTransferSend(
  ...
  const PipeReference &pipeRef = transferPlan.getPipeReference();
  assert(pipeRef.isStatic() && "static sender requires a static pipe");
...
void finalizePipeTransportResources(const PipeTransportPlan &transportPlan,
  ...
  auto compactAddressTableResource = [&](PipeResourceInfo &resource) {
