...
struct LowerComputeToLoops : OpRewritePattern<ComputeOp> {
  ...
  LogicalResult matchAndRewrite(ComputeOp op,
    ...
    // If no sync ops found, this pass can't properly restructure the code.
    // Ensure TTLInsertTileRegsSyncPass runs before this pass.
    if (!phases.commitOp || !phases.waitOp) {
      return rewriter.notifyMatchFailure(
          op, "compute body missing tile_regs_commit/wait; "
              "ensure TTLInsertTileRegsSyncPass runs before this pass");
    }
