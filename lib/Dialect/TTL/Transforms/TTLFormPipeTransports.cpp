...
selectGrouping(PipeTransportLoopCandidate &candidate,
  ...
  for (int64_t groupSize = 2; groupSize <= upperBound; ++groupSize) {
    ...
    std::optional<PipeTransportGrouping> grouping =
        evaluateGrouping(candidate, groupSize, destinationDepth,
                         allocationSummary, existingScratchBytes, budgetBytes);
...
struct TTLFormPipeTransportsPass
  ...
  void runOnOperation() override {
    ...
    FailureOr<PipeGraph> maybePipeGraph =
        PipeGraph::build(module, analysis, transferIndex, foreachLoweringInfo);
