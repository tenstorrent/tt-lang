...
struct PipeWaitAnyAlternative {
  ...
  bool resolveDeviceTransfer = false;
    ...
    for (ArrayRef<Operation *> possiblePosts :
      ...
      for (Operation *post : possiblePosts) {
        ...
        if (auto pipeType = dyn_cast<PipeType>(copyOp.getSrc().getType())) {
          alternatives.push_back({copyOp.getSrc(), pipeType,
                                  DeviceTransferAttr(), true, post});
          allowedDomain =
  ...
  auto postIt =
      llvm::find_if(candidatePostNodes, [&](PipeScheduleNodeId postId) {
        ...
        return postNode.op == alternative.receivePost &&
               ...
               postNode.kernelFunction == waitNode.kernelFunction &&
               haveSamePipeCallSites(postNode.callSites, waitNode.callSites) &&
               getPipeIdentity(postNode.pipeType, postNode.deviceTransfer) ==
                   getPipeIdentity(alternative.pipeType,
                                   alternative.deviceTransfer);
      });
  return postIt == candidatePostNodes.end()
             ? std::nullopt
             : std::optional<PipeScheduleNodeId>(*postIt);
    ...
    if (eventIt != state.pipeEventIndices.end()) {
      for (std::size_t eventIndex : eventIt->second) {
        ...
        if (event.kind == PipeEventKind::ReceiveWaitAny) {
          std::optional<LaunchExecutionLocation> waitAnyLocation;
          for (PipeWaitAnyAlternative &alternative :
               resolvedEvent.waitAnyAlternatives) {
            if (alternative.resolveDeviceTransfer) {
              FailureOr<std::optional<DeviceTransferAttr>>
                  maybeDeviceTransfer = findUniquePipeDeviceTransfer(
                      state.valueOrigins, alternative.pipe,
                      resolveActiveFunctionArgument);
              if (failed(maybeDeviceTransfer)) {
                event.op->emitOpError()
                    << "requires every possible wait-any candidate pipe "
                       "definition at this call site to use the same "
            ...
            FailureOr<LaunchExecutionLocation> maybeAlternativeLocation =
                getPipeExecutionLocation(coord, alternative.deviceTransfer,
                                         PipeRole::Destination);
            if (failed(maybeAlternativeLocation)) {
              event.op->emitOpError(
            ...
            if (waitAnyLocation &&
                !(*waitAnyLocation == *maybeAlternativeLocation)) {
              event.op->emitOpError()
                  << "requires all wait-any candidate receives to execute "
                     "at the same logical-device location";
              state.sawError = true;
              return WalkResult::interrupt();
            }
            waitAnyLocation = *maybeAlternativeLocation;
          }
          assert(waitAnyLocation && "wait-any requires a candidate");
          resolvedEvent.pipe = resolvedEvent.waitAnyAlternatives.front().pipe;
          resolvedEvent.pipeType =
              resolvedEvent.waitAnyAlternatives.front().pipeType;
          resolvedEvent.deviceTransfer =
              resolvedEvent.waitAnyAlternatives.front().deviceTransfer;
