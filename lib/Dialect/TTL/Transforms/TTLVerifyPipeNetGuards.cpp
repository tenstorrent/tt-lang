...
WalkResult walkPipeEventsInProgramOrder(
    ...
    llvm::function_ref<WalkResult(const PipeEvent &,
                                  const LaunchExecutionLocation &,
                                  ArrayRef<PipeCallSite>)>
        visitEvent) {
  ...
  if (eventIt != state.pipeEventIndices.end()) {
    ...
    if (knownLaunchNodeDomainContains(event.domain, coord)) {
      ...
      if (failed(maybeDeviceTransfer)) {
        event.op->emitOpError()
            << "requires every possible pipe definition at this call site to "
               "be ttl.create_pipe with the same device transfer";
        state.sawError = true;
        return WalkResult::interrupt();
      }
      ...
      if (!hasZeroExecutionCount(callSites, resolvedEvent.op, *maybeLocation,
                                 state) &&
