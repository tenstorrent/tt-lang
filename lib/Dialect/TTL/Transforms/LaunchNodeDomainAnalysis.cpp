...
static llvm::APInt getIntegerValue(Value value, int64_t integerValue) {
  ...
  return llvm::APInt(bitWidth, static_cast<std::uint64_t>(integerValue),
                     /*isSigned=*/integerValue < 0);
}

static std::optional<llvm::APInt>
evaluateExecutionCountSymbol(Value value, LaunchNodeCoord coord,
                             const LaunchNodeDomainState &state) {
  if (value.getDefiningOp<CoreXOp>()) {
    return getIntegerValue(value, coord.x);
  }
  if (value.getDefiningOp<CoreYOp>()) {
    return getIntegerValue(value, coord.y);
  }
  if (auto predicate = value.getDefiningOp<PipeNetPredicateOpInterface>()) {
    bool selected = knownLaunchNodeDomainContains(
        state.getRoleDomain(predicate.getReferencedPipeNetId(),
                            predicate.getReferencedRole()),
        coord);
    return getIntegerValue(value, selected);
  }
  return std::nullopt;
}

static std::optional<std::uint64_t>
evaluateExecutionCountRegion(Region &region, LaunchNodeCoord coord,
                             const LaunchNodeDomainState &state) {
...
getStaticExecutionCount(Operation *op, LaunchNodeCoord coord,
  ...
  auto &analysesByCoord = state.executionCountAnalyses[function.getOperation()];
  ...
  if (analysisIt == analysesByCoord.end()) {
    auto analysis = std::make_unique<ExecutionCountAnalysis>(
        ...
        [coord, &state](Value value) {
          return evaluateExecutionCountSymbol(value, coord, state);
        },
        [coord, &state](Region &region) {
          return evaluateExecutionCountRegion(region, coord, state);
        });
...
struct UnresolvedControlContext {
  func::FuncOp function;
  SmallVector<std::pair<Operation *, std::size_t>> frames;
  SmallVector<Value> controlValues;

  bool operator==(const UnresolvedControlContext &rhs) const {
    return function == rhs.function && frames == rhs.frames &&
           controlValues == rhs.controlValues;
  }
};

static std::optional<UnresolvedControlContext>
getUnresolvedControlContext(Operation *op, LaunchNodeCoord coord,
                            const LaunchNodeDomainState &state) {
  UnresolvedControlContext context;
...
static bool haveProvenEqualRuntimeValue(Value value, LaunchNodeCoord lhsCoord,
  ...
  if (auto argument = dyn_cast<BlockArgument>(value)) {
    ...
    if (!equal) {
      if (auto forOp = dyn_cast_if_present<scf::ForOp>(owner->getParentOp());
        ...
        equal = haveProvenEqualRuntimeValue(forOp.getLowerBound(), lhsCoord,
                                            rhsCoord, state, cache) &&
                haveProvenEqualRuntimeValue(forOp.getUpperBound(), lhsCoord,
                                            rhsCoord, state, cache) &&
                haveProvenEqualRuntimeValue(forOp.getStep(), lhsCoord, rhsCoord,
                                            state, cache);
      }
    }
    cache[value] = equal;
  ...
  bool equal = llvm::all_of(definingOp->getOperands(), [&](Value operand) {
    return haveProvenEqualRuntimeValue(operand, lhsCoord, rhsCoord, state,
                                       cache);
  });
  cache[value] = equal;
  return equal;
...
bool haveProvenEqualExecutionMultiplicity(Operation *lhs,
  ...
  std::optional<std::uint64_t> maybeLhsCount =
      getStaticExecutionCount(lhs, lhsCoord, state);
  std::optional<std::uint64_t> maybeRhsCount =
      getStaticExecutionCount(rhs, rhsCoord, state);
  if (maybeLhsCount && maybeRhsCount) {
    return maybeLhsCount == maybeRhsCount;
  }
  std::optional<UnresolvedControlContext> maybeLhsContext =
      getUnresolvedControlContext(lhs, lhsCoord, state);
  std::optional<UnresolvedControlContext> maybeRhsContext =
      getUnresolvedControlContext(rhs, rhsCoord, state);
  ...
  return llvm::all_of(maybeLhsContext->controlValues, [&](Value value) {
    return haveProvenEqualRuntimeValue(value, lhsCoord, rhsCoord, state,
                                       equalValueCache);
  });
}
