...
private:
  /// Adds an isolated event and returns its stable index.
  unsigned addEvent() {
...
collectLogicalDFBs(ModuleOp moduleOp,
  ...
  moduleOp.walk([&](BindCBOp bindOp) {
    ...
    bool declarationIsCompilerCreated =
        bindOp->hasAttr(kCompilerAllocatedAttrName);
    if (inserted) {
      logicalDFBs[logicalIndex].compilerCreated = declarationIsCompilerCreated;
    } else {
      // Any user declaration makes the logical DFB's provisional physical
      // assignment externally supplied.
      logicalDFBs[logicalIndex].compilerCreated &= declarationIsCompilerCreated;
    }
  ...
  moduleOp.walk([&](Operation *operation) {
    // These operations carry DFB identity without accessing its physical
    // storage. Acquire ownership still accounts for tensor uses associated by
    // `ttl.attach_cb`.
    if (!analysisFailure.message.empty() || !mayAccessDFBStorage(operation)) {
      return;
    }
    ...
    for (Value operand : operation->getOperands()) {
      ...
      auto logicalIt = idToLogicalDFB.find(*logicalId);
      assert(logicalIt != idToLogicalDFB.end() &&
             "every resolved identity must have a logical DFB");
