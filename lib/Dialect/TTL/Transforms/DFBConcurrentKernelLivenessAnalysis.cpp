      ...
      for (const DFBAccessOccurrence &access :
        ...
        if (!accessEvents.contains(&access) &&
            getTopLevelKernelOperation(access.operation) ==
                getTopLevelKernelOperation(localReset)) {
...
static DFBQuiescenceProof computeProtocolLifetime(
  ...
  if (!hasProtocolAccess) {
    ...
    lifetime.interfaceStatePreserved = true;
    return {};
  }
...
static DFBQuiescenceProof computePerNodeLifetime(
  ...
  for (auto [epochIndex, accesses] : llvm::enumerate(epochAccesses)) {
    ...
    epoch.interfaceStatePreserved = epochLifetime.interfaceStatePreserved;
    ...
    if (!hasActiveEpoch) {
      ...
      lifetime.interfaceStatePreserved = epochLifetime.interfaceStatePreserved;
      hasActiveEpoch = true;
    }
    ...
    lifetime.interfaceStatePreserved = lifetime.interfaceStatePreserved &&
                                       epochLifetime.interfaceStatePreserved;
