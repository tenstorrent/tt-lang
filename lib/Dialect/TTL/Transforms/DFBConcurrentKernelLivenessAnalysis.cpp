...
/// Structured loops whose Cartesian iteration space executes one operation
/// once. Local access ordering uses loop identity; cross-kernel reset matching
/// uses the constant trip-count sequence.
...
/// Compares dynamic iteration positions across different logical kernels.
/// Equal constant trip counts at each nesting depth define the same ordinal
/// iteration sequence without depending on unrelated loop operation identity.
...
/// Exact-zero accesses cannot contribute ordering edges even when their
/// launch domain is otherwise unknown.
...
/// Retains contradictory order across logical DFB access events. Strict
/// ordering excludes cycles, but unsafe allocation-group policy must
/// distinguish a missing relation from contradictory evidence.
...
static bool runPrecedesWithinEachIteration(const AccessRun &before,
  ...
  if (before.access->operation == after.access->operation) {
    return before.access->getProtocolEffect() &&
           after.access->getProtocolEffect() &&
...
/// sameKindAcquires must contain every same-DFB acquisition of the same kind.
...
static LogicalResult collectLogicalDFBs(
  ...
  for (SynchronizedResetOccurrence &reset : resetOccurrences) {
    ...
    SmallVector<unsigned> physicalAliases = reset.targetLogicalIndices;
    for (unsigned logicalIndex : reset.targetLogicalIndices) {
      ...
      if (allocationGroup) {
        llvm::append_range(
            physicalAliases,
            membersByAllocationGroup.at(allocationGroup.getOrdinal()));
      }
    }
    llvm::sort(physicalAliases);
    physicalAliases.erase(llvm::unique(physicalAliases),
                          physicalAliases.end());
    reset.targetLogicalIndices = std::move(physicalAliases);
  }
...
/// access and protocol synchronization edges depend on domain selection and
/// are added independently after any topology reuse.
...
/// Builds source-order events only for active accesses. Operations in
/// different kernels remain concurrent unless protocol edges order them.
      ...
      for (const DFBAccessOccurrence *after : nestedSingleAccesses) {
        ...
        bool ordered =
            before->operation == after->operation
                ? before->getProtocolEffect() &&
                      after->getProtocolEffect() &&
...
static bool tryAddCumulativeQueueEdges(
  ...
  for (const DFBAccessOccurrence &access : logicalDFB.accesses) {
    ...
    bool producerEffect =
        *protocolEffect == DFBProtocolEffectKind::Reserve ||
        *protocolEffect == DFBProtocolEffectKind::Push;
    bool consumerEffect =
        *protocolEffect == DFBProtocolEffectKind::Wait ||
        *protocolEffect == DFBProtocolEffectKind::Pop;
...
static DFBQuiescenceProof computeProtocolLifetime(
  ...
  bool hasProtocolAccess =
      llvm::any_of(activeAccesses, [](const DFBAccessOccurrence *access) {
        return access->getProtocolEffect() != nullptr;
      });
  ...
  unsigned conditionRelevantAccessCount = 0;
  ...
  if (!conditionalRuns.empty()) {
    ...
    bool sameCondition =
        conditionalRuns.size() == conditionRelevantAccessCount &&
        llvm::all_of(llvm::drop_begin(conditionalRuns),
                     [&](const AccessRun *run) {
                       return proveEquivalentConditionalRuns(reference, *run,
                                                             node, domainState);
                     });
    if (!sameCondition) {
      return {DFBQuiescenceFailureReason::UnsupportedControlFlow,
  ...
  SmallVector<const DFBAccessOccurrence *> earliestAccesses =
      findMinimalEntryAccesses(protocolLifetimeAccesses, graph,
                               operationEvents, accessEvents);
  for (const DFBAccessOccurrence *earliestAccess : earliestAccesses) {
    std::optional<AccessEventSpan> earliestEvents =
        getAccessEventSpan(*earliestAccess, operationEvents, accessEvents);
    assert(earliestEvents && "minimal access must have modeled events");
    if (!llvm::is_contained(lifetime.earliestEntryEvents,
                            earliestEvents->first.entry)) {
      lifetime.earliestEntryEvents.push_back(earliestEvents->first.entry);
    }
    lifetime.earliestAccessOccurrenceIndices.push_back(
        static_cast<unsigned>(earliestAccess - logicalDFB.accesses.data()));
  }
  lifetime.terminalCompletionEvents.push_back(
      protocolTerminalEvents->last.completion);
  lifetime.terminalAccessOccurrenceIndices.push_back(static_cast<unsigned>(
      protocolTerminalAccess - logicalDFB.accesses.data()));
  for (const DFBAccessOccurrence *pointerObservation : pointerObservations) {
    ...
    lifetime.earliestAccessOccurrenceIndices.push_back(static_cast<unsigned>(
        pointerObservation - logicalDFB.accesses.data()));
    if (!llvm::is_contained(lifetime.terminalCompletionEvents,
...
static std::optional<DFBQuiescenceProof> tryComputeRepeatedResetLifetime(
    ...
    const AccessExecutionCounts &executionCounts, const AccessRuns &accessRuns,
    const LaunchNodeDomainState &domainState,
    const StructuralOperationOrder &structuralOrder, bool reportExecutionCounts,
    bool includeUnknownDomains) {
  SmallVector<const DFBAccessOccurrence *> activeAccesses;
  for (const DFBAccessOccurrence &access : logicalDFB.accesses) {
    if (!mayContainLaunchNode(access.launchDomain, node,
...
/// Partitions accesses by collective reset boundaries and proves every
/// resulting epoch independently. A reset restores canonical hardware state;
/// it does not make an otherwise blocking consumer epoch executable.
static DFBQuiescenceProof computePerNodeLifetime(
  ...
  if (std::optional<DFBQuiescenceProof> repeatedResetProof =
          tryComputeRepeatedResetLifetime(
              logicalDFB, logicalIndex, node, lifetimes, synchronizedResets,
              resetBoundaryEvents, graph, operationEvents, accessEvents,
              executionCounts, accessRuns, domainState, structuralOrder,
              reportExecutionCounts, includeUnknownDomains)) {
  ...
  if (boundaries.empty()) {
    return computeProtocolLifetime(
        logicalDFB, node, lifetimes, graph, operationEvents, accessEvents,
        executionCounts, accessRuns, domainState, reportExecutionCounts,
        includeUnknownDomains);
  }
  ...
  for (auto [epochIndex, accesses] : llvm::enumerate(epochAccesses)) {
    ...
    DFBQuiescenceProof proof = computeProtocolLifetime(
        ...
        /*reportExecutionCounts=*/false, includeUnknownDomains, accesses,
  ...
  for (LaunchNodeCoord node : launchNodes) {
    ...
    for (auto [logicalIndex, logicalDFB] : llvm::enumerate(logicalDFBs)) {
      ...
      DFBQuiescenceProof proof = computePerNodeLifetime(
          ...
          structuralOrder, collectAllocationDiagnostics);
    ...
    for (auto [logicalIndex, logicalDFB] : llvm::enumerate(logicalDFBs)) {
      ...
      DFBQuiescenceProof proof = computePerNodeLifetime(
          ...
          possibleAccessRuns, domainState, structuralOrder,
