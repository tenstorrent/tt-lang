...
static LogicalResult collectLogicalDFBs(
  ...
  WalkResult collectionResult = module.walk([&](Operation *operation) {
    ...
    // different summaries. An occurrence without a protocol effect or
    // non-transactional access remains opaque until a synchronized reset proves
    // that its lifetime terminates.
...
struct ProgramOrderTopologyAccess {
  const DFBAccessOccurrence *access = nullptr;
  AccessRun run;
  bool boundaryTarget = false;

  bool operator==(const ProgramOrderTopologyAccess &rhs) const {
    return std::tie(access, run, boundaryTarget) ==
           std::tie(rhs.access, rhs.run, rhs.boundaryTarget);
  }
};

// Inputs that determine event identity and source-order topology. Lifecycle-
// boundary and protocol synchronization edges depend on domain selection and
...
static ProgramOrderTopologyInputs collectProgramOrderTopologyInputs(
  ...
  for (const DFBLogicalLifecycle &logicalDFB : logicalDFBs) {
    for (const DFBAccessOccurrence &access : logicalDFB.accesses) {
      ...
      bool boundaryTarget = boundaryTargetAccesses.contains(&access);
      ...
      bool nestedBoundaryEvent = boundaryTarget &&
                                 projected != access.operation &&
                                 runIt->second.executionCount == 1;
      if (directProtocolEvent || repeatedAccessEvents || nestedBoundaryEvent) {
        inputs.accesses.push_back({&access, runIt->second, boundaryTarget});
      }
    }
  }
...
getEpochQuiescence(const DFBPerNodeLifetime &lifetime, unsigned epochIndex) {
  ...
  assert(epochIndex < lifetime.epochs.size());
  return lifetime.epochs[epochIndex].quiescence;
}
...
void DFBConcurrentKernelLivenessAnalysis::analyze(
  ...
  auto getRefinedAccessDomain =
    ...
    if (domainLattice) {
      ...
      accessDomain = executable && !executable->isLive()
  ...
  for (DFBReconfigurationOccurrence &reconfiguration :
    ...
    reconfiguration.launchDomain =
        getRefinedAccessDomain(reconfiguration.operation).domain;
  }
  ...
  std::optional<ProgramOrderTopologyInputs> cachedGraphTopologyInputs;
  std::optional<ProgramOrderGraphState> cachedGraphTopologyState;
  std::optional<ProgramOrderTopologyInputs> cachedPossibleGraphTopologyInputs;
  std::optional<ProgramOrderGraphState> cachedPossibleGraphTopologyState;
  for (LaunchNodeCoord node : launchNodes) {
    ...
    ProgramOrderTopologyInputs graphTopologyInputs =
        collectProgramOrderTopologyInputs(
            ...
            /*includeUnknownDomains=*/false);
    ProgramOrderGraphState initialGraphState;
    if (cachedGraphTopologyInputs &&
        *cachedGraphTopologyInputs == graphTopologyInputs) {
      assert(cachedGraphTopologyState &&
             "cached topology inputs must have a graph state");
      initialGraphState = *cachedGraphTopologyState;
    } else {
      initialGraphState = buildProgramOrderTopologyState(
          module, graphTopologyInputs, structuralOrder);
      cachedGraphTopologyInputs = graphTopologyInputs;
      cachedGraphTopologyState = initialGraphState;
    }
    ...
    if (hasUnknownDFBLaunchDomain) {
      ...
      } else if (cachedPossibleGraphTopologyInputs &&
                 *cachedPossibleGraphTopologyInputs ==
                     possibleGraphTopologyInputs) {
        assert(cachedPossibleGraphTopologyState &&
               "cached possible topology inputs must have a graph state");
        possibleGraphState = *cachedPossibleGraphTopologyState;
      } else {
        possibleGraphState.emplace(buildProgramOrderTopologyState(
            module, possibleGraphTopologyInputs, structuralOrder));
        cachedPossibleGraphTopologyInputs = possibleGraphTopologyInputs;
        cachedPossibleGraphTopologyState = *possibleGraphState;
      }
#ifndef NDEBUG
  ...
  assert(orderedBeforeByNode.size() == launchNodes.size() &&
         ...
         epochOrderedBeforeByNode.size() == launchNodes.size() &&
         conditionallyEpochOrderedBeforeByNode.size() == launchNodes.size() &&
