...
LogicalResult PipeGraph::assignReceiverAddressSequences(
  ...
  auto processPop = [&](CBPopOp popOp, LaunchNodeCoord coord) -> LogicalResult {
    ...
    PipeReceiverDFBKey receiverDFB{receiverDevice,
                                   PipeReceiverCoord{coord.x, coord.y},
                                   *maybeDFBIndex};
...
FailureOr<PipeGraph> PipeGraph::build(ModuleOp mod,
  ...
  if (failed(
          graph.assignReceiverAddressSequences(mod, transferIndex,
                                               analysisState))) {
    return failure();
  }
