...
getReceiverControlContext(Operation *op, PipeReceiverCoord receiver,
  ...
  while (Block *block = current->getBlock()) {
    ...
    // The graph already represents each selected record separately. Ignoring
    // its generated loop and endpoint guard preserves the callback's order
    // relative to operations before and after the foreach.
...
static bool
isNestedInSelectedRecordControl(Operation *op,
                                const PipeGraphAnalysisState &analysisState) {
...
LogicalResult PipeGraph::assignReceiverAddressSequences(
  ...
  auto processPost = [&](PipeTransferPostOp postOp) -> LogicalResult {
    ...
    FailureOr<PipeTransferCreateOp> createOp =
        findPipeTransferCreateForTransfer(analysis, postOp.getTransfer());
    assert(succeeded(createOp) &&
           "receiver post must have a transfer declaration");
    FailureOr<PipeReference> pipeRef =
    ...
    for (PipeTransferNodeId transferNodeId : transferNodeIds) {
      ...
      pipeKey.forEachReceiver([&](PipeReceiverCoord receiver) {
        ...
        bool reserveRepeatsPerRecord =
            isNestedInSelectedRecordControl(receiverReserveOp, analysisState);
        // A reserve inside the callback advances once per matching record. A
        // reserve outside the callback must retain its one runtime slot.
    ...
    pipeKey.forEachReceiver([&](PipeReceiverCoord receiver) {
      ...
      for (std::size_t sendIndex = 0; sendIndex < candidates.sends.size();
        ...
        FailureOr<PipeTransferCreateOp> postCreate =
            findPipeTransferCreateForTransfer(analysis, postOp.getTransfer());
        assert(succeeded(sendCreate) && succeeded(postCreate) &&
               "pipe protocol operations must have transfer declarations");
