...
struct DFBLogicalLifecycle {
  ...
  /// Kernel-local protocol state prevents reuse across different participants.
  func::FuncOp producerKernel;
  func::FuncOp consumerKernel;
  /// True only when every declaration was created by the compiler.
  bool compilerCreated = false;

  /// Declarations and direct hardware-protocol uses grouped by logical ID.
  ...
  /// Equal lifecycle tile count required for ring-pointer compatibility.
  std::optional<int64_t> transactionTileCount;

  /// Event frontiers used by the pairwise non-overlap proof.
...
public:
  /// Analyzes the `ModuleOp` using the manager's logical-identity result.
  ///
  /// Invalid IR is recorded for the consuming pass to diagnose before mutation.
...
private:
  /// Computes event reachability and logical DFB lifetime frontiers.
