...
buildPipeSendPlan(PipeTransferSendOp sendOp, const DominanceInfo &dominanceInfo,
  ...
  return PipeSendPlan{readFromDFB, payloadSizeBytes, fabricRouteIndex};
}
...
buildPipeModulePlan(ModuleOp module, ValueOriginAnalysis &analysis,
  ...
  auto addTransferPlan =
    ...
    assert((!usesFabricProtocol || staticResources) &&
           "fabric protocol requires a static pipe reference");
    if (usesFabricProtocol &&
      ...
      assert(transferNodeIds.size() == 1 &&
             "static protocol operation requires one PipeGraph transfer");
      const PipeTransferNode &transferNode =
    ...
    if (sendOp) {
      ...
      FailureOr<PipeSendPlan> maybeSendPlan = buildPipeSendPlan(
          sendOp, dominanceInfo, *payloadSize, fabricRoutePlan);
