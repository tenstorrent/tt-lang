...
public:
  TTLToTTKernelTypeConverter() {
    ...
    // High-level pipe copies return TransferHandleType and may carry the post
    // sequence through SCF or tensor containers. DMA handles use the same
...
buildPipeTransferExpansionPlan(ModuleOp mod, ValueOriginAnalysis &analysis) {
  ...
  mod.walk([&](CopyOp op) {
    if (isPipeReceiveCopy(op)) {
      ...
      if (failed(contract) || failed(pipeNetId)) {
        op.emitError()
    ...
    if (isPipeSendCopy(op)) {
      ...
      if (failed(contract)) {
        ...
        plan.sendCopies.push_back({op, *contract, 0});
      }
    }
  });
  if (failed(result)) {
