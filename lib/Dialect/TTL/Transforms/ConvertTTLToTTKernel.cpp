...
static LogicalResult lowerCBToPipe(CopyOp op, Value srcCB, Value pipe,
  ...
  auto pageSizeVal = arith::ConstantOp::create(
      rewriter, loc, i32Ty, rewriter.getI32IntegerAttr(pageSizeBytes));
  ...
  if (pipeType.isUnicast()) {
    ...
    auto semMcastAddr = ttk::GetNocMulticastAddrOp::create(
        rewriter, loc, dstStartXVal, dstStartYVal, dstEndXVal, dstEndYVal, semAddr,
        /*noc=*/Value());
...
static LogicalResult lowerPipeToCB(CopyOp op, Value pipe, Value dstCB,
  ...
  auto oneVal = arith::ConstantOp::create(
