...
// their own. The PipeGraph identifies receiver CBs for each pipe, assigns
// gather slot indices, and tracks the number of PipeNets for semaphore
// allocation.
//===----------------------------------------------------------------------===//
...
public:
  ...
  /// Get the number of distinct PipeNets.
  int64_t getNumPipeNets() const {
    int64_t maxId = -1;
    for (const auto &[key, info] : receiverCBs) {
      maxId = std::max(maxId, key.pipeNetId);
    }
    return maxId + 1;
  }
