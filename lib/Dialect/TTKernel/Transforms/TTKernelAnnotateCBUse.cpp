...
struct TTKernelAnnotateCBUsePass
  ...
  void runOnOperation() override {
    ...
    for (func::FuncOp func : module.getOps<func::FuncOp>()) {
      ...
      func.walk(
          [&](func::CallOp call) { callees[key].push_back(call.getCallee()); });
    }

    // Iterate through all functions and update their usedCBs set.
    // Here we define a function to be "conservative" if it calls a function
    // that is "conservative" or if it calls a function that is not in the
    // symbol map. A "conservative" function is one that uses all CBs up to the
    // max base cta index.
    bool changed = true;
    while (changed) {
      changed = false;
