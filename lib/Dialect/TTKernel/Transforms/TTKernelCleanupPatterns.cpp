...
/// Collect pure same-block value computations that must move before `anchor`.
/// Dependencies precede their users in `definitions`.
static LogicalResult collectValueDefinitionsToMove(
    Value value, Operation *anchor, SmallPtrSetImpl<Operation *> &visited,
...
/// Program posted one-packet write state before sender-side blocking waits.
struct PrearmPostedNocWrite : OpRewritePattern<NocAsyncWriteOp> {
  ...
  LogicalResult matchAndRewrite(NocAsyncWriteOp op,
    ...
    for (Value coordinate : op.getDstCoreXY()) {
      if (failed(collectValueDefinitionsToMove(
              coordinate, firstBlockingWait, movedDefinitions,
              definitionsToMove))) {
        return rewriter.notifyMatchFailure(
    ...
    if (failed(collectValueDefinitionsToMove(
            op.getSize(), firstBlockingWait, movedDefinitions,
            definitionsToMove)) ||
        (op.getNoc() &&
         failed(collectValueDefinitionsToMove(
