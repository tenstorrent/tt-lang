...
class D2MSimpleAllocate
    : public impl::D2MSimpleAllocateBase<D2MSimpleAllocate> {
public:
...
private:
  SmallVector<AllocInfo> collectAllocs(func::FuncOp funcOp,
    ...
    funcOp.walk([&](memref::AllocOp allocOp) {
      ...
      allocation::Planner::AllocSizeT alignment =
          (memSpace == ttcore::MemorySpace::DeviceL1) ? l1Alignment
                                                      : dramAlignment;
      allocation::Planner::AllocSizeT size =
      ...
      allocs.push_back({allocOp, memSpace, size, range, -1});
    });

    return allocs;
  }

  Operation *extendLiveness(
      Value val, Operation *currentEnd,
      const llvm::DenseMap<Operation *, allocation::Planner::SequenceT>
          &opToSeq) {
    ...
    for (Operation *user : val.getUsers()) {
      ...
      if (auto viewOp = dyn_cast<d2m::ViewLayoutOp>(user)) {
        ...
        for (Value result : genericOp.getResults()) {
          Operation *extendedEnd =
              extendLivenessImpl(result, currentEnd, opToSeq, visited);
          if (opToSeq.lookup(extendedEnd) > maxSeq) {
  ...
  LogicalResult allocateL1(func::FuncOp funcOp, ttcore::ChipDescAttr chipDesc,
    ...
    allocation::Planner::AllocSizeT l1Size =
        testAssumeL1Capacity > 0 ? (l1Base + testAssumeL1Capacity)
                                 : chipDesc.getL1Size();
  ...
  void assignAddresses(func::FuncOp funcOp, ttcore::ChipDescAttr chipDesc,
                       SmallVector<AllocInfo> &allocs) {
    ...
    for (auto &info : allocs) {
      if (info.memSpace == ttcore::MemorySpace::DeviceL1) {
        allocation::Planner::AllocSizeT address =
            l1Base + info.size; // size holds offset here
