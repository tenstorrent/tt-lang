...
class DMATransaction:
    ...
    def __init__(
        ...
        match (src, dst):
            case (torch.Tensor(), RingView()):
                # tensor → RingView: supported
                pass
            case (RingView(), torch.Tensor()):
                # RingView → tensor: supported
                pass
            case (RingView(), MulticastAddress()):
                # RingView → MulticastAddress: multicast send (supported)
                pass
            case (MulticastAddress(), RingView()):
    ...
    def wait(self) -> None:
        ...
        if not self._completed:
            ...
            match (self._src, self._dst):
                ...
                case _:
                    # This should never happen since types are validated in __init__
                    raise AssertionError(
                        f"Impossible: unsupported type combination ({type(self._src).__name__}, {type(self._dst).__name__})"
                    )
