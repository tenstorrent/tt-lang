...
def _get_cached_runtime_resources_impl(
    ...
    *,
    ...
    if cache is not None:
        ...
        reconfiguration_l1_addresses = {
            int(tensor.buffer_address())
            for tensor in (
                *reconfiguration_resources.scratch_tensors.values(),
                *reconfiguration_resources.configuration_tensors,
            )
        }
        cache.owned_l1_buffer_addresses = frozenset(
            (*pipe_resources.l1_buffer_addresses, *reconfiguration_l1_addresses)
        )
...
def _build_dfb_descriptors(
    ...
    for members_by_core in static_members_by_storage_by_core.values():
        for core, member_indices in members_by_core.items():
            storage_size = _shared_static_storage_size(
                tuple(member_indices), allocations
            )
            bytes_by_core[core] += _align_up(storage_size, allocation_quantum_bytes)
    ...
    if overflow_cores:
        ...
        for storage_index, members_by_core in sorted(
            ...
            member_indices = members_by_core.get(core)
            if not member_indices:
                continue
            sorted_member_indices = tuple(sorted(member_indices))
            storage_size = _shared_static_storage_size(
                sorted_member_indices, allocations
            )
            storage_groups.append(
                f"  storage[{storage_index}] DFBs={list(sorted_member_indices)} -> "
                f"{_align_up(storage_size, allocation_quantum_bytes)} bytes"
            )
        breakdown = "\n".join(storage_groups)
        raise ValueError(
            ...
            + breakdown
            + "\n  hint: reduce DFB shapes, block_count, or overlapping storage lifetimes."
        )
        ...
        for source, source_cores in cores_by_source.items():
            ...
            descriptor_plans.append(
                _DFBDescriptorPlan(
                    ...
                    total_size=_align_up(
                        allocation.total_size, allocation_quantum_bytes
                    ),
    ...
    for _, members_by_core in sorted(static_members_by_storage_by_core.items()):
        ...
        for member_indices, source_core_set in sorted(cores_by_member_indices.items()):
            ...
            descriptor = ttnn.CBDescriptor(
                ...
                core_ranges=_make_node_core_ranges(source_cores),
                format_descriptors=[
            ...
            descriptor_plans.append(
                _DFBDescriptorPlan(
                    ...
                    total_size=_align_up(total_size, allocation_quantum_bytes),
                    nodes=source_cores,
