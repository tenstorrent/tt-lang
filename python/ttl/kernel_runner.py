...
def build_pipe_computed_address_dfb_tensors(
    ...
    for dfb_index in dfb_indices:
        ...
        total_size = _get_dfb_allocation(config).total_size
...
def build_cb_descriptors(
    ...
    # Compute sizes first so validation completes before descriptor allocation.
    allocations = []
    ...
    for physical_index, config in enumerate(cb_configs):
        ...
        allocations.append((allocation, description))
    ...
    # and the finalized page-size metadata. Computed-address backing
    ...
    static_allocations = [
        entry
        for physical_index, entry in enumerate(allocations)
        if physical_index not in backing_tensors
    ]
    static_cb_bytes = sum(allocation.total_size for allocation, _ in static_allocations)
    if static_cb_bytes > remaining_bytes:
        breakdown = "\n".join(description for _, description in static_allocations)
        raise ValueError(
    ...
    cb_descriptors = []
    for cb_index, (allocation, _) in enumerate(allocations):
...
def emit_runner_source(
    ...
    for physical_index, config in enumerate(cb_configs):
        ...
        lines.append(
            "    PhysicalDFBConfig("
            f"dfb_index={physical_index}, "
            f"num_tiles={allocation.num_tiles}, "
            f"data_format={config.data_format!r}, "
            f"block_count={allocation.block_count}, "
            f"page_size={allocation.page_size}, "
            f"tile={allocation.tile!r}),  # DFB {physical_index}"
        )
    lines.append("]")
    lines.append("")
