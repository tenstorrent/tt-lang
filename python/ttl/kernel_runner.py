...
def _get_dfb_descriptor_configs(
    ...
    for cb_index, dfb_config in enumerate(cb_configs):
        ...
        if isinstance(dfb_config, tuple):
        ...
        elif isinstance(dfb_config, PhysicalDFBConfig):
            _validate_physical_dfb_config(dfb_config, cb_index)
            allocation = _get_dfb_allocation(dfb_config)
            ...
            raise TypeError(
                "DFB runtime configuration must be a finalized "
                f"PhysicalDFBConfig, got {type(dfb_config).__name__}"
            )
...
def build_cb_descriptors(
    ...
    # Compute sizes first so invalid metadata fails before descriptor creation.
    descriptor_configs = _get_dfb_descriptor_configs(cb_configs)
