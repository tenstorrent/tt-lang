...
class KernelSpec:
    ...
    Attributes:
        ...
        thread_type: Type of kernel ("compute" or "noc").
...
def build_kernel_descriptors(
    ...
    Args:
        ...
        tensors: List of device tensors (in global order).
    ...
    # Track which NOC kernel is reader vs writer.
    noc_kernel_idx = 0
...
def build_cb_descriptors(
    ...
    Args:
        ...
        core_ranges: CoreRangeSet for CB allocation.

    Returns:
        List of CBDescriptor objects.
    """
    if ttnn is None:
        raise RuntimeError("ttnn is not available")
...
def run_kernel_on_device(
    ...
    Args:
        kernel_specs: List of kernel specifications (path, thread_type, tensor_indices).
        tensors: List of device tensors (in global order matching CB indices).
        cb_configs: List of (shape, buffer_factor) tuples for each CB.
        core_ranges: CoreRangeSet for kernel execution.

    Returns:
        Result from ttnn.generic_op.
    """
    if ttnn is None:
        raise RuntimeError("ttnn is not available")
