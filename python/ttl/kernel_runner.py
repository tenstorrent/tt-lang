...
def _get_cb_descriptor_rows(cb_configs: List[Any]) -> List[Any]:
    ...
    for cb_index, config in enumerate(cb_configs):
        ...
        if isinstance(config, tuple) and len(config) == 5:
            shape, block_count, data_format, page_size, total_size = config
            rows.append(
                (
                    data_format,
                    page_size,
        ...
        elif isinstance(config, tuple) and len(config) == 6:
            num_tiles, block_count, data_format, tile_shape, page_size, total_size = (
                config
            )
            tile_descriptor = ttnn.TileDescriptor(ttnn.Tile(tile_shape))
            rows.append(
                (
                    data_format,
                    page_size,
                    total_size,
                    tile_descriptor,
                    f"  DFB[{cb_index}]: num_tiles={num_tiles} "
                    f"block_count={block_count} format={data_format} "
                    f"tile={tile_shape} -> {total_size} bytes",
                )
            )
        elif isinstance(config, PhysicalDFBConfig):
            config = _validate_physical_dfb_config(config, cb_index)
            ...
            rows.append(
                (
                    ...
                    f"  DFB[{cb_index}]: num_tiles={allocation.num_tiles} "
            ...
            data_format = (
                ...
                else torch_dtype_to_ttnn_datatype(config.dtype)
            )
            tile = ttnn.Tile(config.tile)
            tile_descriptor = ttnn.TileDescriptor(tile)
            page_size = tile.get_tile_size(data_format)
            num_tiles = config.shape[0] * config.shape[1] * config.block_count
            ...
            rows.append(
                (
                    ...
                    total_size,
                    tile_descriptor,
                    f"  CB[{cb_index}]: shape={config.shape} "
                    f"block_count={config.block_count} -> {total_size} bytes",
                )
            )
...
def build_cb_descriptors(
    ...
    # calculation; see issue #511. Computed-address backing tensors are
    # allocated separately before this check, so their L1 is already reflected in
