...
def validate_cb_descriptors_override(
    ...
    Their capacities may differ.  A descriptor may also expose multiple CB ids
    as format views of its one backing allocation.  L1 usage is checked as the
    maximum sum of backing allocations on any one core, rather than summing
    mutually exclusive descriptors or aliased format views.

    This intentionally supports the static subset of Blaze's descriptor
    model.  It does not support runtime phase reconfiguration or overlapping
    backing allocations.
    """
    ...
    for descriptor_index, descriptor in enumerate(descriptors):
        ...
        if total_size <= 0:
            raise ValueError(
                f"CB descriptor override entry {descriptor_index} requires "
                f"positive total_size; got {total_size}"
            )

        descriptor_cores = _core_range_coordinates(
            getattr(descriptor, "core_ranges", None),
            label=f"CB descriptor {descriptor_index} core ranges",
        )
        outside = descriptor_cores - program_cores
        if outside:
            raise ValueError(
                f"CB descriptor {descriptor_index} claims cores outside the program grid: "
                f"{sorted(outside)}"
            )
        ...
        for fmt in formats:
            ...
            if cb_id in descriptor_ids:
                raise ValueError(
                    f"CB descriptor override entry {descriptor_index} repeats "
                    f"CB id {cb_id}"
                )
            descriptor_ids.add(cb_id)
        ...
        allocation_label = min(descriptor_ids)
        for core in descriptor_cores:
            bytes_by_core[core] += total_size
            claim_sizes_by_core[core].append((allocation_label, total_size))
