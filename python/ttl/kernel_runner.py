...
def build_pipe_computed_address_dfb_tensors(
    ...
    for dfb_index in dfb_indices:
        ...
        outside = backing_cores - program_cores
        if outside:
            raise ValueError(
                f"DFB[{dfb_index}] allocation nodes {sorted(outside)} are "
                "outside the program grid"
            )
        ...
        if not backing_cores:
            # The PipeNet runtime ABI always passes a receiver address. A
            # one-core backing allocation supplies it without assigning the
            # unreachable DFB descriptor to any executing core.
...
def _resolve_cb_placements(
    ...
    for cb_index, config in enumerate(cb_configs):
        ...
        outside = allocation_cores - program_cores
        if outside:
            raise ValueError(
                f"DFB[{cb_index}] allocation nodes {sorted(outside)} are "
                "outside the program grid"
            )
...
def _order_static_dfb_descriptor_plans(
    ...
    if current_score[0] > 0:
        ...
        raise ValueError(
            "Static DFB descriptor packing on core "
            f"{overflow_core} requires {required_bytes} bytes but only "
            f"{available_bytes} bytes remain; deterministic descriptor ordering "
            f"still exceeds the L1 budget by {required_bytes - available_bytes} bytes"
        )
...
def _build_partitioned_cb_descriptors(
    tensors: List[Any],
    cb_configs: List[PhysicalDFBConfig],
    ...
    if overflow_cores:
        ...
        breakdown = "\n".join(
            ...
            f"{allocations[index].total_size} bytes"
    ...
    for cb_index, placement in enumerate(placements):
        ...
        for source, source_cores in cores_by_source.items():
            ...
            sorted_source_cores = tuple(sorted(source_cores))
            descriptor_plans.append(
                _DFBDescriptorPlan(
                    descriptor=descriptor,
                    physical_index=cb_index,
                    total_size=allocation.total_size,
                    nodes=sorted_source_cores,
...
def build_cb_descriptors(
    ...
    placements = _resolve_cb_placements(
    ...
    if placements is not None:
        ...
        return _build_partitioned_cb_descriptors(
            tensors,
            cb_configs,
            allocations,
            placements,
...
def emit_runner_source(
    ...
    if requires_runtime_resource_factory:
        lines.append("_RUNTIME_RESOURCE_LIFETIMES = ()")
    ...
    lines.append("")
    if requires_runtime_resource_factory:
        lines.append("def _commit_runtime_resource_lifetimes(lifetimes):")
        lines.append("    global _RUNTIME_RESOURCE_LIFETIMES")
        lines.append("    _RUNTIME_RESOURCE_LIFETIMES = lifetimes")
        lines.append("")
    ...
    if requires_runtime_resource_factory:
        ...
        lines.append(
            ...
            "_commit_runtime_resource_lifetimes,"
        )
