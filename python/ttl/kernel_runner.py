...
class KernelSpec:
    ...
    Attributes:
        ...
        sram_receiver_targets: Destination DFB, core, and device identities for
            computed receiver arguments when SRAM layouts differ by core.
...
def _partition_core_ranges_by_per_core_tensor_addresses(
    core_ranges: Any,
    spec: KernelSpec,
    tensors: List[Any],
    mesh_coordinate: Optional[Tuple[int, ...]],
) -> List[Tuple[Any, Optional[Tuple[int, int]]]]:
    per_core_tensor_indices = list(
        dict.fromkeys(
            tensor_index
            for tensor_index in spec.tensor_indices
            if tensors[tensor_index].is_per_core_allocated()
        )
    )
    if not per_core_tensor_indices:
        return [(core_ranges, None)]

    from ._sram_domains import tensor_base

    coordinates_by_addresses: Dict[Tuple[int, ...], List[Tuple[int, int]]] = {}
    for core in ttnn.corerange_to_cores(core_ranges, row_wise=True):
        ...
        addresses = tuple(
            ...
            for tensor_index in per_core_tensor_indices
        )
        coordinates_by_addresses.setdefault(addresses, []).append(coordinate)

    return [
        (_make_singleton_core_ranges(coordinates), coordinates[0])
        for coordinates in coordinates_by_addresses.values()
    ]


def _bind_per_core_tensor_runtime_addresses(
    ...
    for argument_index, tensor_index in enumerate(spec.tensor_indices):
        ...
        if coordinate is None:
            raise ValueError(
                "per-core tensor address binding requires an executing core"
            )
        common_runtime_args[argument_index] = tensor_base(
            ttnn,
            tensor,
            coordinate,
            mesh_coordinate,
    ...
    for kernel_spec_index, spec in enumerate(kernel_specs):
        ...
        common_runtime_args = [
            (
                0
                if tensors[tensor_index].is_per_core_allocated()
                else tensors[tensor_index].buffer_address()
            )
            for tensor_index in spec.tensor_indices
        ]
        ...
        for descriptor_variant in descriptor_variants:
            ...
            for partition_ranges, representative_coordinate in descriptor_partitions:
                for (
                    address_ranges,
                    address_coordinate,
                ) in _partition_core_ranges_by_per_core_tensor_addresses(
                    partition_ranges,
                    spec,
                    tensors,
                    sram_mesh_coordinate,
                ):
                    address_partitions.append(
                        (
                            address_ranges,
                            address_coordinate or representative_coordinate,
                        )
                    )
            descriptor_partitions = address_partitions
            for partition_ranges, representative_coordinate in descriptor_partitions:
                ...
                _bind_per_core_tensor_runtime_addresses(
                    partition_common_runtime_args,
                    spec,
                    tensors,
...
def _print_sram_runtime_report(
    arena, core_ranges, requested_bytes, operation_name, *, allocation_domain=False
):
    """Report the arena's reservation from TTNN's reference-device allocator."""
    independent = allocation_domain and arena.is_per_core_allocated()
    if independent:
        if arena.buffer_num_pages() != core_ranges.num_cores():
            raise RuntimeError("SRAM arena reporting requires one buffer page per core")
        reserved_bytes = int(arena.buffer_aligned_page_size())
    else:
        address = int(arena.buffer_address())
        buffers = [
            buffer
            for buffer in ttnn._ttnn.reports.get_buffers(arena.device())
            if buffer.buffer_type == ttnn.BufferType.L1
            and int(buffer.address) == address
        ]
        if len(buffers) != 1:
            raise RuntimeError(
                "SRAM report requires one reference-device arena allocation"
            )
    ...
    if allocation_domain:
        report["accounting_source"] = (
            "tensor-buffer-geometry" if independent else "buffer-report"
        )
...
def _run_kernel_on_device_impl(
    ...
    if sram_core_sizes:
        ...
        for spec in kernel_specs:
            kernel_ranges = (
                core_ranges if spec.core_ranges is None else spec.core_ranges
            )
            kernel_nodes = {
                (int(core.x), int(core.y))
                for core in ttnn.corerange_to_cores(kernel_ranges, row_wise=True)
    ...
    if device_domain is not None:
        ...
        program = build_device_mesh_program_descriptor(
            {
                coordinate: build_device_program(mesh_coordinate=coordinate)
                for coordinate in mesh_coordinates
            }
        )
    else:
...
def emit_runner_source(
    ...
    lines.append(
        f"KERNEL_SRAM_RECEIVER_TARGETS = {[spec.sram_receiver_targets for spec in kernel_specs]!r}"
    )
