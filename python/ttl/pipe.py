...
class PipeNet:
    ...
    multicast). The TTKernel lowering allocates one semaphore group per
    PipeNet, and the unicast and multicast handshakes use the group with
    incompatible semantics; mixing them in one PipeNet races when the same
    node participates in both. Use separate PipeNets.
