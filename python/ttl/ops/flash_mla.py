...
def make_flash_shard_core(B, PNHt, DHt, vDHt, Sk_chunk_t, N_CHUNKS, scale=1.0):
    ...
    @ttl.atom()
    def flash_shard_core(
        q_in: ttl.DFB, k, v, o_out: ttl.DFB, m_out: ttl.DFB, l_out: ttl.DFB
    ):
        ...
        m0 = m_out.reserve()
        m0.store(ttl.block.fill(-1e30, shape=m0.shape))
        l0 = l_out.reserve()
        l0.store(ttl.block.fill(0.0, shape=l0.shape))
        o0 = o_out.reserve()
        o0.store(ttl.block.fill(0.0, shape=o0.shape))
        ...
        for _ in range(N_CHUNKS):
            ...
            ex_w.store(
                ttl.exp(
                    ttl.sub(
                        sv2,
                        ttl.block.broadcast(mn_for_state, dims=[1], shape=sv2.shape),
                    )
                )
            )
            m_next = m_out.reserve()
            m_next.store(mn_for_state)
            ...
            alpha_re = alpha_cb.reserve()
            alpha_re.store(alpha)

            ex2 = ex_cb.wait()
            v_blk = ttl.math.typecast(v_cb.wait(), torch.bfloat16)
            pv_w = pv_cb.reserve()
            pv_w.store(ex2 @ v_blk)
            ...
            o_next.store(
                ttl.add(
                    ttl.mul(
                        ttl.block.broadcast(alpha2, dims=[1], shape=o_old.shape), o_old
                    ),
                    pv_blk,
                )
            )
...
def make_flash_normalize(grid, PNHt, vDHt):
    ...
    def flash_normalize(o_in, l_in, o_out):
        ...
        od = o_b.reserve()
        ttl.copy(o_in[base : base + PNHt, 0:vDHt], od)
        ld = l_b.reserve()
        ttl.copy(l_in[base : base + PNHt, 0:1], ld)
...
def make_flash_tree_reduce_core(PNHt, vDHt, B=1):
    ...
    def flash_tree_reduce_core(
        ...
        s0_m = ttl.PipeNet(
            [
                ttl.Pipe(src=(2 * i + 1, b), dst=(2 * i, b))
                for b in range(B)
                for i in range(4)
            ]
        )
        ...
        s1_m = ttl.PipeNet(
            [
                ttl.Pipe(src=(4 * i + 2, b), dst=(4 * i, b))
                for b in range(B)
                for i in range(2)
            ]
        )
...
def make_flash_tree_reduce(PNHt, vDHt, B=1):
    ...
    def flash_tree_reduce(in_o, in_m, in_l, out_o, out_l):
        ...
        od = o_b.reserve()
        ttl.copy(in_o[base : base + PNHt, 0:vDHt], od)
        md = m_b.reserve()
        ttl.copy(in_m[base : base + PNHt, 0:1], md)
        ld = l_b.reserve()
        ttl.copy(in_l[base : base + PNHt, 0:1], ld)
...
def make_flash_mla(n_cols, B, PNHt, DHt, vDHt, Sk_chunk_t, N_CHUNKS, scale=1.0):
    ...
    def reduce_norm(o_in: ttl.DFB, m_in: ttl.DFB, l_in: ttl.DFB, norm_out):
        ...
        nout = ttl.make_dataflow_buffer_like(
            norm_out, shape=(PNHt, vDHt), block_count=1
        )
