        ...
        *,
        ...
        with self._lock:
            ...
            if (
                ...
                or any(
                    len(node) != 2
                    or any(type(value) is not int or value < 0 for value in node)
                    for node in nodes
                )
            ):
                raise ValueError("nodes must be distinct nonnegative coordinate pairs")
            ...
            if addressing not in ("uniform", "per-node"):
                raise ValueError("addressing must be uniform or per-node")
            ...
            if any(node[0] >= grid.x or node[1] >= grid.y for node in nodes):
                raise ValueError("persistent tensor node is outside the device grid")
            core_ranges = api.CoreRangeSet(
                [
                    api.CoreRange(api.CoreCoord(*node), api.CoreCoord(*node))
                    for node in nodes
                ]
            )
            rectangle = core_ranges.bounding_box()
            extent = rectangle.grid_size()
            complete_rectangle = extent.x * extent.y == len(nodes)
            ...
            if (
                ...
                raise ValueError("block-sharded nodes must form a complete rectangle")
            spec = api.TensorSpec(
            ...
            def build_resources(retain):
                ...
                for declaration in self._declarations:
                    ...
                    config.experimental_set_per_core_allocation(
                        declaration.addressing == "per-node"
                    )
