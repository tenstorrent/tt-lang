...
// Passing a unit `accumulate` attribute makes the store an L1
// read-modify-write into the same slot instead of an overwrite.
...
// Callers use the returned attach's result wherever a tensor view of the
// slot is needed.
AttachCBOp createDFBWaitAndAttach(Value dfb, RankedTensorType tensorType,
                                  Location loc, OpBuilder &builder);

// Routes `intermediate` through a fresh compiler-allocated DFB and
// returns a tensor SSA value backed by it. The new `bind_cb` is placed at
// function entry (see `createCompilerAllocatedDFB`); the store, wait, and
// attach are emitted at `intermediate.getDefiningOp()`.
