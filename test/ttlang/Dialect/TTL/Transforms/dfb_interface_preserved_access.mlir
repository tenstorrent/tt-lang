...
// CHECK-LABEL: func.func @ordered_interface_preserved_access
...
// CHECK: ttl.opaque_call "inspect" dfb_dependencies(%[[DESCRIPTOR]] : !ttl.cb<{{.*}}>) dfb_accesses [#ttl.dfb_non_transactional_access<interface_preserved, 0>]

// REPORT: DFB allocation group #ttl.dfb_allocation_group<0> members=[0, 1]
// REPORT: DFB logical_id=0 bounded=1
// REPORT: access 0 effect=none non_transactional=interface_preserved
// REPORT: node (0,0) quiescence=none domain_assumption=exact conditional_execution=0 interface_state_preserved=1
// REPORT-NOT: DFB conflict lhs=0 rhs=1

module attributes {ttl.launch_grid = [1, 1]} {
  func.func @ordered_interface_preserved_access()
    ...
    ttl.opaque_call "inspect"
        ...
        dfb_accesses [#ttl.dfb_non_transactional_access<interface_preserved, 0>]
        () {header = "inspect.hpp"} : () -> ()
