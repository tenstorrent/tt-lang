...
// RUN: ttlang-opt %s --pass-pipeline='builtin.module(func.func(convert-ttl-to-compute,ttl-lower-binary-tiles,ttl-set-compute-kernel-config, ttl-assign-dst,ttl-subblock-compute-for-dst),canonicalize,cse)' | FileCheck %s --check-prefix=SUBBLOCK
...
// SUBBLOCK:            ttl.copy_tile %{{.*}}[%[[I_DIM0]], %[[I_DIM1]]] into dst[%[[C0]]]
// SUBBLOCK:            ttl.tile_relu %{{.*}} into dst[%[[C0]]]
// SUBBLOCK:            ttl.tile_store %{{.*}}, %{{.*}}[%[[I_DIM0]], %[[I_DIM1]]] from dst[%[[C0]]]
