...
// RUN: ttlang-opt %s --split-input-file --verify-diagnostics -pass-pipeline='builtin.module(ttl-validate-cb-budget{l1-budget-override=2079})'

module attributes {ttl.target_arch = #ttcore.arch<blackhole>} {
  func.func @reset_overflow() {
    // expected-error @below {{total DFB and synchronized-reset allocation (2080 bytes) exceeds L1 budget (2079 bytes)}}
