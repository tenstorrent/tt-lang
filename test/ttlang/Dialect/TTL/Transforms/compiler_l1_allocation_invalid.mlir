...
// RUN: ttlang-opt %s --split-input-file --verify-diagnostics -pass-pipeline='builtin.module(ttl-finalize-dfb-indices{memory-model=compiler-sram l1-budget-override=1024})'
...
module attributes {ttl.launch_grid = [1, 1]} {
  func.func @unsupported_tensor_backing() attributes {ttl.kernel_thread = #ttkernel.thread<noc>, ttl.logical_kernel = #ttl.logical_kernel<kind = data_movement>, ttl.noc_index = 0 : i32} {
    // expected-error @below {{compiler-sram requires independently owned storage without tensor backing or allocation groups}}
