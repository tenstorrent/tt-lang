...
// Purpose: Reduction dimension exceeds DST capacity -- no subblocking possible.
// Shape: 2x10 iteration domain (parallel=2, reduction=10).
// totalTiles=20, unroll_factor=8.
// reductionProduct=10 > unrollFactor=8 -> skip subblocking entirely.

#map_in3 = affine_map<(d0, d1) -> (d0, d1)>
#map_out3 = affine_map<(d0, d1) -> (d0)>

// SUBBLOCK-LABEL: func.func @reduction_exceeds_dst_budget
// No subblocking -- reductionProduct (10) exceeds unrollFactor (8).
// SUBBLOCK-NOT:   scf.for
// Compute op left unchanged (no full_linearization_strides, unroll_factor kept).
// SUBBLOCK:       ttl.compute
// SUBBLOCK-SAME:  ins({{.*}} : tensor<2x10x!ttcore.tile<32x32, f32>>)
// SUBBLOCK-SAME:  outs({{.*}} : tensor<2x!ttcore.tile<32x32, f32>>)
// SUBBLOCK-SAME:  iterator_types = ["parallel", "reduction"]
...
func.func @reduction_exceeds_dst_budget(
  ...
  %cb0 = ttl.bind_cb {cb_index = 0, buffer_factor = 2} : !ttl.cb<[2, 10], !ttcore.tile<32x32, f32>, 2>
  %cb1 = ttl.bind_cb {cb_index = 16, buffer_factor = 2} : !ttl.cb<[2], !ttcore.tile<32x32, f32>, 2>
  ...
  %result = ttl.compute
      ...
      outs(%init_cb : tensor<2x!ttcore.tile<32x32, f32>>)
      {indexing_maps = [#map_in3, #map_out3],
       iterator_types = ["parallel", "reduction"],
       ttl.unroll_factor = 8 : i64} {
  ^bb0(%in: !ttcore.tile<32x32, f32>, %acc: !ttcore.tile<32x32, f32>):
    %add = ttl.tile_add %in, %acc : !ttcore.tile<32x32, f32>
