// Summary: A loop body contains two computes that both store acc=true to
// the same view. Both computes get the same accumulation group ID.
// Pattern: for K: o.store(a, acc=True); o.store(b, acc=True) -> K*(a+b)
//
// RUN: ttlang-opt %s -ttl-form-accumulation-groups | FileCheck %s

#map = affine_map<(d0, d1) -> (d0, d1)>

// CHECK-LABEL: func.func @multi_store_per_iter
// CHECK: scf.for
// First compute in loop body.
// CHECK: ttl.compute
// CHECK-SAME: ttl.acc_group = 0
// CHECK: ttl.tile_store
// CHECK-SAME: acc = true
// Second compute in loop body.
// CHECK: ttl.compute
// CHECK-SAME: ttl.acc_group = 0
// CHECK: ttl.tile_store
// CHECK-SAME: acc = true

func.func @multi_store_per_iter(%a: tensor<1x1x!ttcore.tile<32x32, f32>>,
  ...
  %r = scf.for %i = %c0 to %c3 step %c1 iter_args(%acc = %init) -> tensor<1x1x!ttcore.tile<32x32, f32>> {
    // First store in loop body.
    %r1 = ttl.compute
