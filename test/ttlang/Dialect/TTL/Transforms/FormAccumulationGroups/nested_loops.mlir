// Summary: A compute with acc=true nested inside two user loops forms
// an accumulation group (nested loop iterations accumulate).
//
...
func.func @nested_loops(%a: tensor<1x1x!ttcore.tile<32x32, f32>>)
  ...
  %r = scf.for %i = %c0 to %c3 step %c1 iter_args(%acc0 = %init) -> tensor<1x1x!ttcore.tile<32x32, f32>> {
    %r1 = scf.for %j = %c0 to %c2 step %c1 iter_args(%acc1 = %acc0) -> tensor<1x1x!ttcore.tile<32x32, f32>> {
      %r2 = ttl.compute
