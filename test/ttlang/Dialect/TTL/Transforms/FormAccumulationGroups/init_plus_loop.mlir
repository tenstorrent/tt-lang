// Summary: Init compute (non-acc store) + loop with acc compute form a
// single accumulation group. The init store is converted to acc=true.
//
// RUN: ttlang-opt %s -ttl-form-accumulation-groups | FileCheck %s

#map = affine_map<(d0, d1) -> (d0, d1)>

// CHECK-LABEL: func.func @init_plus_loop
// The init compute gets acc_group=0.
// CHECK: ttl.compute
// CHECK-SAME: ttl.acc_group = 0
// CHECK: ttl.tile_store
// CHECK-SAME: acc = true
// The loop compute also gets acc_group=0.
// CHECK: scf.for
// CHECK: ttl.compute
// CHECK-SAME: ttl.acc_group = 0
// CHECK: ttl.tile_store
...
func.func @init_plus_loop(%a: tensor<1x1x!ttcore.tile<32x32, f32>>,
  ...
  // Init store (acc=false). Should be converted to acc=true.
  %r0 = ttl.compute
