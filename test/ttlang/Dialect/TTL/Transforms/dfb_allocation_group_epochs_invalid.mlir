...
module attributes {ttl.launch_grid = [1, 1]} {
  func.func @missing_reset_producer()
    ...
    ttl.opaque_call "reset_a0" dfb_reset <0, all_local = false, participants[<kind = compute, identity = "consumer", operation = "missing_reset">, <kind = data_movement, identity = "producer", operation = "missing_reset">]> (%a) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 3>) -> ()
...
module attributes {ttl.launch_grid = [1, 1]} {
  func.func @owner_noc0()
    ...
    ttl.opaque_call "reset_a0" dfb_reset <0, all_local = false, participants[<kind = data_movement, identity = "noc0", operation = "owner_handoff">, <kind = data_movement, identity = "noc1", operation = "owner_handoff">]> (%a) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 3>) -> ()
    ...
    ttl.opaque_call "order" dfb_reset <1, all_local = false, participants[<kind = data_movement, identity = "noc0", operation = "owner_handoff">, <kind = data_movement, identity = "noc1", operation = "owner_handoff">]> (%control) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>) -> ()
    return
  }
...
module attributes {ttl.launch_grid = [1, 1]} {
  func.func @epoch_envelope_crossing()
    ...
    ttl.opaque_call "reset_a0" dfb_reset <0, all_local = false, participants[<kind = compute>]> (%a) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> ()
