...
// Finalization records the exact tile type for a compiler-allocated subtile
// dataflow buffer.

// CHECK-LABEL: func.func @subtile_intermediate_metadata
// CHECK: ttl.bind_cb{cb_index = 3, block_count = 1} {ttl.compiler_allocated} : <[1, 1], !ttcore.tile<16x32, bf16>, 1>
// FINALIZE: module attributes {ttl.dfb_allocations = {{.*}}element_type = !ttcore.tile<16x32, bf16>{{.*}}page_size = 1024 : i32{{.*}}}
// FINALIZE-LABEL: func.func @subtile_intermediate_metadata
func.func @subtile_intermediate_metadata()
    attributes {ttl.kernel_thread = #ttkernel.thread<compute>,
                ttl.base_cta_index = 3 : i32, ttl.crta_indices = []} {
  %cb0 = ttl.bind_cb {cb_index = 0, block_count = 2} {dfb_id = 0 : index} : !ttl.cb<[1, 1], !ttcore.tile<16x32, bf16>, 2>
  %cb1 = ttl.bind_cb {cb_index = 1, block_count = 2} {dfb_id = 1 : index} : !ttl.cb<[1, 1], !ttcore.tile<16x32, bf16>, 2>
  %cb2 = ttl.bind_cb {cb_index = 2, block_count = 2} {dfb_id = 2 : index} : !ttl.cb<[1, 1], !ttcore.tile<16x32, bf16>, 2>
  %a_wait = ttl.cb_wait %cb0 : <[1, 1], !ttcore.tile<16x32, bf16>, 2> -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  %a = ttl.attach_cb %a_wait, %cb0 : (tensor<1x1x!ttcore.tile<16x32, bf16>>, !ttl.cb<[1, 1], !ttcore.tile<16x32, bf16>, 2>) -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  %b_wait = ttl.cb_wait %cb1 : <[1, 1], !ttcore.tile<16x32, bf16>, 2> -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  %b = ttl.attach_cb %b_wait, %cb1 : (tensor<1x1x!ttcore.tile<16x32, bf16>>, !ttl.cb<[1, 1], !ttcore.tile<16x32, bf16>, 2>) -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  %s_wait = ttl.cb_wait %cb2 : <[1, 1], !ttcore.tile<16x32, bf16>, 2> -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  %s = ttl.attach_cb %s_wait, %cb2 : (tensor<1x1x!ttcore.tile<16x32, bf16>>, !ttl.cb<[1, 1], !ttcore.tile<16x32, bf16>, 2>) -> tensor<1x1x!ttcore.tile<16x32, bf16>>

  %add = ttl.add %a, %b : tensor<1x1x!ttcore.tile<16x32, bf16>>, tensor<1x1x!ttcore.tile<16x32, bf16>> -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  %reduced = ttl.reduce %add, %s 0 : i32 [1] : (tensor<1x1x!ttcore.tile<16x32, bf16>>, tensor<1x1x!ttcore.tile<16x32, bf16>>) -> tensor<1x1x!ttcore.tile<16x32, bf16>>
  return
}
...
// MulUnaryConstOp narrowing: input from a fusible producer (ttl.add) must
// NOT be materialized to a DFB. Otherwise a single fused compute would be
// split into two unnecessary compute regions.

// CHECK-LABEL: func.func @mul_unary_const_skips_fusible_input
// CHECK-NOT: ttl.compiler_allocated
// CHECK: ttl.add
// CHECK-NEXT: ttl.mul_unary_const
// CHECK: return
func.func @mul_unary_const_skips_fusible_input()
  ...
  %add = ttl.add %a, %b : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<32x32, bf16>> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  %scaled = ttl.mul_unary_const %add, 5.000000e-01 : tensor<1x1x!ttcore.tile<32x32, bf16>> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  return
}

// -----

// MulOp narrowing: neither operand comes from Reduce/Matmul, so the mul
// must NOT trigger materialization for either side.

// CHECK-LABEL: func.func @mul_skips_two_fusible_inputs
// CHECK-NOT: ttl.compiler_allocated
// CHECK: ttl.add
// CHECK: ttl.sub
// CHECK: ttl.mul
...
func.func @mul_skips_two_fusible_inputs()
  ...
  %add = ttl.add %a, %b : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<32x32, bf16>> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  %sub = ttl.sub %a, %b : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<32x32, bf16>> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  %prod = ttl.mul %add, %sub : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<32x32, bf16>> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  return
}

// -----

// MulOp narrowing: exactly the Reduce operand is materialized. The add
// operand must NOT be materialized even though it shares the consumer.

// CHECK-LABEL: func.func @mul_materializes_only_reduce_side
// CHECK-COUNT-1: ttl.compiler_allocated
// CHECK-NOT: ttl.compiler_allocated
// CHECK: ttl.add
// CHECK: ttl.reduce
...
func.func @mul_materializes_only_reduce_side()
  ...
  %add = ttl.add %a, %b : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<32x32, bf16>> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  %r = ttl.reduce %a, %s 0 : i32 [1] : (tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<32x32, bf16>>) -> tensor<1x1x!ttcore.tile<32x32, bf16>>
