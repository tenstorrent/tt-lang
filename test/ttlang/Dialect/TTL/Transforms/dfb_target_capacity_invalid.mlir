// Tests pass-level rejection for supported 32-index and missing targets.
// The allocation oracle tests Quasar's limit without invoking unsupported
// Quasar kernel configuration.
// RUN: ttlang-opt %s --split-input-file --verify-diagnostics -pass-pipeline='builtin.module(ttl-finalize-dfb-indices)'

// expected-error @below {{DFB allocation needs 33 unspilled physical indices but exceeds the 32-DFB-index Wormhole B0 target capacity}}
...
// expected-error @below {{DFB allocation needs 33 unspilled physical indices but exceeds the conservative 32-DFB-index capacity used when target metadata is absent}}
module {
