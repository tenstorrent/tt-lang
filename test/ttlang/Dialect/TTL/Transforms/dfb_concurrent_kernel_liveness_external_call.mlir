...
// A typed inspection keeps A live through the external call without changing
// its queue state. The acknowledgment orders B after A, so they share physical
// index 0. The index query after the pop does not extend A's storage lifetime.
