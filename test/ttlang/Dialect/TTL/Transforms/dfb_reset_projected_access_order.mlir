...
module attributes {ttl.launch_grid = [1, 1]} {
  func.func @nested_reset_target_access()
    ...
    scf.for %outer = %zero to %one step %one {
      ...
      ttl.opaque_call "reset_first_epoch_0" dfb_reset <0, all_local = true, participants[<kind = compute>]> () {header = "reset.hpp"} : () -> ()
      ...
      ttl.opaque_call "reset_second_epoch_0" dfb_reset <1, all_local = true, participants[<kind = compute>]> () {header = "reset.hpp"} : () -> ()
      ...
      ttl.opaque_call "reset_first_epoch_1" dfb_reset <2, all_local = true, participants[<kind = compute>]> () {header = "reset.hpp"} : () -> ()
