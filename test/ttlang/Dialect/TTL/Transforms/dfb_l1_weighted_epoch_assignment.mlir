// Verifies that weighted allocation can choose a same-index-count assignment
// whose per-index capacity maxima fit the L1 budget.
// RUN: ttlang-opt %s -pass-pipeline='builtin.module(ttl-finalize-dfb-indices{reuse-user-dfbs=true l1-budget-override=300000})' | FileCheck %s
...
// CHECK: ttl.bind_cb{cb_index = 0, block_count = 1} {dfb_id = 0 : index}
// CHECK: ttl.bind_cb{cb_index = 1, block_count = 1} {dfb_id = 1 : index}
// CHECK: ttl.bind_cb{cb_index = 1, block_count = 1} {dfb_id = 2 : index}
// CHECK: ttl.bind_cb{cb_index = 0, block_count = 1} {dfb_id = 3 : index}
