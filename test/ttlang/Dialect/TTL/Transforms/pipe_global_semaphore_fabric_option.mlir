...
// The default allocates both intra-device counters from local semaphore ids.
// The fabric transfer needs only a global completion counter because its
// readiness is statically satisfied by its single execution.
...
// LOCAL: %[[LOCAL_REMOTE_DATA:.*]] = ttkernel.get_noc_addr
// LOCAL: %[[LOCAL_REMOTE_DONE:.*]] = ttkernel.get_noc_addr({{.*}}, {{.*}}, %[[LOCAL_FABRIC_DONE]], {{.*}})
// LOCAL-NOT: ttkernel.experimental.semaphore_wait_min
// LOCAL: ttkernel.routing_plane.striped_fused_write_atomic_inc({{.*}}, %[[LOCAL_REMOTE_DATA]], %[[LOCAL_REMOTE_DONE]])
...
// LOCAL-NOT: ttkernel.routing_plane.atomic_inc
...
// GLOBAL: %[[GLOBAL_REMOTE_DATA:.*]] = ttkernel.get_noc_addr
// GLOBAL: %[[GLOBAL_REMOTE_DONE:.*]] = ttkernel.get_noc_addr({{.*}}, {{.*}}, %[[GLOBAL_FABRIC_DONE]], {{.*}})
// GLOBAL-NOT: ttkernel.experimental.semaphore_wait_min
// GLOBAL: ttkernel.routing_plane.striped_fused_write_atomic_inc({{.*}}, %[[GLOBAL_REMOTE_DATA]], %[[GLOBAL_REMOTE_DONE]])
...
// GLOBAL-NOT: ttkernel.routing_plane.atomic_inc
