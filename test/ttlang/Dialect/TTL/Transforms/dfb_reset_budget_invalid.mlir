...
// RUN: ttlang-opt %s --split-input-file --verify-diagnostics -pass-pipeline='builtin.module(ttl-finalize-dfb-indices{reuse-user-dfbs=true l1-budget-override=2079})'

// expected-error @below {{DFB and synchronized-reset allocation requires 2080 L1 bytes but the budget is 2079 (DFB=2048, reset scratch=32)}}
module attributes {
  ttl.launch_grid = [1, 1],
