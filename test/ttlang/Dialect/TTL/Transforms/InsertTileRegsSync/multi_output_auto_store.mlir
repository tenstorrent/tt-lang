// RUN: ttlang-opt %s --split-input-file -ttl-insert-tile-regs-sync | FileCheck %s
...
// Test: same tile value inserted into multiple outputs. The auto-store
// dedup must be keyed by (tile, cb) pair, not just tile, so that the same
// tile gets separate stores for each output CB.

#map = affine_map<(d0, d1) -> (d0 * 2 + d1)>

// CHECK-LABEL: func.func @same_tile_two_outputs
func.func @same_tile_two_outputs()
    attributes {ttl.kernel_thread = #ttkernel.thread<compute>} {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  ...
  // CHECK: ttl.init_sfpu
  // Two reserves: one per output CB, each producing a distinct view.
  // CHECK: %[[VIEW1:.*]] = ttl.cb_reserve %[[CB1:.*]] :
  // CHECK: %[[VIEW2:.*]] = ttl.cb_reserve %[[CB2:.*]] :
  // CHECK: scf.for
  %r:2 = scf.for %i = %c0 to %c2 step %c1 iter_args(%arg0 = %out0, %arg1 = %out1) -> (tensor<2x2x!ttcore.tile<32x32, f32>>, tensor<2x2x!ttcore.tile<32x32, f32>>) {
    // CHECK: scf.for
    %inner:2 = scf.for %j = %c0 to %c2 step %c1 iter_args(%arg2 = %arg0, %arg3 = %arg1) -> (tensor<2x2x!ttcore.tile<32x32, f32>>, tensor<2x2x!ttcore.tile<32x32, f32>>) {
      ...
      // Same tile (%exp) inserted into BOTH outputs.
      %ins0 = tensor.insert %exp into %arg2[%i, %j] : tensor<2x2x!ttcore.tile<32x32, f32>>
      %ins1 = tensor.insert %exp into %arg3[%i, %j] : tensor<2x2x!ttcore.tile<32x32, f32>>

      // Both stores use the SAME tile but DIFFERENT views (different output CBs).
      // CHECK: ttl.tile_regs_commit
      // CHECK-NEXT: ttl.tile_regs_wait
      // CHECK-NEXT: ttl.store %[[TILE:.*]], %[[VIEW1]]
      // CHECK-NEXT: ttl.store %[[TILE]], %[[VIEW2]]
  ...
  // Two pushes matching the reserved CBs (order may vary due to map iteration).
  // CHECK-DAG: ttl.cb_push %[[CB1]]
  // CHECK-DAG: ttl.cb_push %[[CB2]]
