...
// A row-prefix publication keeps the full-tile recurrence in one DST tile and
// converts only the final store to the compact destination representation.
// FORM-LABEL: func.func @row_prefix_tensor_recurrence_scope
// FORM: ttl.accumulation_scope outs(%[[OUT:.*]] : tensor<1x14x!ttcore.tile<1x32, bf16>>) inits(%[[INIT:.*]] : tensor<1x1x!ttcore.tile<32x32, bf16>>) {
// FORM:   scf.for
// FORM:   ttl.store %{{.*}}, %[[OUT]] {row_prefix}
// FORM:   ttl.yield

// LOWER-LABEL: func.func @row_prefix_tensor_recurrence_scope
// LOWER: ttl.dst_section
// LOWER: ttl.copy_tile
// LOWER: scf.for
// LOWER: ttl.tile_accumulate
// LOWER: ttl.tile_store %{{.*}}, %{{.*}}[%{{.*}}, %{{.*}}] from dst[%{{.*}}] {row_prefix}
// LOWER-NOT: ttl.accumulation_scope
func.func @row_prefix_tensor_recurrence_scope() {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c3 = arith.constant 3 : index
  %init_cb = ttl.bind_cb {cb_index = 0, block_count = 2} : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>
  %contrib_cb = ttl.bind_cb {cb_index = 1, block_count = 4} : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 4>
  %out_cb = ttl.bind_cb {cb_index = 16, block_count = 2} : !ttl.cb<[1, 14], !ttcore.tile<1x32, bf16>, 2>

  %init_wait = ttl.cb_wait %init_cb : <[1, 1], !ttcore.tile<32x32, bf16>, 2> -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  %init = ttl.attach_cb %init_wait, %init_cb : (tensor<1x1x!ttcore.tile<32x32, bf16>>, !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> tensor<1x1x!ttcore.tile<32x32, bf16>>
  %out = ttl.cb_reserve %out_cb : <[1, 14], !ttcore.tile<1x32, bf16>, 2> -> tensor<1x14x!ttcore.tile<1x32, bf16>>
  ...
  ttl.store %result, %out {row_prefix} : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x14x!ttcore.tile<1x32, bf16>>
  return
}
