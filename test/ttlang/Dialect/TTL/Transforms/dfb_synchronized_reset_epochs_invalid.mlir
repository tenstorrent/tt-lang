...
module {
  func.func @non_transactional_reset()
    ...
    ttl.opaque_call "reset" dfb_accesses [#ttl.dfb_non_transactional_access<inspect, 0>] dfb_reset <0, all_local = false, participants[<kind = data_movement>]> (%dfb) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>) -> ()
    return
  }
}
