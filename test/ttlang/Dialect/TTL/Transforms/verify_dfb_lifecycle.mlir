...
// An `inspect` contract has no protocol effect. Declared `dfb_effects` leave
// the kernel's counters unknown and contribute their pushes and pops to the
// cross-kernel totals.
...
// readiness threshold that one pushed block follows. The kernel's counters are
// unknown; the totals compare its pushes with the consumer's pops.
...
// published before each reset is not outstanding afterwards: the producer's
// counters are unknown and the totals are not compared.
