...
module attributes {ttl.launch_grid = [1, 1]} {
  func.func @reader()
    ...
    ttl.opaque_call "reset_a0" dfb_reset <0, all_local = false, participants[<kind = compute, identity = "compute", operation = "allocation_group_epochs">, <kind = data_movement, identity = "reader", operation = "allocation_group_epochs">, <kind = data_movement, identity = "writer", operation = "allocation_group_epochs">]> (%a) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 3>) -> ()
    ttl.opaque_call "reset_b0" dfb_reset <1, all_local = false, participants[<kind = compute, identity = "compute", operation = "allocation_group_epochs">, <kind = data_movement, identity = "reader", operation = "allocation_group_epochs">, <kind = data_movement, identity = "writer", operation = "allocation_group_epochs">]> (%b) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 3>) -> ()
    %a1 = ttl.cb_reserve %a : <[1, 1], !ttcore.tile<32x32, bf16>, 3>
        -> tensor<1x1x!ttcore.tile<32x32, bf16>>
    ttl.cb_push %a : <[1, 1], !ttcore.tile<32x32, bf16>, 3>
    ttl.opaque_call "reset_a1" dfb_reset <2, all_local = false, participants[<kind = compute, identity = "compute", operation = "allocation_group_epochs">, <kind = data_movement, identity = "reader", operation = "allocation_group_epochs">, <kind = data_movement, identity = "writer", operation = "allocation_group_epochs">]> (%a) {header = "reset.hpp"} : (!ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 3>) -> ()
    return
  }
