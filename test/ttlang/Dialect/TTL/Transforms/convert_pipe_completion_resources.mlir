// RUN: ttlang-opt %s --split-input-file --pass-pipeline='builtin.module(convert-ttl-to-ttkernel{pipe-computed-addresses=false})' | FileCheck %s
// RUN: ttlang-opt %s --split-input-file --pass-pipeline='builtin.module(convert-ttl-to-ttkernel{pipe-computed-addresses=true pipe-capacity-sync=true})' | FileCheck %s --check-prefix=CAPACITY

// Summary: Verifies shared local and GlobalSemaphore allocation for PipeNet
// completion, sender-ready, and capacity counters.

// Sixteen concurrently posted transfers consume all local semaphore ids and
// require eight sender-ready counters per source. GlobalSemaphore instances at
// the two source nodes reuse the same eight indices.
// CHECK-LABEL: module attributes
// CHECK-SAME: ttl.pipe_global_semaphore_count = 8 : i64
// CHECK-SAME: ttl.pipe_sram_scratch_bytes = 32 : i64
// CHECK-SAME: ttl.pipe_sync_semaphore_count = 16 : i64
// CHECK-LABEL: func.func @completion_limit_uses_global_ready_counters
// CHECK-DAG: %[[FIRST_READY:.*]] = arith.constant 1 : index
// CHECK-DAG: %[[LAST_READY:.*]] = arith.constant 8 : index
// CHECK: ttkernel.get_common_arg_val(%[[FIRST_READY]])
// CHECK: ttkernel.get_common_arg_val(%[[LAST_READY]])
// CHECK-COUNT-16: ttkernel.get_semaphore
// CHECK-NOT: ttkernel.get_semaphore
// CHECK: return
module attributes {ttl.launch_grid = array<i64: 3, 1>} {
  func.func @completion_limit_uses_global_ready_counters()
      attributes {"ttl.kernel_thread" = #ttkernel.thread<noc>} {
    %src = ttl.bind_cb {cb_index = 0, block_count = 1} {dfb_id = 0 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst0 = ttl.bind_cb {cb_index = 1, block_count = 1} {dfb_id = 1 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst1 = ttl.bind_cb {cb_index = 2, block_count = 1} {dfb_id = 2 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst2 = ttl.bind_cb {cb_index = 3, block_count = 1} {dfb_id = 3 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst3 = ttl.bind_cb {cb_index = 4, block_count = 1} {dfb_id = 4 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst4 = ttl.bind_cb {cb_index = 5, block_count = 1} {dfb_id = 5 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst5 = ttl.bind_cb {cb_index = 6, block_count = 1} {dfb_id = 6 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst6 = ttl.bind_cb {cb_index = 7, block_count = 1} {dfb_id = 7 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst7 = ttl.bind_cb {cb_index = 8, block_count = 1} {dfb_id = 8 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst8 = ttl.bind_cb {cb_index = 9, block_count = 1} {dfb_id = 9 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst9 = ttl.bind_cb {cb_index = 10, block_count = 1} {dfb_id = 10 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst10 = ttl.bind_cb {cb_index = 11, block_count = 1} {dfb_id = 11 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst11 = ttl.bind_cb {cb_index = 12, block_count = 1} {dfb_id = 12 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst12 = ttl.bind_cb {cb_index = 13, block_count = 1} {dfb_id = 13 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst13 = ttl.bind_cb {cb_index = 14, block_count = 1} {dfb_id = 14 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst14 = ttl.bind_cb {cb_index = 15, block_count = 1} {dfb_id = 15 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst15 = ttl.bind_cb {cb_index = 16, block_count = 1} {dfb_id = 16 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %p0 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 0
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0>
    %p1 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 1
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 1>
    %p2 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 2
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 2>
    %p3 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 3
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 3>
    %p4 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 4
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 4>
    %p5 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 5
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 5>
    %p6 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 6
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 6>
    %p7 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 7
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 7>
    %p8 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 8
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 8>
    %p9 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 9
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 9>
    %p10 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 10
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 10>
    %p11 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 11
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 11>
    %p12 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 12
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 12>
    %p13 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 13
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 13>
    %p14 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 14
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 14>
    %p15 = ttl.create_pipe src(1, 0) dst(2, 0) to(2, 0) net 15
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 15>
    ttl.if_dst %p0
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0> {
      %recv0 = ttl.cb_reserve %dst0
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post0 = ttl.copy %p0, %recv0
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv1 = ttl.cb_reserve %dst1
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post1 = ttl.copy %p1, %recv1
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 1>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv2 = ttl.cb_reserve %dst2
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post2 = ttl.copy %p2, %recv2
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 2>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv3 = ttl.cb_reserve %dst3
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post3 = ttl.copy %p3, %recv3
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 3>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv4 = ttl.cb_reserve %dst4
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post4 = ttl.copy %p4, %recv4
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 4>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv5 = ttl.cb_reserve %dst5
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post5 = ttl.copy %p5, %recv5
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 5>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv6 = ttl.cb_reserve %dst6
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post6 = ttl.copy %p6, %recv6
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 6>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv7 = ttl.cb_reserve %dst7
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post7 = ttl.copy %p7, %recv7
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 7>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv8 = ttl.cb_reserve %dst8
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post8 = ttl.copy %p8, %recv8
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 8>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv9 = ttl.cb_reserve %dst9
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post9 = ttl.copy %p9, %recv9
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 9>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv10 = ttl.cb_reserve %dst10
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post10 = ttl.copy %p10, %recv10
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 10>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv11 = ttl.cb_reserve %dst11
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post11 = ttl.copy %p11, %recv11
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 11>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv12 = ttl.cb_reserve %dst12
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post12 = ttl.copy %p12, %recv12
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 12>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv13 = ttl.cb_reserve %dst13
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post13 = ttl.copy %p13, %recv13
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 13>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv14 = ttl.cb_reserve %dst14
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post14 = ttl.copy %p14, %recv14
          : (!ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 14>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv15 = ttl.cb_reserve %dst15
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post15 = ttl.copy %p15, %recv15
          : (!ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 15>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
    }
    ttl.if_src %p0
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0> {
      %send0 = ttl.copy %src, %p0
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0>)
          -> !ttl.transfer_handle<write>
      %send2 = ttl.copy %src, %p2
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 2>)
          -> !ttl.transfer_handle<write>
      %send4 = ttl.copy %src, %p4
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 4>)
          -> !ttl.transfer_handle<write>
      %send6 = ttl.copy %src, %p6
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 6>)
          -> !ttl.transfer_handle<write>
      %send8 = ttl.copy %src, %p8
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 8>)
          -> !ttl.transfer_handle<write>
      %send10 = ttl.copy %src, %p10
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 10>)
          -> !ttl.transfer_handle<write>
      %send12 = ttl.copy %src, %p12
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 12>)
          -> !ttl.transfer_handle<write>
      %send14 = ttl.copy %src, %p14
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 14>)
          -> !ttl.transfer_handle<write>
    }
    ttl.if_src %p1
        : !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 1> {
      %send1 = ttl.copy %src, %p1
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 1>)
          -> !ttl.transfer_handle<write>
      %send3 = ttl.copy %src, %p3
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 3>)
          -> !ttl.transfer_handle<write>
      %send5 = ttl.copy %src, %p5
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 5>)
          -> !ttl.transfer_handle<write>
      %send7 = ttl.copy %src, %p7
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 7>)
          -> !ttl.transfer_handle<write>
      %send9 = ttl.copy %src, %p9
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 9>)
          -> !ttl.transfer_handle<write>
      %send11 = ttl.copy %src, %p11
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 11>)
          -> !ttl.transfer_handle<write>
      %send13 = ttl.copy %src, %p13
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 13>)
          -> !ttl.transfer_handle<write>
      %send15 = ttl.copy %src, %p15
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(1, 0) dst(2, 0) to(2, 0) net 15>)
          -> !ttl.transfer_handle<write>
    }
    func.return
  }
}
// -----

// Seventeen transfers sharing a receiver use all sixteen local semaphore ids.
// One completion counter and seventeen concurrent sender-ready counters use
// GlobalSemaphore storage.
// CHECK-LABEL: module attributes
// CHECK-SAME: ttl.pipe_global_semaphore_count = 18 : i64
// CHECK-SAME: ttl.pipe_sram_scratch_bytes = 96 : i64
// CHECK-SAME: ttl.pipe_sync_semaphore_count = 16 : i64
// CHECK-LABEL: func.func @completion_overflow_uses_global_counter
// CHECK-DAG: %[[GLOBAL_COMPLETION_INDEX:.*]] = arith.constant 1 : index
// CHECK-DAG: %[[FIRST_READY_INDEX:.*]] = arith.constant 2 : index
// CHECK-DAG: %[[LAST_READY_INDEX:.*]] = arith.constant 18 : index
// CHECK: ttkernel.get_common_arg_val(%[[FIRST_READY_INDEX]])
// CHECK-COUNT-16: ttkernel.get_semaphore
// CHECK-NOT: ttkernel.get_semaphore
// CHECK: ttkernel.get_common_arg_val(%[[LAST_READY_INDEX]])
// CHECK-DAG: %[[GLOBAL_COMPLETION_WAIT:.*]] = ttkernel.get_common_arg_val(%[[GLOBAL_COMPLETION_INDEX]])
// CHECK-DAG: %[[GLOBAL_COMPLETION_PTR:.*]] = ttkernel.reinterpret_cast{{.*}}(%[[GLOBAL_COMPLETION_WAIT]])
// CHECK: ttkernel.experimental.semaphore_wait_min(%[[GLOBAL_COMPLETION_PTR]]
// CHECK: ttkernel.cb_pop_front
// CHECK-DAG: %[[LAST_READY_SEND:.*]] = ttkernel.get_common_arg_val(%[[LAST_READY_INDEX]])
// CHECK-DAG: %[[LAST_READY_PTR:.*]] = ttkernel.reinterpret_cast(%[[LAST_READY_SEND]])
// CHECK-DAG: %[[GLOBAL_COMPLETION_SEND:.*]] = ttkernel.get_common_arg_val(%[[GLOBAL_COMPLETION_INDEX]])
// CHECK-DAG: %[[GLOBAL_COMPLETION_NOC:.*]] = ttkernel.get_noc_addr({{.*}}, {{.*}}, %[[GLOBAL_COMPLETION_SEND]], {{.*}})
// CHECK: scf.if
// CHECK: ttkernel.experimental.semaphore_wait(%[[LAST_READY_PTR]]
// CHECK: ttkernel.noc_semaphore_set(%[[LAST_READY_PTR]]
// CHECK: ttkernel.noc_async_write
// CHECK: ttkernel.noc_async_write_barrier
// CHECK: ttkernel.noc_semaphore_inc(%[[GLOBAL_COMPLETION_NOC]]
// CHECK: return
// The computed-address configuration also proves capacity synchronization for
// p15. Its capacity counter shares the existing counter allocation.
// CAPACITY: module attributes {{.*}}ttl.pipe_global_semaphore_count = 18 : i64{{.*}}ttl.pipe_sync_semaphore_count = 16 : i64
// CAPACITY-LABEL: func.func @completion_overflow_uses_global_counter
// CAPACITY: %[[CAPACITY_ADDRESS:.*]] = ttkernel.get_common_arg_val(%[[CAPACITY_ARG_INDEX:.*]])
// CAPACITY: %[[CAPACITY_PTR:.*]] = ttkernel.reinterpret_cast{{.*}}(%[[CAPACITY_ADDRESS]])
// CAPACITY: ttkernel.noc_semaphore_set(%[[CAPACITY_PTR]]
// CAPACITY-DAG: %[[RELEASE_ADDRESS:.*]] = ttkernel.get_common_arg_val(%[[CAPACITY_ARG_INDEX]])
// CAPACITY-DAG: %[[RELEASE_NOC_ADDRESS:.*]] = ttkernel.get_noc_addr({{.*}}, {{.*}}, %[[RELEASE_ADDRESS]], {{.*}})
// CAPACITY: ttkernel.cb_pop_front
// CAPACITY: ttkernel.noc_semaphore_inc(%[[RELEASE_NOC_ADDRESS]]
// CAPACITY-DAG: %[[ACQUIRE_ADDRESS:.*]] = ttkernel.get_common_arg_val(%[[CAPACITY_ARG_INDEX]])
// CAPACITY-DAG: %[[ACQUIRE_PTR:.*]] = ttkernel.reinterpret_cast{{.*}}(%[[ACQUIRE_ADDRESS]])
// CAPACITY: ttkernel.experimental.semaphore_wait_min(%[[ACQUIRE_PTR]]
module attributes {ttl.launch_grid = array<i64: 17, 1>} {
  func.func @completion_overflow_uses_global_counter()
      attributes { "ttl.kernel_thread" = #ttkernel.thread<noc> } {
    %cb = ttl.bind_cb {cb_index = 0, block_count = 1} {dfb_id = 0 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %p = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 16
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 16>
    %dst0 = ttl.bind_cb {cb_index = 1, block_count = 1} {dfb_id = 1 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst1 = ttl.bind_cb {cb_index = 2, block_count = 1} {dfb_id = 2 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst2 = ttl.bind_cb {cb_index = 3, block_count = 1} {dfb_id = 3 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst3 = ttl.bind_cb {cb_index = 4, block_count = 1} {dfb_id = 4 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst4 = ttl.bind_cb {cb_index = 5, block_count = 1} {dfb_id = 5 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst5 = ttl.bind_cb {cb_index = 6, block_count = 1} {dfb_id = 6 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst6 = ttl.bind_cb {cb_index = 7, block_count = 1} {dfb_id = 7 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst7 = ttl.bind_cb {cb_index = 8, block_count = 1} {dfb_id = 8 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst8 = ttl.bind_cb {cb_index = 9, block_count = 1} {dfb_id = 9 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst9 = ttl.bind_cb {cb_index = 10, block_count = 1} {dfb_id = 10 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst10 = ttl.bind_cb {cb_index = 11, block_count = 1} {dfb_id = 11 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst11 = ttl.bind_cb {cb_index = 12, block_count = 1} {dfb_id = 12 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst12 = ttl.bind_cb {cb_index = 13, block_count = 1} {dfb_id = 13 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst13 = ttl.bind_cb {cb_index = 14, block_count = 1} {dfb_id = 14 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst14 = ttl.bind_cb {cb_index = 15, block_count = 1} {dfb_id = 15 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %dst15 = ttl.bind_cb {cb_index = 16, block_count = 1} {dfb_id = 16 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>
    %p0 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 0
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 0>
    %p1 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 1
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 1>
    %p2 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 2
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 2>
    %p3 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 3
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 3>
    %p4 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 4
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 4>
    %p5 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 5
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 5>
    %p6 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 6
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 6>
    %p7 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 7
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 7>
    %p8 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 8
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 8>
    %p9 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 9
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 9>
    %p10 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 10
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 10>
    %p11 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 11
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 11>
    %p12 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 12
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 12>
    %p13 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 13
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 13>
    %p14 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 14
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 14>
    %p15 = ttl.create_pipe src(0, 0) dst(16, 0) to(16, 0) net 15
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 15>
    %transfer15 = ttl.pipe_transfer.create %p15 {kind = #ttl.pipe_transfer_kind<point_to_point>}
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 15>
        -> !ttl.pipe_transfer
    ttl.if_dst %p
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 16> {
      %recv = ttl.cb_reserve %cb
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post = ttl.copy %p, %recv
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 16>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv0 = ttl.cb_reserve %dst0
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post0 = ttl.copy %p0, %recv0
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 0>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv1 = ttl.cb_reserve %dst1
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post1 = ttl.copy %p1, %recv1
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 1>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv2 = ttl.cb_reserve %dst2
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post2 = ttl.copy %p2, %recv2
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 2>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv3 = ttl.cb_reserve %dst3
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post3 = ttl.copy %p3, %recv3
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 3>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv4 = ttl.cb_reserve %dst4
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post4 = ttl.copy %p4, %recv4
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 4>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv5 = ttl.cb_reserve %dst5
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post5 = ttl.copy %p5, %recv5
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 5>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv6 = ttl.cb_reserve %dst6
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post6 = ttl.copy %p6, %recv6
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 6>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv7 = ttl.cb_reserve %dst7
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post7 = ttl.copy %p7, %recv7
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 7>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv8 = ttl.cb_reserve %dst8
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post8 = ttl.copy %p8, %recv8
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 8>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv9 = ttl.cb_reserve %dst9
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post9 = ttl.copy %p9, %recv9
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 9>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv10 = ttl.cb_reserve %dst10
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post10 = ttl.copy %p10, %recv10
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 10>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv11 = ttl.cb_reserve %dst11
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post11 = ttl.copy %p11, %recv11
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 11>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv12 = ttl.cb_reserve %dst12
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post12 = ttl.copy %p12, %recv12
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 12>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv13 = ttl.cb_reserve %dst13
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post13 = ttl.copy %p13, %recv13
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 13>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
      %recv14 = ttl.cb_reserve %dst14
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post14 = ttl.copy %p14, %recv14
          : (!ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 14>,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.receive_request
    }
    ttl.if_src %p
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 16> {
      %send = ttl.copy %cb, %p
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 16>)
          -> !ttl.transfer_handle<write>
      %send0 = ttl.copy %cb, %p0
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 0>)
          -> !ttl.transfer_handle<write>
      %send1 = ttl.copy %cb, %p1
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 1>)
          -> !ttl.transfer_handle<write>
      %send2 = ttl.copy %cb, %p2
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 2>)
          -> !ttl.transfer_handle<write>
      %send3 = ttl.copy %cb, %p3
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 3>)
          -> !ttl.transfer_handle<write>
      %send4 = ttl.copy %cb, %p4
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 4>)
          -> !ttl.transfer_handle<write>
      %send5 = ttl.copy %cb, %p5
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 5>)
          -> !ttl.transfer_handle<write>
      %send6 = ttl.copy %cb, %p6
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 6>)
          -> !ttl.transfer_handle<write>
      %send7 = ttl.copy %cb, %p7
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 7>)
          -> !ttl.transfer_handle<write>
      %send8 = ttl.copy %cb, %p8
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 8>)
          -> !ttl.transfer_handle<write>
      %send9 = ttl.copy %cb, %p9
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 9>)
          -> !ttl.transfer_handle<write>
      %send10 = ttl.copy %cb, %p10
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 10>)
          -> !ttl.transfer_handle<write>
      %send11 = ttl.copy %cb, %p11
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 11>)
          -> !ttl.transfer_handle<write>
      %send12 = ttl.copy %cb, %p12
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 12>)
          -> !ttl.transfer_handle<write>
      %send13 = ttl.copy %cb, %p13
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 13>)
          -> !ttl.transfer_handle<write>
      %send14 = ttl.copy %cb, %p14
          : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>,
             !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 14>)
          -> !ttl.transfer_handle<write>
    }
    ttl.if_dst %p15
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 15> {
      %recv15 = ttl.cb_reserve %dst15
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %post15 = ttl.pipe_transfer.post %transfer15, %recv15
          : (!ttl.pipe_transfer,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.pipe_token<net 15>
      ttl.pipe_transfer.wait %post15 : !ttl.pipe_token<net 15>
      ttl.cb_push %dst15 : <[1, 1], !ttcore.tile<32x32, f32>, 1>
      %ready15 = ttl.cb_wait %dst15
          : <[1, 1], !ttcore.tile<32x32, f32>, 1>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      ttl.cb_pop %dst15 : <[1, 1], !ttcore.tile<32x32, f32>, 1>
    }
    ttl.if_src %p15
        : !ttl.pipe<src(0, 0) dst(16, 0) to(16, 0) net 15> {
      %send15 = ttl.pipe_transfer.send %transfer15, %cb
          : (!ttl.pipe_transfer,
             !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 1>)
          -> !ttl.transfer_handle<write>
      ttl.wait %send15 : !ttl.transfer_handle<write>
    }
    func.return
  }
}

// -----

// Two capacity-2 transfers on different source cores reuse one counter
// allocation. A capacity-3 transfer uses a distinct counter because
// initialization is unconditional.
// CAPACITY-LABEL: module attributes
// CAPACITY-SAME: ttl.pipe_sync_semaphore_count = 3 : i64
// CAPACITY-NOT: ttl.pipe_global_semaphore_count
// CAPACITY-LABEL: func.func @capacity_counters_reuse_across_source_cores
// CAPACITY-COUNT-2: ttkernel.noc_semaphore_set
module attributes {ttl.launch_grid = array<i64: 4, 1>} {
  func.func @capacity_counters_reuse_across_source_cores()
      attributes {ttl.kernel_thread = #ttkernel.thread<noc>} {
    %src0 = ttl.bind_cb {cb_index = 0, block_count = 2} {dfb_id = 0 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
    %src1 = ttl.bind_cb {cb_index = 1, block_count = 2} {dfb_id = 1 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
    %src2 = ttl.bind_cb {cb_index = 4, block_count = 3} {dfb_id = 4 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 3>
    %dst0 = ttl.bind_cb {cb_index = 2, block_count = 2} {dfb_id = 2 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
    %dst1 = ttl.bind_cb {cb_index = 3, block_count = 2} {dfb_id = 3 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
    %dst2 = ttl.bind_cb {cb_index = 5, block_count = 3} {dfb_id = 5 : index}
        : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 3>
    %pipe0 = ttl.create_pipe src(0, 0) dst(2, 0) to(2, 0) net 0
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0>
    %pipe1 = ttl.create_pipe src(1, 0) dst(3, 0) to(3, 0) net 1
        : !ttl.pipe<src(1, 0) dst(3, 0) to(3, 0) net 1>
    %pipe2 = ttl.create_pipe src(2, 0) dst(0, 0) to(0, 0) net 2
        : !ttl.pipe<src(2, 0) dst(0, 0) to(0, 0) net 2>
    %transfer0 = ttl.pipe_transfer.create %pipe0 {
        kind = #ttl.pipe_transfer_kind<point_to_point>}
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0>
        -> !ttl.pipe_transfer
    %transfer1 = ttl.pipe_transfer.create %pipe1 {
        kind = #ttl.pipe_transfer_kind<point_to_point>}
        : !ttl.pipe<src(1, 0) dst(3, 0) to(3, 0) net 1>
        -> !ttl.pipe_transfer
    %transfer2 = ttl.pipe_transfer.create %pipe2 {
        kind = #ttl.pipe_transfer_kind<point_to_point>}
        : !ttl.pipe<src(2, 0) dst(0, 0) to(0, 0) net 2>
        -> !ttl.pipe_transfer
    ttl.if_dst %pipe0
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0> {
      %recv0 = ttl.cb_reserve %dst0
          : <[1, 1], !ttcore.tile<32x32, f32>, 2>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %token0 = ttl.pipe_transfer.post %transfer0, %recv0
          : (!ttl.pipe_transfer,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.pipe_token<net 0>
      ttl.pipe_transfer.wait %token0 : !ttl.pipe_token<net 0>
      ttl.cb_push %dst0 : <[1, 1], !ttcore.tile<32x32, f32>, 2>
      %ready0 = ttl.cb_wait %dst0
          : <[1, 1], !ttcore.tile<32x32, f32>, 2>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      ttl.cb_pop %dst0 : <[1, 1], !ttcore.tile<32x32, f32>, 2>
    }
    ttl.if_src %pipe0
        : !ttl.pipe<src(0, 0) dst(2, 0) to(2, 0) net 0> {
      %send0 = ttl.pipe_transfer.send %transfer0, %src0
          : (!ttl.pipe_transfer,
             !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>)
          -> !ttl.transfer_handle<write>
      ttl.wait %send0 : !ttl.transfer_handle<write>
    }
    ttl.if_dst %pipe1
        : !ttl.pipe<src(1, 0) dst(3, 0) to(3, 0) net 1> {
      %recv1 = ttl.cb_reserve %dst1
          : <[1, 1], !ttcore.tile<32x32, f32>, 2>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %token1 = ttl.pipe_transfer.post %transfer1, %recv1
          : (!ttl.pipe_transfer,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.pipe_token<net 1>
      ttl.pipe_transfer.wait %token1 : !ttl.pipe_token<net 1>
      ttl.cb_push %dst1 : <[1, 1], !ttcore.tile<32x32, f32>, 2>
      %ready1 = ttl.cb_wait %dst1
          : <[1, 1], !ttcore.tile<32x32, f32>, 2>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      ttl.cb_pop %dst1 : <[1, 1], !ttcore.tile<32x32, f32>, 2>
    }
    ttl.if_src %pipe1
        : !ttl.pipe<src(1, 0) dst(3, 0) to(3, 0) net 1> {
      %send1 = ttl.pipe_transfer.send %transfer1, %src1
          : (!ttl.pipe_transfer,
             !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>)
          -> !ttl.transfer_handle<write>
      ttl.wait %send1 : !ttl.transfer_handle<write>
    }
    ttl.if_dst %pipe2
        : !ttl.pipe<src(2, 0) dst(0, 0) to(0, 0) net 2> {
      %recv2 = ttl.cb_reserve %dst2
          : <[1, 1], !ttcore.tile<32x32, f32>, 3>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      %token2 = ttl.pipe_transfer.post %transfer2, %recv2
          : (!ttl.pipe_transfer,
             tensor<1x1x!ttcore.tile<32x32, f32>>)
          -> !ttl.pipe_token<net 2>
      ttl.pipe_transfer.wait %token2 : !ttl.pipe_token<net 2>
      ttl.cb_push %dst2 : <[1, 1], !ttcore.tile<32x32, f32>, 3>
      %ready2 = ttl.cb_wait %dst2
          : <[1, 1], !ttcore.tile<32x32, f32>, 3>
          -> tensor<1x1x!ttcore.tile<32x32, f32>>
      ttl.cb_pop %dst2 : <[1, 1], !ttcore.tile<32x32, f32>, 3>
    }
    ttl.if_src %pipe2
        : !ttl.pipe<src(2, 0) dst(0, 0) to(0, 0) net 2> {
      %send2 = ttl.pipe_transfer.send %transfer2, %src2
          : (!ttl.pipe_transfer,
             !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 3>)
          -> !ttl.transfer_handle<write>
      ttl.wait %send2 : !ttl.transfer_handle<write>
    }
    func.return
  }
}
