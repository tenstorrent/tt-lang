...
// the pass rejects invalid inputs. Error reporting uses
// notifyMatchFailure (no emitted diagnostic) so only the conversion
// framework's "failed to legalize" error is visible.
...
// getAttachedCB returns null and the pattern cannot match.
