...
func.func @idempotent_subblocked(%arg0: tensor<1x8x!ttcore.tile<32x32, f32>>) -> tensor<1x8x!ttcore.tile<32x32, f32>> {
  ...
  %result = ttl.compute ins(%arg_cb : tensor<1x8x!ttcore.tile<32x32, f32>>) outs(%out_cb : tensor<1x8x!ttcore.tile<32x32, f32>>) {indexing_maps = [#map, #map], iterator_types = ["parallel", "parallel"], ttl.full_linearization_strides = array<i64: 8, 1>, ttl.unroll_factor = 8 : i64} {
    ^bb0(%in: !ttcore.tile<32x32, f32>, %out: !ttcore.tile<32x32, f32>):
      %idx = ttl.linearized_index #map1 : index
      %c0 = arith.constant 0 : index
      %tok, %tile = ttl.copy_tile %in, %idx, %c0 {dst_idx = 0 : i32} : !ttcore.tile<32x32, f32>, index, index -> !ttl.dst, !ttcore.tile<32x32, f32>
