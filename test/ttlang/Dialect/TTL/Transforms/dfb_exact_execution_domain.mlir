...
// Per-node counts establish a strict subset. The two complete, sequential
// lifecycles therefore share one physical index without relying on equality of
// their separately evaluated runtime predicates.

// REUSE-LABEL: func.func @exact_subset_reuses
...
// REPORT: DFB logical_id=14 bounded=1 compiler_created=0
// REPORT-SAME: domain={(0,0)}
// REPORT: access 0 effect=reserve tiles=1 sequence=0 domain={(0,0)}
// REPORT: DFB logical_id=15 bounded=1 compiler_created=0
// REPORT-SAME: domain={(0,0)}
// REPORT: DFB assignment: logical DFB 14 -> physical index 0 (bounded)
// REPORT-NEXT: DFB assignment: logical DFB 15 -> physical index 0 (bounded)

module attributes {ttl.launch_grid = array<i64: 2, 1>} {
  func.func @exact_subset_reuses(%runtime_offset: index)
    ...
    %second_node = arith.cmpi eq, %core_x, %zero : index
