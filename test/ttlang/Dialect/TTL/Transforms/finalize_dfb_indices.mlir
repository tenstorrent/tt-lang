...
// The pass should update base_cta_index to 4 and emit ttl.scratch_dfbs.

// CHECK: module attributes {ttl.scratch_dfbs = [{block_count = 1 : i32, dfb_index = 3 : i32, element_type = !ttcore.tile<32x32, bf16>, num_tiles = 1 : i32}]}
...
func.func @compute()
  ...
  %scratch = ttl.bind_cb {cb_index = 3, block_count = 1} {ttl.compiler_allocated} : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>
  return
}
...
// No compiler-allocated DFBs: pass should not add ttl.scratch_dfbs,
// but should still update base_cta_index to the true DFB count (3).

// CHECK-NOT: ttl.scratch_dfbs
