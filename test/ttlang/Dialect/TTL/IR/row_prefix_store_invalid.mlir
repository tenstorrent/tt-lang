...
func.func @source_must_be_tiled(%source: tensor<32x32xbf16>) {
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<32x32xbf16>, tensor<1x1x!ttcore.tile<16x32, bf16>>
  func.return
}
...
func.func @destination_must_be_tiled(
  ...
  %cb = ttl.bind_cb {cb_index = 0, block_count = 1}
      : !ttl.cb<[1, 1], bf16, 1>
  %view = ttl.cb_reserve %cb
      : <[1, 1], bf16, 1> -> tensor<1x1xbf16>
  // expected-error @below {{'ttl.store' op row_prefix requires tiled source and destination tensors}}
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1xbf16>
  func.return
}
...
func.func @source_tile_must_be_full(
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<16x32, bf16>>, tensor<1x1x!ttcore.tile<16x32, bf16>>
  func.return
}
...
func.func @source_must_be_one_tile(
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<1x2x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<16x32, bf16>>
  func.return
}
...
func.func @data_types_must_match(
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<32x32, f32>>, tensor<1x1x!ttcore.tile<16x32, bf16>>
  func.return
}
...
func.func @unsupported_data_type(
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<32x32, bfp_bf8>>, tensor<1x1x!ttcore.tile<16x32, bfp_bf8>>
  func.return
}
...
func.func @view_must_be_reserved(
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<16x32, bf16>>
  func.return
}
...
func.func @destination_width_must_match(
  ...
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x1x!ttcore.tile<16x16, bf16>>
  func.return
}

// -----

// A row-prefix store writes exactly one destination tile.
func.func @destination_must_be_one_tile(
    %source: tensor<1x1x!ttcore.tile<32x32, bf16>>) {
  %cb = ttl.bind_cb {cb_index = 0, block_count = 1}
      : !ttl.cb<[1, 2], !ttcore.tile<16x32, bf16>, 1>
  %view = ttl.cb_reserve %cb
      : <[1, 2], !ttcore.tile<16x32, bf16>, 1>
      -> tensor<1x2x!ttcore.tile<16x32, bf16>>
  // expected-error @below {{'ttl.store' op row_prefix destination must contain exactly one tile, got 2}}
  ttl.store %source, %view {row_prefix}
      : tensor<1x1x!ttcore.tile<32x32, bf16>>, tensor<1x2x!ttcore.tile<16x32, bf16>>
  func.return
}
...
func.func @tile_store_unbacked_view(
    %tile: !ttcore.tile<32x32, bf16>,
    %view: tensor<1x1x!ttcore.tile<16x32, bf16>>) {
  %c0 = arith.constant 0 : index
  // expected-error @below {{'ttl.tile_store' op row_prefix requires a producer-reserved view}}
  ttl.tile_store %tile, %view[%c0, %c0] from dst[%c0] {row_prefix}
      : !ttcore.tile<32x32, bf16>, tensor<1x1x!ttcore.tile<16x32, bf16>>
