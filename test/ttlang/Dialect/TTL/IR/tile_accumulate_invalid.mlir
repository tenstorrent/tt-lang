...
// Summary: ttl.tile_accumulate rejects an unsupported combiner (parser), a
// contribution type that differs from the accumulator (verifier), and an
// accumulator/result type mismatch (AllTypesMatch).
...
func.func @contribution_type_mismatch(
    ...
    -> !ttcore.tile<32x32, bf16> {
  %c0 = arith.constant 0 : index
  // expected-error @+1 {{requires contribution and accumulator to have the same type for add}}
