...
func.func @access_dependency_out_of_range() attributes {ttl.kernel_thread = #ttkernel.thread<compute>} {
  ...
  ttl.opaque_call "foo" dfb_dependencies(%dfb : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>) dfb_accesses [#ttl.dfb_non_transactional_access<interface_preserved, 1>] () {header = "h.hpp"} : () -> ()
  return
}
...
func.func @duplicate_non_transactional_access() attributes {ttl.kernel_thread = #ttkernel.thread<compute>} {
  ...
  ttl.opaque_call "foo" dfb_dependencies(%dfb : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>) dfb_accesses [#ttl.dfb_non_transactional_access<interface_preserved, 0>, #ttl.dfb_non_transactional_access<interface_preserved, 0>] () {header = "h.hpp"} : () -> ()
  return
}
...
func.func @protocol_and_non_transactional_access() attributes {ttl.kernel_thread = #ttkernel.thread<compute>} {
  ...
  ttl.opaque_call "foo" dfb_dependencies(%dfb : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>) dfb_effects [#ttl.dfb_protocol_effect<wait, 0, 1>] dfb_accesses [#ttl.dfb_non_transactional_access<interface_preserved, 0>] () {header = "h.hpp"} : () -> ()
  return
}
