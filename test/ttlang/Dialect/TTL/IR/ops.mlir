...
// CHECK: %[[CB:.*]] = ttl.bind_cb{cb_index = 0, block_count = 2} : <[1, 1], !ttcore.tile<32x32, f32>, 2>
func.func @bind_cb() {
...
// CHECK: %[[CB:.*]] = ttl.bind_cb{cb_index = 0, block_count = 2} : <[1, 1], !ttcore.tile<32x32, f32>, 2>
// CHECK: %[[XF:.*]] = ttl.copy %[[T]], %[[CB]] : (tensor<1x1x!ttcore.tile<32x32, f32>, #ttl.layout<{{.*}}>>, !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>) -> !ttl.transfer_handle<read>
...
func.func @copy_read_wait(%t: tensor<1x1x!ttcore.tile<32x32, f32>, #layout_interleaved>) attributes {ttl.kernel_thread = #ttkernel.thread<noc>} {
  %cb = ttl.bind_cb {cb_index = 0, block_count = 2} : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
  %xf = ttl.copy %t, %cb : (tensor<1x1x!ttcore.tile<32x32, f32>, #layout_interleaved>, !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>) -> !ttl.transfer_handle<read>
...
// CHECK-SAME: (%[[T:.*]]: tensor<1x1x!ttcore.tile<32x32, f32>, #ttl.layout<{{.*}}>>)
// CHECK: %[[CB:.*]] = ttl.bind_cb{cb_index = 0, block_count = 2} : <[1, 1], !ttcore.tile<32x32, f32>, 2>
// CHECK: %[[XF:.*]] = ttl.copy %[[CB]], %[[T]] : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>, tensor<1x1x!ttcore.tile<32x32, f32>, #ttl.layout<{{.*}}>>) -> !ttl.transfer_handle<write>
...
func.func @copy_write_wait(%t: tensor<1x1x!ttcore.tile<32x32, f32>, #layout_interleaved>) attributes {ttl.kernel_thread = #ttkernel.thread<noc>} {
  %cb = ttl.bind_cb {cb_index = 0, block_count = 2} : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
  %xf = ttl.copy %cb, %t : (!ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>, tensor<1x1x!ttcore.tile<32x32, f32>, #layout_interleaved>) -> !ttl.transfer_handle<write>
...
func.func @copy_read_wait_tile_layout(%t: tensor<1x1x!ttcore.tile<32x32, f32>, #layout_tile>) attributes {ttl.kernel_thread = #ttkernel.thread<noc>} {
  %cb = ttl.bind_cb {cb_index = 0, block_count = 2} : !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>
  %xf = ttl.copy %t, %cb : (tensor<1x1x!ttcore.tile<32x32, f32>, #layout_tile>, !ttl.cb<[1, 1], !ttcore.tile<32x32, f32>, 2>) -> !ttl.transfer_handle<read>
  ttl.wait %xf : !ttl.transfer_handle<read>
  func.return
}
