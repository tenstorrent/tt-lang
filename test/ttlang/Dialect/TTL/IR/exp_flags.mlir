...
// CHECK: ttl.exp %{{.*}} {{[{].*}}approx = true{{.*}}iterations = 4 : i32{{.*}}scale = 2.000000e+00 : f32{{.*[}]}} : tensor<2x2x!ttcore.tile<32x32, f32>> -> tensor<2x2x!ttcore.tile<32x32, f32>>
func.func @exp_with_flags(
    %arg0: tensor<2x2x!ttcore.tile<32x32, f32>>)
    -> tensor<2x2x!ttcore.tile<32x32, f32>> {
  %0 = ttl.exp %arg0 {approx = true, scale = 2.000000e+00 : f32,
                      input_clamping = #ttl.input_clamping<none>,
                      iterations = 4 : i32}
      : tensor<2x2x!ttcore.tile<32x32, f32>> -> tensor<2x2x!ttcore.tile<32x32, f32>>
  return %0 : tensor<2x2x!ttcore.tile<32x32, f32>>
}
...
// CHECK: ttl.tile_exp %{{.*}} into dst[%{{.*}}] {{[{].*}}approx = true{{.*}}iterations = 4 : i32{{.*}}scale = 2.000000e+00 : f32{{.*[}]}} : !ttcore.tile<32x32, f32> -> !ttcore.tile<32x32, f32>
