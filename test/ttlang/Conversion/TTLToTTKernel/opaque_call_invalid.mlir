...
module attributes {ttl.memory_model = "compiler-sram", ttl.l1_arena_bytes = 2112 : i64, ttl.dfb_allocations = [{block_count = 1 : i32, dfb_index = 0 : i32, element_type = !ttcore.tile<32x32, bf16>, l1_allocation_bytes = 2048 : i64, l1_offset = 0 : i64, l1_payload_offset = 64 : i64, num_tiles = 1 : i32, page_size = 2048 : i32, storage_index = 0 : i32}]} {
  func.func @compiler_l1_metal_index() attributes {ttl.kernel_thread = #ttkernel.thread<noc>} {
    ...
    // expected-error @below {{compiler-sram external calls cannot use Metal DFB indices; use ttl.dfb_descriptor()}}
...
module attributes {ttl.memory_model = "compiler-sram", ttl.l1_arena_bytes = 0 : i64, ttl.dfb_allocations = []} {
  func.func @compiler_l1_unknown_effects() attributes {ttl.kernel_thread = #ttkernel.thread<noc>} {
    // expected-error @below {{compiler-sram requires typed DFB effects for external calls}}
