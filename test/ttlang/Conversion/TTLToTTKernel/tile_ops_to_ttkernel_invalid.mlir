...
// Polymorphic tile_mul is lowered to tile_mul_sfpu inside convert-ttl-to-ttkernel
// before dialect conversion; diagnostics reference the concrete op.
...
func.func @tile_mul_lhs_missing_dst_idx(%idx: index) -> !ttcore.tile<32x32, f32> {
  ...
  // expected-error @+1 {{failed to legalize operation 'ttl.tile_mul_sfpu' that was explicitly marked illegal}}
...
// FPU binary tile op where operands cannot be traced to CBs.
// The operands are plain function arguments with no CB association. The FPU
// pattern fails (cannot find CBs), and no other pattern matches
// tile_add_fpu, so the op becomes illegal.
func.func @fpu_add_no_cb(%a: !ttcore.tile<32x32, bf16>, %b: !ttcore.tile<32x32, bf16>)
    -> !ttcore.tile<32x32, bf16>
    attributes {ttl.kernel_thread = #ttkernel.thread<compute>} {
  %c0 = arith.constant 0 : index
  // expected-error @+1 {{failed to legalize operation 'ttl.tile_add_fpu' that was explicitly marked illegal}}
  %sum = ttl.tile_add_fpu %a, %b into dst[%c0]  : !ttcore.tile<32x32, bf16>, !ttcore.tile<32x32, bf16> -> !ttcore.tile<32x32, bf16>
