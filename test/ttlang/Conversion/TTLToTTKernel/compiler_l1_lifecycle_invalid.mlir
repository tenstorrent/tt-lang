...
module attributes {ttl.compiler_sram_reconfiguration_resets = [{dfb_indices = array<i32: 1>, ordinal = 0 : i64}], ttl.dfb_allocations = [{block_count = 1 : i32, dfb_index = 0 : i32, element_type = !ttcore.tile<32x32, bf16>, l1_allocation_bytes = 2048 : i64, l1_offset = 0 : i64, l1_payload_offset = 64 : i64, num_tiles = 1 : i32, page_size = 2048 : i32, storage_index = 0 : i32}], ttl.l1_arena_bytes = 2112 : i64, ttl.launch_grid = [1, 1], ttl.memory_model = "compiler-sram", ttl.target_arch = #ttcore.arch<blackhole>} {
  func.func @compute() attributes {ttl.base_cta_index = 1 : i32, ttl.kernel_thread = #ttkernel.thread<compute>, ttl.logical_kernel = #compute} {
    %dfb = ttl.bind_cb {cb_index = 0, block_count = 1} {dfb_id = 0 : index} : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>
    ttl.dfb_reconfiguration #boundary
    return
...
#compute = #ttl.logical_kernel<kind = compute, identity = "compute", operation = "oversized_ordinal">
#reader = #ttl.logical_kernel<kind = data_movement, identity = "reader", operation = "oversized_ordinal">
#writer = #ttl.logical_kernel<kind = data_movement, identity = "writer", operation = "oversized_ordinal">
#boundary = #ttl.dfb_reconfiguration<0, participants[#compute, #reader, #writer]>

// Reset metadata must fit the boundary ordinal type before lookup.
// expected-error @below {{'builtin.module' op contains malformed compiler-sram reconfiguration reset metadata}}
module attributes {ttl.compiler_sram_reconfiguration_resets = [{dfb_indices = array<i32: 0>, ordinal = 18446744073709551616 : i128}], ttl.dfb_allocations = [{block_count = 1 : i32, dfb_index = 0 : i32, element_type = !ttcore.tile<32x32, bf16>, l1_allocation_bytes = 2048 : i64, l1_offset = 0 : i64, l1_payload_offset = 64 : i64, num_tiles = 1 : i32, page_size = 2048 : i32, storage_index = 0 : i32}], ttl.l1_arena_bytes = 2112 : i64, ttl.launch_grid = [1, 1], ttl.memory_model = "compiler-sram", ttl.target_arch = #ttcore.arch<blackhole>} {
  func.func @compute() attributes {ttl.base_cta_index = 1 : i32, ttl.kernel_thread = #ttkernel.thread<compute>, ttl.logical_kernel = #compute} {
    %dfb = ttl.bind_cb {cb_index = 0, block_count = 1} {dfb_id = 0 : index} : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 1>
    ttl.dfb_reconfiguration #boundary
    return
