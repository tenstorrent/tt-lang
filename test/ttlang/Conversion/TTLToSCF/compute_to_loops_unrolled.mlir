...
//
// LIMITATION: Current implementation unconditionally generates all unrolled iterations.
// When num_tiles % unroll_factor != 0, the final loop iteration accesses out-of-bounds
// indices (e.g., 5 tiles with unroll=2: when i=4, the second iteration accesses index 5).
// This works at runtime because device tensor ops don't bounds-check. Tests include
// remainder cases to document this behavior.
//
...
// Expected: scf.for step=2, two unrolled iterations at indices i and i+1.
...
func.func @unroll_1d_binary_even(%a: tensor<4x!ttcore.tile<32x32, f32>>,
  ...
  // Verify loop step equals unroll_factor=2
  // CHECK: scf.for %[[I:.*]] = %[[C0]] to %[[C4]] step %[[C2]] iter_args(%[[ACC:.*]] = %{{.*}}) -> (tensor<4x!ttcore.tile<32x32, f32>>)
  ...
  // CHECK: return %{{.*}} : tensor<4x!ttcore.tile<32x32, f32>>
...
// Expected: scf.for step=2 processes tiles 0,1 then 2,3 then 4 (final iteration only processes 1 tile).
// The loop bound is 5, so iterations are: i=0 (processes 0,1), i=2 (processes 2,3), i=4 (processes 4).
...
func.func @unroll_1d_binary_remainder(%a: tensor<5x!ttcore.tile<32x32, f32>>,
  ...
  // Verify loop step=2 with upper bound=5
  // CHECK: scf.for %[[I:.*]] = %[[C0]] to %[[C5]] step %[[C2]] iter_args(%[[ACC:.*]] = %{{.*}}) -> (tensor<5x!ttcore.tile<32x32, f32>>)
  ...
  // CHECK: %[[SUM_0:.*]] = ttl.tile_add %[[EXT_A_0]], %[[EXT_B_0]] : !ttcore.tile<32x32, f32>
  ...
  // CHECK: %[[SUM_1:.*]] = ttl.tile_add %[[EXT_A_1]], %[[EXT_B_1]] : !ttcore.tile<32x32, f32>
  // CHECK-NEXT: %[[INS_1:.*]] = tensor.insert %[[SUM_1]] into %[[INS_0]][%[[I_PLUS_1]]] : tensor<5x!ttcore.tile<32x32, f32>>

  // CHECK: scf.yield %[[INS_1]] : tensor<5x!ttcore.tile<32x32, f32>>
...
func.func @unroll_2d_binary(%a: tensor<2x2x!ttcore.tile<32x32, f32>>,
  ...
  // CHECK: scf.yield %[[INS_1]] : tensor<2x2x!ttcore.tile<32x32, f32>>
...
// Expected: scf.for step=4, four unrolled iterations in loop body.
...
func.func @unroll_1d_unary_high(%a: tensor<4x!ttcore.tile<32x32, f32>>)
  ...
  // Verify loop step=4 (processes all 4 tiles in single iteration)
  // CHECK: scf.for %[[I:.*]] = %[[C0]] to %[[C4]] step %[[C4]] iter_args(%[[ACC:.*]] = %{{.*}}) -> (tensor<4x!ttcore.tile<32x32, f32>>)

  // First iteration: i
  // CHECK-NEXT: %[[EXT_0:.*]] = tensor.extract %{{.*}}[%[[I]]] : tensor<4x!ttcore.tile<32x32, f32>>
  // CHECK: %[[EXP_0:.*]] = ttl.tile_exp %[[EXT_0]] : !ttcore.tile<32x32, f32>
  ...
  // CHECK: %[[EXP_1:.*]] = ttl.tile_exp %[[EXT_1]] : !ttcore.tile<32x32, f32>
  ...
  // CHECK: %[[EXP_2:.*]] = ttl.tile_exp %[[EXT_2]] : !ttcore.tile<32x32, f32>
  ...
  // CHECK: %[[EXP_3:.*]] = ttl.tile_exp %[[EXT_3]] : !ttcore.tile<32x32, f32>
  // CHECK-NEXT: %[[INS_3:.*]] = tensor.insert %[[EXP_3]] into %[[INS_2]][%[[I_PLUS_3]]] : tensor<4x!ttcore.tile<32x32, f32>>

  // CHECK: scf.yield %[[INS_3]] : tensor<4x!ttcore.tile<32x32, f32>>
...
func.func @unroll_with_store(%a: tensor<4x!ttcore.tile<32x32, f32>>,
  ...
  // CHECK: scf.yield %[[INS_1]] : tensor<4x!ttcore.tile<32x32, f32>>
