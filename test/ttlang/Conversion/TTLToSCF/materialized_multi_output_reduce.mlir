...
// RUN: ttlang-opt %s -pass-pipeline='builtin.module(func.func(ttl-form-producer-compute,ttl-insert-intermediate-dfbs,convert-ttl-to-compute,ttl-set-compute-kernel-config,ttl-assign-dst,ttl-lower-to-loops{dst-accumulation=true}))' | FileCheck %s
// RUN: ttlang-opt %s -pass-pipeline='builtin.module(func.func(ttl-form-producer-compute,ttl-insert-intermediate-dfbs,convert-ttl-to-compute,ttl-set-compute-kernel-config,ttl-assign-dst,ttl-lower-to-loops{dst-accumulation=true}))' | FileCheck %s --check-prefix=NO-COMPUTE
