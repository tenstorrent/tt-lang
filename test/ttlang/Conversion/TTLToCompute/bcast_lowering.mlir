...
// Scalar reduce → ROW bcast: override to SCALAR because REDUCE_SCALAR places
// its result at tile element [0,0], and ROW bcast would read garbage from the
// rest of row 0. The pass traces through the CB push/wait to detect this (#444).
// This IR mirrors the structure the frontend produces for:
//   r.store(ttl.math.reduce_sum(x, s, dims=[0, 1]))
//   o.store(ttl.math.broadcast(r, o, dims=[0]))
// CHECK-LABEL: func.func @bcast_row_after_scalar_reduce
func.func @bcast_row_after_scalar_reduce() {
  ...
  // Reduce to scalar, store to red_cb, push.
  ...
  // Wait on red_cb, then broadcast with ROW type (2) — should be overridden to SCALAR (3).
...
// Scalar reduce → COL bcast: same override to SCALAR (#444).
// CHECK-LABEL: func.func @bcast_col_after_scalar_reduce
func.func @bcast_col_after_scalar_reduce() {
  ...
  ttl.cb_push %out_cb : !ttl.cb<[1, 2], !ttcore.tile<32x32, bf16>, 2>
  ttl.cb_pop %red_cb : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>
  ttl.cb_pop %sc_cb : !ttl.cb<[1, 1], !ttcore.tile<32x32, bf16>, 2>
  ttl.cb_pop %inp_cb : !ttl.cb<[2, 2], !ttcore.tile<32x32, bf16>, 2>
  func.return
