...
func.func @binary_add(%arg0: tensor<2x2xf32>, %arg1: tensor<2x2xf32>) -> tensor<2x2xf32> {
  ...
  // CHECK-NEXT: %[[ADD:.+]] = ttl.tile_add %[[IN0]], %[[IN1]] {dst_idx = 0 : i32} : f32
  // CHECK-NEXT: ttl.yield %[[ADD]] : f32
  // CHECK-NEXT: } -> tensor<2x2xf32>
  // CHECK-NEXT: return %[[RESULT]]
  %0 = ttl.add %arg0, %arg1 : tensor<2x2xf32>, tensor<2x2xf32> -> tensor<2x2xf32>
...
func.func @unary_exp(%arg0: tensor<4x4xf32>) -> tensor<4x4xf32> {
  ...
  // CHECK-NEXT: %[[EXP:.+]] = ttl.tile_exp %[[IN]] {dst_idx = 0 : i32} : f32
  // CHECK-NEXT: ttl.yield %[[EXP]] : f32
...
// CHECK-SAME: (%[[ARG0:.+]]: tensor<2x2xf32>, %[[ARG1:.+]]: tensor<2x2xf32>)
func.func @chained_ops(%arg0: tensor<2x2xf32>, %arg1: tensor<2x2xf32>) -> tensor<2x2xf32> {
  // CHECK: %[[INIT0:.+]] = tensor.empty() : tensor<2x2xf32>
  // CHECK-NEXT: %[[ADD:.+]] = ttl.compute
  // CHECK-SAME: ins(%[[ARG0]], %[[ARG1]] : tensor<2x2xf32>, tensor<2x2xf32>)
  // CHECK-SAME: outs(%[[INIT0]] : tensor<2x2xf32>)
  // CHECK-NEXT: ^bb0(%[[IN0:.+]]: f32, %[[IN1:.+]]: f32, %[[OUT0:.+]]: f32):
  // CHECK-NEXT: %[[TILE_ADD:.+]] = ttl.tile_add %[[IN0]], %[[IN1]] {dst_idx = 0 : i32} : f32
  // CHECK-NEXT: ttl.yield %[[TILE_ADD]] : f32
  %0 = ttl.add %arg0, %arg1 : tensor<2x2xf32>, tensor<2x2xf32> -> tensor<2x2xf32>

  // CHECK: %[[INIT1:.+]] = tensor.empty() : tensor<2x2xf32>
  // CHECK-NEXT: %[[RESULT:.+]] = ttl.compute
  // CHECK-SAME: ins(%[[ADD]] : tensor<2x2xf32>)
  // CHECK-SAME: outs(%[[INIT1]] : tensor<2x2xf32>)
  // CHECK-NEXT: ^bb0(%[[IN2:.+]]: f32, %[[OUT1:.+]]: f32):
  // CHECK-NEXT: %[[TILE_RELU:.+]] = ttl.tile_relu %[[IN2]] {dst_idx = 0 : i32} : f32
  // CHECK-NEXT: ttl.yield %[[TILE_RELU]] : f32
  %1 = ttl.relu %0 : tensor<2x2xf32> -> tensor<2x2xf32>
...
func.func @all_binary_ops(%a: tensor<2x2xf32>, %b: tensor<2x2xf32>) -> tensor<2x2xf32> {
  // CHECK: ttl.compute
  // CHECK: ttl.tile_add
...
func.func @all_unary_ops(%x: tensor<2x2xf32>) -> tensor<2x2xf32> {
  // CHECK: ttl.compute
  // CHECK: ttl.tile_exp
  ...
  %sigmoid = ttl.sigmoid %tanh : tensor<2x2xf32> -> tensor<2x2xf32>

  // CHECK: ttl.compute
  // CHECK: ttl.tile_neg
  %neg = ttl.neg %sigmoid : tensor<2x2xf32> -> tensor<2x2xf32>
  ...
  // CHECK: ttl.compute
  // CHECK: ttl.tile_relu
  %relu = ttl.relu %abs : tensor<2x2xf32> -> tensor<2x2xf32>
...
// CHECK-SAME: (%[[A:.+]]: tensor<2x2xf32>, %[[B:.+]]: tensor<2x2xf32>, %[[C:.+]]: tensor<2x2xf32>)
func.func @dst_assignment_chain(%a: tensor<2x2xf32>, %b: tensor<2x2xf32>, %c: tensor<2x2xf32>) -> tensor<2x2xf32> {
