...
// CHECK: #[[$MAP_R0:.*]] = affine_map<(d0, d1) -> (0, d1)>
...
func.func @reduce_sum_dim0() attributes {ttl.base_cta_index = 3 : i32, ttl.crta_indices = [], ttl.kernel_thread = #ttkernel.thread<compute>} {
  ...
  %cb2 = ttl.bind_cb {cb_index = 2, buffer_factor = 2} : !ttl.cb<[1, 2], !ttcore.tile<32x32, bf16>, 2>
  ...
  // CHECK-SAME: indexing_maps = [#[[$MAP_ID]], #[[$MAP_C00]], #[[$MAP_R0]]]
  ...
  ttl.cb_push %cb2 : <[1, 2], !ttcore.tile<32x32, bf16>, 2>
...
// CHECK: #[[$MAP_ID:.*]] = affine_map<(d0, d1) -> (d0, d1)>
// CHECK: #[[$MAP_C00:.*]] = affine_map<(d0, d1) -> (0, 0)>
// CHECK: #[[$MAP_C1:.*]] = affine_map<(d0, d1) -> (d0, 0)>
...
func.func @reduce_sum_dim1() attributes {ttl.base_cta_index = 3 : i32, ttl.crta_indices = [], ttl.kernel_thread = #ttkernel.thread<compute>} {
  ...
  // CHECK: ttl.compute
  // CHECK-SAME: indexing_maps = [#[[$MAP_ID]], #[[$MAP_C00]], #[[$MAP_C1]]]
  ...
  ttl.cb_push %cb2 : <[2, 1], !ttcore.tile<32x32, bf16>, 2>
  ttl.cb_pop %cb1 : <[1, 1], !ttcore.tile<32x32, bf16>, 2>
  ttl.cb_pop %cb0 : <[2, 2], !ttcore.tile<32x32, bf16>, 2>
  func.return
}
