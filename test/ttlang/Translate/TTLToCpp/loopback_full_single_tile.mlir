...
// First accessor uses literal base CTA index = num_cbs = 1
...
// CHECK:   auto [[ARGS_WRITE:tensor_accessor_args_[0-9]+]] = TensorAccessorArgs<[[ARGS_READ]].next_compile_time_args_offset(), [[ARGS_READ]].next_common_runtime_args_offset()>();
// CHECK:   TensorAccessor [[ACC_WRITE:v[0-9]+]] = TensorAccessor([[ARGS_WRITE]], [[RT_ARG_W]], [[ADDR]]);
// CHECK:   for (size_t [[IV:i[0-9]+]] = [[LB]]; [[IV]] < [[UB]]; [[IV]] += [[STEP]]) {
// Read: tensor → CB (uses get_write_ptr for CB destination, pre-materialized ACC_READ)
...
// Write: CB → tensor (uses get_read_ptr for CB source, pre-materialized ACC_WRITE)
// CHECK:     int32_t {{v[0-9]+}} = get_common_arg_val<uint32_t>([[STEP]]);
...
// CHECK-NEXT: }
module {
  func.func @loopback(%src: tensor<32x32xf32, #layout>, %dst: tensor<32x32xf32, #layout>) attributes {ttl.kernel_thread = #ttkernel.thread<noc>, ttl.base_cta_index = 1 : i32} {
