...
Follows the elementwise example pattern from ~/tt/elementwise.
"""
...
def get_compute_kernel(
    op: ComputeOpSpec, config: TestConfig, device: Any = None
) -> str:
    """
    ...
    _, compute_kernel = translate_module_to_kernels(compiled_module)
...
def run_compute_test(
    ...
    kernel_dir = Path(f"/tmp/ttlang_kernels_{op.name}_{id(config)}")
