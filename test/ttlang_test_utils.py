...
def open_fabric_mesh(
    ...
    *,
    ...
    if fabric_config == ttnn_module.FabricConfig.FABRIC_1D:
        non_unit_dims = sum(1 for extent in requested_mesh_shape if int(extent) > 1)
        if non_unit_dims > 1:
            raise ValueError("FABRIC_1D requires a linear logical mesh")
