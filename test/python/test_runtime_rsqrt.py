...
def test_runtime_rsqrt(input, out):
    input_accessor = TensorAccessor(input)

    @compute()
    async def rsqrt_compute(input_cb: CircularBuffer, out_cb: CircularBuffer):
        inp = input_cb.wait()
        o = out_cb.reserve()
        result = rsqrt(inp)
        o.store(result)
    ...
    return Program(rsqrt_compute, dm_input)(input, out)


# CHECK: func.func @test_runtime_rsqrt
# CHECK: ^compute{{[0-9]+}}
# CHECK: %{{.+}} = linalg.generic
# CHECK: "d2m.tile_rsqrt"

# CHECK-LOWERED: func.func @test_runtime_rsqrt
# CHECK-LOWERED: emitc.call_opaque "rsqrt_tile"
...
print(f"Expected: all 0.5 (1/sqrt(4))")

test_runtime_rsqrt(input, out)
...
if torch.allclose(out, expected, rtol=1e-2, atol=1e-2):
    print("PASS: Output matches expected rsqrt(4.0) = 0.5")
