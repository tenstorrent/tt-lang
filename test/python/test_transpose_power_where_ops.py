...
def transpose_2x2_kernel(inp, out):
    ...
    out_cb = ttl.make_dataflow_buffer_like(out, shape=(2, 2), buffer_factor=2)
...
def transpose_4x2_kernel(inp, out):
    ...
    """
    inp_cb = ttl.make_dataflow_buffer_like(inp, shape=(4, 2), buffer_factor=2)
    out_cb = ttl.make_dataflow_buffer_like(out, shape=(2, 4), buffer_factor=2)
...
def where_kernel(cond, true_val, false_val, out):
    ...
    cond_cb = ttl.make_dataflow_buffer_like(cond, shape=(1, 1), buffer_factor=2)
    true_cb = ttl.make_dataflow_buffer_like(true_val, shape=(1, 1), buffer_factor=2)
    false_cb = ttl.make_dataflow_buffer_like(false_val, shape=(1, 1), buffer_factor=2)
    out_cb = ttl.make_dataflow_buffer_like(out, shape=(1, 1), buffer_factor=2)
