...
# Test: Element-wise multiply followed by matmul

import torch
from ttlang.d2m_api import *

@pykernel_gen(grid=(1, 1), block_factors=[(1, 1), (1, 1), (1, 1), (1, 1)])
def test_mul_matmul(A, B, C, out):
...
# CHECK: func.func @test_mul_matmul
# CHECK: "d2m.tile_mul"
# CHECK: "d2m.tile_matmul"

# Test: (2 * 3) @ 4 = 6 @ 4 = 6 * 4 * 32 = 768
...
print(f"Expected: (2 * 3) @ 4 = 6 @ 4 = {6.0 * 4.0 * 32}")

# Compute expected
AB_expected = A * B  # All 6.0
result_expected = AB_expected @ C  # 6 * 4 * 32 = 768 per element

test_mul_matmul(A, B, C, out)
...
if abs(out[0, 0].item() - result_expected[0, 0].item()) / result_expected[0, 0].item() < 0.1:
    print(f"PASS: mul → matmul produced correct result")
    # CHECK-OUTPUT: PASS: mul
else:
