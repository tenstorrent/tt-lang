...
DEVICE_ADDRESSING_MODES = (
    "uniform",
    pytest.param("per-node", marks=pytest.mark.hybrid_allocator),
)
...
def test_persistent_state_shared_by_distinct_operations(
    ...
    shape, nodes, grid, sharding = configurations[sharding_name]
...
def test_allocate_initialize_once_and_repeated_external_updates(
    ...
    shape, nodes, sharding = geometries[layout]
    with SRAMStorage(device=runtime.device) as storage:
        ...
        expected.experimental_set_per_core_allocation(addressing == "per-node")
...
def test_external_tensor_alias_wrapper_restores_owned_reference(runtime, addressing):
    ...
    if addressing == "per-node":
        make_per_node(runtime.allocations[0])
...
def test_block_sharding_requires_complete_rectangular_node_set(runtime):
    with SRAMStorage(device=runtime.device) as storage:
