...
def matmul_single_tile_kernel(a, b, c):
    ...
    def compute_fn():
        with a_cb.wait() as a_tile, b_cb.wait() as b_tile, c_cb.reserve() as c_out:
            c_out.store(ttl.math.matmul(a_tile, b_tile))
...
def matmul_m_tiling_kernel(a, b, c):
    ...
    """
    a_cb = ttl.make_dataflow_buffer_like(a, shape=(2, 1), buffer_factor=2)
    b_cb = ttl.make_dataflow_buffer_like(b, shape=(1, 1), buffer_factor=2)
    c_cb = ttl.make_dataflow_buffer_like(c, shape=(2, 1), buffer_factor=2)
...
def matmul_n_tiling_kernel(a, b, c):
    ...
    """
    a_cb = ttl.make_dataflow_buffer_like(a, shape=(1, 1), buffer_factor=2)
    b_cb = ttl.make_dataflow_buffer_like(b, shape=(1, 2), buffer_factor=2)
...
def matmul_full_multitile_kernel(a, b, c):
    ...
    c_cb = ttl.make_dataflow_buffer_like(c, shape=(2, 2), buffer_factor=2)
...
def matmul_large_multitile_kernel(a, b, c):
    ...
    """
    a_cb = ttl.make_dataflow_buffer_like(a, shape=(4, 4), buffer_factor=2)
    b_cb = ttl.make_dataflow_buffer_like(b, shape=(4, 4), buffer_factor=2)
    c_cb = ttl.make_dataflow_buffer_like(c, shape=(4, 4), buffer_factor=2)
