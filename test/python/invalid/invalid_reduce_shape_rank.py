...
"""Validation test: reduce rejects a result shape with the wrong rank."""
...
# CHECK: reduce shape (1,) has 1 dimensions but input has rank 2
@ttl.operation(grid=(1, 1))
def invalid_reduce_shape_rank_kernel(inp, out):
    ...
    def compute_fn():
        with inp_dfb.wait() as inp_blk, out_dfb.reserve() as out_blk:
            result = ttl.math.reduce_sum(inp_blk, dims=[0, 1], shape=(1,))
...
if __name__ == "__main__":
    ...
    try:
        ...
        invalid_reduce_shape_rank_kernel(inp, out)
    finally:
