...
# RUN: env TTLANG_COMPILE_ONLY=1 TTLANG_CASE=dynamic not %python %s 2>&1 | FileCheck %s --check-prefix=DYNAMIC
...
@ttl.operation(grid=(1, 1), device_domain=DEVICE_DOMAIN)
def dynamic_identity_operand():
    @ttl.datamovement()
    def data_movement():
        expected_source, _node_y = ttl.node(dims=2)

        def callback(pipe):
            if pipe.source_device_index == expected_source:
                pass
...
# TYPES: error: invalid pipe callback identity comparison:
