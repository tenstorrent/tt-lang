...
"""
Validation test: CB buffer_factor must be >= 1.

This test verifies that using buffer_factor=0 for make_circular_buffer_like()
...
# CHECK: buffer_factor must be >= 1
@ttl.kernel(grid=(1, 1))
def invalid_buffer_factor_kernel(lhs, rhs, out):
    """This kernel should fail because buffer_factor=0 is invalid."""
    lhs_accessor = TensorAccessor(lhs)
    rhs_accessor = TensorAccessor(rhs)
    out_accessor = TensorAccessor(out)

    # INVALID: buffer_factor must be >= 1
    lhs_cb = make_circular_buffer_like(lhs, shape=(1, 1), buffer_factor=0)
...
if __name__ == "__main__":
    ...
    print("=== CB Buffer Factor Validation Test ===")
    ...
    try:
        ...
        invalid_buffer_factor_kernel(lhs, rhs, out)
