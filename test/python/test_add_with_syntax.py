...
# This test uses the 'with' statement syntax for CircularBuffer reserve().
...
def test_add_with_syntax(lhs, rhs, out):
    @compute()
    def add_compute(
        lhs_cb: CircularBuffer, rhs_cb: CircularBuffer, out_cb: CircularBuffer
    ):
        # Consumer pattern: explicit wait + pop
        l = lhs_cb.wait()
        r = rhs_cb.wait()
...
# CHECK: func.func @test_add_with_syntax
...
# Verify: push is generated from 'with' statement
# CHECK: d2m.push %{{.+}} : <tensor<1x1x!ttcore.tile<{{[0-9]+}}x{{[0-9]+}}, {{.*}}>>>

# CHECK-LOWERED: func.func @test_add_with_syntax
...
test_add_with_syntax(lhs, rhs, out)
