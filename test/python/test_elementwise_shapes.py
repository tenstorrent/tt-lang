...
def {name}_kernel(lhs, rhs, out):
    ...
    @ttl.datamovement()
    def dm_read():
        ttl.copy(lhs[{slice_syntax}], lhs_dfb.reserve())
        ttl.copy(rhs[{slice_syntax}], rhs_dfb.reserve())

    @ttl.datamovement()
    def dm_write():
        ttl.copy(out_dfb.wait(), out[{slice_syntax}])
...
def {name}_kernel(inp, out):
    ...
    @ttl.datamovement()
    def dm_read():
        ttl.copy(inp[{slice_syntax}], inp_dfb.reserve())
