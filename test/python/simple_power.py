...
"""
Simple power kernel - verifies power(x, 2) lowers to correct TTL ops and C++ code.

Tests single-tile power with integer exponent.
"""
...
def power_kernel(inp, out):
    ...
    def power_compute():
        with inp_dfb.wait() as inp, out_dfb.reserve() as out:
            out.store(ttl.math.power(inp, 2))
...
# CHECK-LABEL: func.func @power_compute
...
# CHECK-CPP: power_tile_init();
# CHECK-CPP: power_tile(
