...
def test_runtime_mul(lhs, rhs, out):
    lhs_accessor = TensorAccessor(lhs)
    rhs_accessor = TensorAccessor(rhs)

    @compute()
    async def compute_mul(lhs_cb: CircularBuffer, rhs_cb: CircularBuffer, out_cb: CircularBuffer):
        l = lhs_cb.wait()
        r = rhs_cb.wait()
        o = out_cb.reserve()
        result = l * r
    ...
    return Program(compute_mul, dm_lhs, dm_rhs)(lhs, rhs, out)


# CHECK: func.func @test_runtime_mul
# CHECK: "d2m.tile_mul"

lhs = torch.full((32, 32), 3.0)
rhs = torch.full((32, 32), 4.0)
out = torch.full((32, 32), -999.0)

print("=== BEFORE KERNEL ===")
print(f"lhs: all 3.0, rhs: all 4.0, Expected: 12.0")

test_runtime_mul(lhs, rhs, out)

print("\n=== AFTER KERNEL ===")
# CHECK-OUTPUT: === AFTER KERNEL ===
expected = lhs * rhs
if torch.allclose(out, expected, rtol=1e-2, atol=1e-2):
    print("PASS: Output matches expected mul(3.0, 4.0) = 12.0")
