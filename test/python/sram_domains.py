...
@ttl.operation(grid=(2, 1))
def node_sensitive_copy(source, destination):
...
def test_per_node_temporal_reuse(
    ...
    # Both buffers execute on both worker nodes; only node zero completes the
    # first buffer before acquiring the second. The optimum is one page on node
    # zero and two pages on node one when reuse is enabled.
    monkeypatch.delenv("TTLANG_COMPILER_OPTIONS", raising=False)
    ...
    assert totals["uniform"] - totals["per-node"] == (
