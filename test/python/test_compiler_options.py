...
class TestFromString:
    ...
    def test_disable_maximize_dst(self):
        opts = CompilerOptions.from_string("--no-maximize-dst")
        assert opts.maximize_dst is False
        assert opts.enable_fpu_binary_ops is True
        assert "maximize_dst" in opts._explicit

    def test_disable_fpu(self):
        opts = CompilerOptions.from_string("--no-fpu-binary-ops")
    ...
    def test_enable_flags_explicitly(self):
        opts = CompilerOptions.from_string("--maximize-dst --fpu-binary-ops")
        assert opts.maximize_dst is True
        assert opts.enable_fpu_binary_ops is True
        assert opts._explicit == frozenset({"maximize_dst", "enable_fpu_binary_ops"})

    def test_contradictory_last_wins(self):
        opts = CompilerOptions.from_string("--no-maximize-dst --maximize-dst")
        assert opts.maximize_dst is True
    ...
    def test_multiple_flags(self):
        opts = CompilerOptions.from_string("--no-maximize-dst --no-fpu-binary-ops")
        assert opts.maximize_dst is False
        assert opts.enable_fpu_binary_ops is False
...
class TestMerge:
    def test_override_replaces_base(self):
        base = CompilerOptions.from_string("--maximize-dst")
        override = CompilerOptions.from_string("--no-maximize-dst")
    ...
    def test_both_set_same_field(self):
        base = CompilerOptions.from_string("--no-fpu-binary-ops")
        override = CompilerOptions.from_string("--fpu-binary-ops")
        result = base.merge(override)
...
class TestFromArgv:
    def test_extracts_known_flags(self):
        with mock.patch.object(
            sys, "argv", ["script.py", "--no-maximize-dst", "some_file.py"]
        ):
    ...
    def test_ignores_unknown_flags(self):
        with mock.patch.object(
            sys, "argv", ["script.py", "--unknown-flag", "-v", "--no-fpu-binary-ops"]
        ):
            opts = CompilerOptions.from_argv()
        assert opts.enable_fpu_binary_ops is False
    ...
    def test_help_exits_for_direct_script(self):
        with mock.patch.object(sys, "argv", ["my_kernel.py", "--help"]):
            with pytest.raises(SystemExit) as exc_info:
                CompilerOptions.from_argv()
            assert exc_info.value.code == 0

    def test_help_ignored_under_pytest(self):
        with mock.patch.object(sys, "argv", ["pytest", "--help", "--no-maximize-dst"]):
            opts = CompilerOptions.from_argv()
        # --help should be ignored; --no-maximize-dst should still be parsed.
        assert opts.maximize_dst is False
...
class TestEquality:
    def test_same_values_equal_regardless_of_explicit(self):
        a = CompilerOptions()
        b = CompilerOptions.from_string("--maximize-dst --fpu-binary-ops")
        assert a == b

    def test_different_values_not_equal(self):
        a = CompilerOptions()
        b = CompilerOptions.from_string("--no-maximize-dst")
        assert a != b
