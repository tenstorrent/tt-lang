...
def test_binary_op_sharded(device, op_name):
    """Test binary elementwise operation with L1 height-sharded memory."""
    kernel, torch_fn = BINARY_OPS[op_name]
    ...
    lhs = to_l1_sharded(lhs_torch, device)
    rhs = to_l1_sharded(rhs_torch, device)
    out = to_l1_sharded(out_torch, device)
...
def test_unary_op_sharded(device, op_name):
    """Test unary elementwise operation with L1 height-sharded memory."""
    kernel, torch_fn = UNARY_OPS[op_name]
    ...
    inp = to_l1_sharded(inp_torch, device)
    out = to_l1_sharded(out_torch, device)
