# SPDX-FileCopyrightText: (c) 2025 Tenstorrent AI ULC
#
# SPDX-License-Identifier: Apache-2.0

# REQUIRES: ttnn, tt-device
# RUN: env TTLANG_COMPILE_ONLY=1 TTLANG_INITIAL_MLIR=%t.initial.mlir %python %s > %t.output 2>&1
# RUN: FileCheck %s < %t.initial.mlir

"""
Pipe forward test - verifies data transfer from core (0,0) to core (1,0).

Core (0,0) reads input tile, sends it to core (1,0) via pipe.
Core (1,0) receives and writes to output.
"""

import os

os.environ["TTLANG_COMPILE_ONLY"] = "1"

import ttnn
import ttl


@ttl.kernel(grid=(2, 1))
def pipe_forward(inp, out):
    """Forward data from core 0 to core 1 via pipe."""
    # Pipe from src=(0,0) to dst=(1,0)
    pipe = ttl.Pipe(src=(0, 0), dst=(1, 0))

    inp_cb = ttl.make_circular_buffer_like(inp, shape=(1, 1), buffer_factor=2)
    out_cb = ttl.make_circular_buffer_like(out, shape=(1, 1), buffer_factor=2)

    @ttl.compute()
    def compute():
        # Identity compute - just pass through
        with inp_cb.wait() as tile_in:
            with out_cb.reserve() as tile_out:
                tile_out.store(tile_in)

    @ttl.datamovement()
    def dm_read():
        with inp_cb.reserve() as inp_blk:
            # Source core reads input and sends via pipe
            with pipe.if_src():
                tx_read = ttl.copy(inp[0, 0], inp_blk)
                tx_read.wait()
                tx_send = ttl.copy(inp_blk, pipe)
                tx_send.wait()

            # Destination core receives via pipe
            with pipe.if_dst():
                tx_recv = ttl.copy(pipe, inp_blk)
                tx_recv.wait()

    @ttl.datamovement()
    def dm_write():
        # Both cores write output
        with out_cb.wait() as out_blk:
            x, y = ttl.core(dims=2)
            tx_write = ttl.copy(out_blk, out[y, x])
            tx_write.wait()


# =============================================================================
# Initial IR Checks - TTL dialect ops for pipe
# =============================================================================

# CHECK-LABEL: func.func @dm_read
# CHECK-SAME: ttl.kernel_thread = #ttkernel.thread<noc>

# Check pipe creation
# CHECK: ttl.create_pipe src(0, 0) dst(1, 0) to(1, 0)

# Check if_src block
# CHECK: ttl.if_src %{{.+}} : <src(0, 0) dst(1, 0) to(1, 0)>
# CHECK: ttl.copy

# Check if_dst block
# CHECK: ttl.if_dst %{{.+}} : <src(0, 0) dst(1, 0) to(1, 0)>
# CHECK: ttl.copy


if __name__ == "__main__":
    import torch
    from ttlang_test_utils import require_hardware

    print("=== Pipe Forward Test ===")
    require_hardware()

    device = ttnn.open_device(device_id=0)

    try:
        # 2x1 grid = 64x32 tensor (2 tiles wide, 1 tile tall)
        inp_torch = torch.full((32, 64), 42.0, dtype=torch.bfloat16)
        out_torch = torch.zeros((32, 64), dtype=torch.bfloat16)

        inp = ttnn.from_torch(
            inp_torch,
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=device,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )
        out = ttnn.from_torch(
            out_torch,
            dtype=ttnn.bfloat16,
            layout=ttnn.TILE_LAYOUT,
            device=device,
            memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )

        print("Running pipe forward kernel...")
        pipe_forward(inp, out)

        # Verify output
        out_torch = ttnn.to_torch(out)
        expected = torch.full((32, 64), 42.0, dtype=torch.bfloat16)

        if torch.allclose(out_torch, expected):
            print("PASS: Output matches expected")
        else:
            print(f"FAIL: Output mismatch")
            print(f"Expected: {expected[0, :4]}")
            print(f"Got: {out_torch[0, :4]}")

        print("=== Pipe Forward Test Complete ===")

    finally:
        ttnn.close_device(device)
