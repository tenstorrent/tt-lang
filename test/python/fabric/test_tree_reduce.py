...
def test_tree_all_reduce(
    ...
    print(f"Tree {ttnn_dtype}: create input tensor", flush=True)
    inp = _mesh_tensor(participant_mesh, inp_torch, ttnn_dtype)
    print(f"Tree {ttnn_dtype}: create output tensor", flush=True)
    out = _mesh_tensor(participant_mesh, out_torch, ttnn_dtype)

    print(f"Tree {ttnn_dtype}: dispatch", flush=True)
    tree_all_reduce(inp, out)
    print(f"Tree {ttnn_dtype}: dispatch returned", flush=True)

    print(f"Tree {ttnn_dtype}: read output", flush=True)
    result = ttnn.to_torch(
        out,
        mesh_composer=ttnn.ConcatMeshToTensor(participant_mesh, dim=0),
    )
    print(f"Tree {ttnn_dtype}: read output returned", flush=True)
