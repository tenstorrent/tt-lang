...
"""Two-endpoint routing-plane PipeNet ping-pong coverage."""
...
def _make_ping_pong(mesh_shape):
    ...
    return_net = ttl.PipeNet(
        graph=ttl.TransferGraph.edges(
            device_domain, edges=[(destination_device, source_device)]
        )
    )
    ...
    return ping_pong, device_domain, source_device
...
def test_ping_pong(torch_dtype, ttnn_dtype):
    ...
    source_index = device_domain.index_order(source_device)
    source_slice = slice(source_index * TILE_SIZE, (source_index + 1) * TILE_SIZE)
    expected[source_slice, :] = inp_torch[source_slice, :]
