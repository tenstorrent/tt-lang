...
# Test: reduce_sum with broadcast=True to validate broadcasting works correctly
# This is an isolated test to debug the broadcast functionality before using it in FA
...
# CHECK: func.func @test_reduce_broadcast
# CHECK: "d2m.tile_reduce_sum"
# CHECK: linalg.generic
# CHECK: linalg.yield

# Test: reduce_sum(2.0 * 32 columns) = 64.0, should be broadcast to all positions
...
print(f"Expected: reduce_sum(2.0 * 32) = 64.0 at ALL positions")
print(f"Reduction dim: 1 (sum across columns)")

test_reduce_broadcast(A, ones, out)
...
if all_rows_same and abs(out[0, 0].item() - expected) / expected < tolerance:
    print(f"\nPASS: reduce_sum with broadcast=True produced correct result!")
    # CHECK-OUTPUT: PASS: reduce_sum with broadcast=True produced correct result
else:
