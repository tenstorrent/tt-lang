...
def test_build_cb_descriptors_excludes_computed_address_backing_tensors(
    ...
    cb_configs = [
        PhysicalDFBConfig(0, 1, "bfloat16", 1, 512, (16, 16)),
        PhysicalDFBConfig(1, 1, "bfloat16", 1, 800, (20, 20)),
    ]
...
def test_emit_runner_source_preserves_subtile_geometry(monkeypatch):
    ...
    assert "page_size=512, tile=(16, 16)),  # DFB 0" in source
...
def test_emit_runner_source_accepts_physical_dfb_configs():
    ...
    assert (
        "PhysicalDFBConfig(dfb_index=0, num_tiles=3, "
        "data_format='bfloat16', block_count=2, page_size=32, "
        "tile=(1, 16)),  # DFB 0"
    ) in source
