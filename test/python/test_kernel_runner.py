...
SUBTILE_DFB_CONFIG = PhysicalDFBConfig(
    dfb_index=0,
    num_tiles=1,
...
def test_build_cb_descriptors_excludes_computed_address_backing_tensors(
    ...
    cb_configs = [
        (1, 1, object(), (32, 32), 512, 512),
        (1, 1, object(), (32, 32), 800, 800),
    ]
...
def test_serialized_dfb_config_requires_current_format():
    with pytest.raises(
        ValueError,
        match="Serialized DFB config 0 has 5 fields; regenerate the runner",
    ):
        kernel_runner._get_dfb_descriptor_configs(
            [(1, 2, _FakeDataFormat(), (16, 16), 1024)]
        )
...
def test_emit_runner_source_preserves_subtile_geometry(monkeypatch):
    ...
    assert "(1, 2, ttnn.bfloat16, (16, 16), 512, 1024)" in source
