...
def test_reconfiguration_runtime_storage_uses_exact_node_union(monkeypatch):
    ...
    assert int(host_configurations[0][0, 1]) == 0
    assert int(host_configurations[0][1, 1]) == 1
...
def test_reconfiguration_runtime_storage_honors_shared_launch_formats(monkeypatch):
    fake_ttnn = _FakeTTNN()
    fake_ttnn.uint32 = "uint32"
    fake_ttnn.ROW_MAJOR_LAYOUT = "row-major"
    fake_ttnn.ShardOrientation = type("ShardOrientation", (), {"ROW_MAJOR": 0})
    ...
    scratch_tensor = _FakeTensor(device, address=0x8000)
    scratch_allocations = []
    fake_ttnn.from_torch = lambda *_args, **_kwargs: _FakeTensor(
        device, address=0x9000
    )
    monkeypatch.setattr(kernel_runner, "ttnn", fake_ttnn)
    ...
    runtime_configs = (
        PhysicalDFBConfig(
            0, 1, "bfloat16", 1, 64, (1, 32), storage_index=4
        ),
        PhysicalDFBConfig(
            1, 1, "bfloat16", 1, 64, (1, 32), storage_index=4
        ),
    )
    launch_configs = (
        runtime_configs[0],
        PhysicalDFBConfig(
            1, 1, "bfloat16", 1, 2048, (32, 32), storage_index=4
        ),
    )
    plan = DFBReconfigurationPlan(
        boundary_ordinals=(7,),
        dfb_epochs=(
            (
                DFBConfigurationEpoch(None, runtime_configs[0]),
                DFBConfigurationEpoch(7, runtime_configs[0]),
            ),
            (
                DFBConfigurationEpoch(None, runtime_configs[1]),
                DFBConfigurationEpoch(7, runtime_configs[1]),
            ),
        ),
    )
    ...
    resources = kernel_runner.build_dfb_reconfiguration_runtime_resources(
        ...
        plan=plan,
        device=device,
        cb_configs=list(launch_configs),
    )
