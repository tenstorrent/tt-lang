...
def reduce_sum_scalar_kernel(inp, scaler, out):
    ...
    def compute_fn():
        with inp_cb.wait() as i, scaler_cb.wait() as s, out_cb.reserve() as o:
            result = ttl.math.reduce_sum(i, s, dims=[0, 1])
            o.store(result)
...
def reduce_max_scalar_kernel(inp, scaler, out):
    ...
    def compute_fn():
        with inp_cb.wait() as i, scaler_cb.wait() as s, out_cb.reserve() as o:
            result = ttl.math.reduce_max(i, s, dims=[0, 1])
            o.store(result)
...
def bcast_then_reduce_kernel(inp, bcast_in, scaler, out):
    ...
    """
    inp_cb = ttl.make_dataflow_buffer_like(inp, shape=(2, 2), buffer_factor=2)
    bcast_cb = ttl.make_dataflow_buffer_like(bcast_in, shape=(2, 2), buffer_factor=2)
    bcast_out_cb = ttl.make_dataflow_buffer_like(inp, shape=(2, 2), buffer_factor=2)
    add_out_cb = ttl.make_dataflow_buffer_like(inp, shape=(2, 2), buffer_factor=2)
    scaler_cb = ttl.make_dataflow_buffer_like(scaler, shape=(1, 1), buffer_factor=2)
    out_cb = ttl.make_dataflow_buffer_like(out, shape=(1, 1), buffer_factor=2)
    ...
    def compute_fn():
        ...
        with add_out_cb.wait() as a_in, scaler_cb.wait() as s, out_cb.reserve() as o:
            result = ttl.math.reduce_sum(a_in, s, dims=[0, 1])
            o.store(result)
...
def matmul_then_reduce_kernel(a, b, scaler, out):
    ...
    """
    a_cb = ttl.make_dataflow_buffer_like(a, shape=(2, 2), buffer_factor=2)
    b_cb = ttl.make_dataflow_buffer_like(b, shape=(2, 2), buffer_factor=2)
    c_cb = ttl.make_dataflow_buffer_like(out, shape=(2, 2), buffer_factor=2)
    scaler_cb = ttl.make_dataflow_buffer_like(scaler, shape=(1, 1), buffer_factor=2)
    out_cb = ttl.make_dataflow_buffer_like(out, shape=(1, 1), buffer_factor=2)
    ...
    def compute_fn():
        ...
        with c_cb.wait() as c, scaler_cb.wait() as s, out_cb.reserve() as o:
            result = ttl.math.reduce_sum(c, s, dims=[0, 1])
            o.store(result)
...
def reduce_bcast_matmul_kernel(reduce_in, bcast_out_ref, matmul_b, scaler, out):
    ...
    """
    # Input CBs
    reduce_in_cb = ttl.make_dataflow_buffer_like(reduce_in, shape=(8, 8), buffer_factor=2)
    scaler_cb = ttl.make_dataflow_buffer_like(scaler, shape=(1, 1), buffer_factor=2)
    matmul_b_cb = ttl.make_dataflow_buffer_like(matmul_b, shape=(8, 8), buffer_factor=2)

    # Intermediate CBs
    reduce_out_cb = ttl.make_dataflow_buffer_like(scaler, shape=(1, 1), buffer_factor=2)
    bcast_out_cb = ttl.make_dataflow_buffer_like(bcast_out_ref, shape=(8, 8), buffer_factor=2)

    # Output CB
    out_cb = ttl.make_dataflow_buffer_like(out, shape=(8, 8), buffer_factor=2)
    ...
    def compute_fn():
        ...
        with (
            ...
            reduced = ttl.math.reduce_sum(r_in, s, dims=[0, 1])
            r_out.store(reduced)
