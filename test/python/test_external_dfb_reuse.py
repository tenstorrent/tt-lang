...
def _get_final_physical_dfb_indices(final_mlir_path):
    ...
    return [
        int(index)
        for index in re.findall(r"dfb_index = (\d+)", allocations.group(1))
    ]
...
def test_conditional_external_dfb_reuse_across_reconfiguration(
    ...
    source_host = torch.arange(TILE * TILE, dtype=torch.float32).reshape(
        TILE, TILE
    ).to(dtype)
    ...
    operation(source, result, options=METAL_STORAGE_OPTIONS)
