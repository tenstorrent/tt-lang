...
def test_later_record_failure_uses_earlier_owner_completion():
    first, _, _ = make_owner()
    second, _, _ = make_owner()
    earlier, later = sorted((first, second), key=id)
    later._backend.fail = "record"
    launch = with_persistent_storage(lambda *values: None)
    with pytest.raises(RuntimeError, match="record failed"):
        launch(first.reference(0), second.reference(0))

    assert len(earlier._backend.events) == 1
    assert later._backend.events == []
    earlier._backend.complete()
    later._backend.fail = None
    earlier.close()
    later.close()
    assert earlier._backend.recoveries == 0
    assert later._backend.recoveries == 1
