...
"""Verify explicit-handle synchronized DFB reset composition and lowering."""
...
def make_reset_operation():
    ...
    def synchronized_reset_explicit_operation(input_tensor):
        target = ttl.make_dfb("bf16", shape=(1, 1), block_count=2)
        reset_helper(target)

    return synchronized_reset_explicit_operation


synchronized_reset_explicit_operation = make_reset_operation()


# INITIAL-LABEL: func.func @synchronized_reset_explicit_operation__trisc
...
# INITIAL-LABEL: func.func @synchronized_reset_explicit_operation__ncrisc
# INITIAL-SAME: ttl.logical_kernel = #ttl.logical_kernel<kind = data_movement, identity = "dfb_reset_participant_data_movement_0_1_0"
# INITIAL: ttl.reset_dfbs <{{[0-9]+}}, participants[<kind = compute, identity = "dfb_reset_participant_compute_0_1_0"
# INITIAL-NEXT: ttl.reset_all_dfbs <{{[0-9]+}}, participants[<kind = compute, identity = "dfb_reset_participant_compute_0_1_0"
# INITIAL-LABEL: func.func @synchronized_reset_explicit_operation__brisc
...
if __name__ == "__main__":
    ...
    synchronized_reset_explicit_operation(input_tensor)
