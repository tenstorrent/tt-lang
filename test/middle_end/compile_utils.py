...
def compile_ttl_module(module: Module, system_desc_path: Optional[str] = None) -> Module:
    """
...
def translate_to_cpp(module: Module) -> CompiledKernels:
    ...
    if not all([reader_name, compute_name, writer_name]):
        raise ValueError(
            f"Could not identify all kernel types from: {kernel_info}"
        )
...
def get_kernel_output_dir(op_name: str, config_name: str, build_dir: Optional[str] = None) -> str:
    """
