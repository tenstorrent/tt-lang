...
class TestTwoComputesChained(TwoComputeTestBase):
    """
    Test two sequential compute ops with an intermediate CB.

    Pattern: compute1(a + b) -> CB2, then compute2(CB2 * CB2) -> CB3

    The result is (a + b)^2.

    Uses 4 CBs:
      - CB0, CB1: inputs (filled by reader)
      - CB2: intermediate (output of compute1, input of compute2)
      - CB3: final output (read by writer)
    ...
    3. The second compute correctly reads from CB2 and writes to CB3

    Note: Device execution requires inter-loop CB sync (cb_push/cb_wait
    between the compute loops), which is not yet auto-generated.
    This test is compile-only until that support is added.

    TODO(inter-loop-sync): Once compiler auto-generates inter-loop CB sync:
      - Remove skip markers from test_execute and test_validate_golden
      - The compiler should insert cb_push after first loop, cb_wait before second
    """
    ...
    def get_mlir_template(self, config: E2EConfig) -> str:
        ...
        Uses 4 CBs: 2 inputs, 1 intermediate, 1 final output.

        Note: In real hardware, there would need to be inter-loop CB sync
        (cb_push/cb_wait) between the computes. This test is compile-only
        ...
        writer_mlir = dm_builder.build_writer(output_cbs=[3], total_cbs=4)
  ...
  func.func @compute_two_ops(%a: tensor<{rows}x{cols}x!ttcore.tile<32x32, {dtype}>>,
    ...
    // TODO(inter-loop-sync): Compiler auto-inserts cb_push after first loop,
    // but we also need cb_wait before second compute reads from CB2.

    // Second compute reads from CB2, writes to CB3.
    %r0_input = ttl.attach_cb %r0, %cb2 : (tensor<{rows}x{cols}x!ttcore.tile<32x32, {dtype}>>, !ttl.cb<[{rows}, {cols}], !ttcore.tile<32x32, {dtype}>, {bf}>) -> tensor<{rows}x{cols}x!ttcore.tile<32x32, {dtype}>>
    ...
    // Second compute: r0 * r0 -> CB3 (final output)
    %result = ttl.compute
        ins(%r0_input, %r0_input : tensor<{rows}x{cols}x!ttcore.tile<32x32, {dtype}>>,
                                   tensor<{rows}x{cols}x!ttcore.tile<32x32, {dtype}>>)
