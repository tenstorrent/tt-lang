...
result = ttl.math.asin(x)     # Arcsine
result = ttl.math.acos(x)     # Arccosine
result = ttl.math.atan(x)     # Arctangent
```
...
def gather_kernel(inp, out):
    ...
    def compute():
        ...
        if x == 0:
            ...
            for _ in range(3):
                with gather_dfb.wait() as g, out_dfb.reserve() as o:
                    o.store(g)  # Simplified - real code would accumulate
        else:
            # Workers: process and send to gather_dfb
            with inp_dfb.wait() as i, gather_dfb.reserve() as g:
    ...
    def dm_write():
        ...
        if x == 0:
            with gather_dfb.reserve() as blk:
...
def full_reduce_bcast_matmul_kernel(A, B, scaler, out):
    ...
    def compute():
        ...
        if x == COORDINATOR:
            with matmul_out_dfb.wait() as m0, matmul_acc_dfb.reserve() as macc:
                macc.store(m0)

            for _ in range(NUM_WORKERS):
                with matmul_gather_dfb.wait() as m, matmul_acc_dfb.wait() as acc, matmul_acc_dfb.reserve() as new_acc:
                    new_acc.store(acc + m)

            with matmul_acc_dfb.wait() as acc, out_dfb.reserve() as final_out:
                final_out.store(acc)
        else:
            with matmul_out_dfb.wait() as mout, matmul_gather_dfb.reserve() as mg:
                mg.store(mout)
    ...
    def dm_write():
        ...
        if x == 1:
            with matmul_gather_dfb.wait() as blk:
        ...
        if x == COORDINATOR:
            with matmul_gather_dfb.reserve() as blk:
...
def compute():
    with scaler_dfb.wait() as sc:
        ...
        with x_dfb.wait() as xv:
            ...
            with some_intermediate_dfb.wait() as intermediate:
                # ... more operations ...
...
def fused_block_kernel(attn_concat, x, wo, ln2_w, w_fc, w_proj, scaler, out):
    ...
    def compute():
        with scaler_dfb.wait() as sc:
            ...
            with act_dfb.wait() as ap, x_dfb.wait() as xv:
                with hidden1_dfb.reserve() as h1:
                    h1.store(ap + xv)

            # Keep hidden1 in scope for residual 2!
            with hidden1_dfb.wait() as h1v, ln2_w_dfb.wait() as ln2_wv:
                # Step 3: RMSNorm
                ...
                with mlp_acc_dfb.wait() as mlp_out, out_dfb.reserve() as o:
                    o.store(mlp_out + h1v)
    ...
    def dm_read():
        ...
        with x_dfb.reserve() as blk:
            tx = ttl.copy(x[0:SEQ_TILES, 0:EMBD_TILES], blk)
            tx.wait()
        with wo_dfb.reserve() as blk:
            tx = ttl.copy(wo[0:EMBD_TILES, 0:EMBD_TILES], blk)
            tx.wait()
        with ln2_w_dfb.reserve() as blk:
        ...
        for chunk in range(NUM_MLP_CHUNKS):
            ...
            with w_fc_chunk_dfb.reserve() as blk:
                tx = ttl.copy(w_fc[0:EMBD_TILES, col_start:col_end], blk)
                tx.wait()
            with w_proj_chunk_dfb.reserve() as blk:
...
def streaming_mlp_kernel(x, w_fc, w_proj, out):
    ...
    def compute():
        with x_dfb.wait() as xv:
            ...
            # Copy to output
            with out_acc_dfb.wait() as acc, out_dfb.reserve() as o:
                o.store(acc)
    ...
    def compute():
        ...
        with x_dfb.wait() as xv, scaler_dfb.wait() as sc:
            ...
            with max_bcast_dfb.wait() as mxbv:
                ...
                with sum_bcast_dfb.wait() as smbv, out_dfb.reserve() as o:
                    shifted2 = xv - mxbv
...
def rmsnorm_kernel(x, weight, scaler, out):
    ...
    @ttl.compute()
    def compute():
        with x_dfb.wait() as xv, weight_dfb.wait() as wv, scaler_dfb.wait() as sc:
            # Square
...
def layer2_kernel(hidden, w2, bias2, scaler, out):
    ...
    def compute():
        ...
        with acc_dfb.wait() as accv, bias2_dfb.wait() as b2v:
            with logits_dfb.reserve() as lg:
                lg.store(accv + b2v)
    ...
    def dm_read():
        ...
        for i in range(NUM_CHUNKS):
            ...
            with hidden_dfb.reserve() as blk:
