    ...
    }
    {
        ...
        else
        {
            for (std::uint32_t loop = 0; loop < LOOP_FACTOR; loop++)
            {
                ...
                {
                    // Operands from the StimuliConfig layout rather than PERF_ADDRESS, whose
                    // 4096-byte tile stride (perf.h:36) is right only for Float32 and makes
                    // this kernel walk L1 differently from the migrated sources. reduce_tile
                    // is measured only here, so no cross-source check would catch it.
                    _llk_unpack_AB_reduce_<POOL_TYPE, REDUCE_DIM>(L1_ADDRESS(buffer_A[tile]), L1_ADDRESS(buffer_B[tile]));
                }
            }
        }
        PROFILER_SYNC();
