# SPDX-FileCopyrightText: (c) 2025 Tenstorrent AI ULC
# SPDX-License-Identifier: Apache-2.0

"""
Device-side softmax for Qwen attention.

Three-kernel approach (data stays on device between kernels):
  1. masked_max_kernel: apply mask, find row-max via sequential accumulation
  2. exp_sum_kernel: compute exp(x - max), find row-sum
  3. normalize_kernel: divide by sum

For decode: scores are [32, 512] = [1, 16] tiles.
"""

import torch
import ttl
import ttnn

TILE = 32


@ttl.kernel(grid=(1, 1))
def masked_max_kernel(scores, mask, masked_out, max_out):
    """masked_out = scores + mask; max_out = max across columns.

    scores: [Mt, Nt]
    mask: [Mt, Nt]
    masked_out: [Mt, Nt] — stores scores + mask
    max_out: [Mt, 1] — per-row max (single tile column)
    """
    Mt = scores.shape[0] // TILE
    Nt = scores.shape[1] // TILE

    s_dfb = ttl.make_dataflow_buffer_like(scores, shape=(1, 1), buffer_factor=2)
    m_dfb = ttl.make_dataflow_buffer_like(mask, shape=(1, 1), buffer_factor=2)
    acc_dfb = ttl.make_dataflow_buffer_like(max_out, shape=(1, 1), buffer_factor=2)
    o_dfb = ttl.make_dataflow_buffer_like(masked_out, shape=(1, 1), buffer_factor=2)
    mx_dfb = ttl.make_dataflow_buffer_like(max_out, shape=(1, 1), buffer_factor=2)

    @ttl.datamovement()
    def read():
        for row in range(Mt):
            for col in range(Nt):
                with s_dfb.reserve() as blk:
                    tx = ttl.copy(scores[row, col], blk)
                    tx.wait()
                with m_dfb.reserve() as blk:
                    tx = ttl.copy(mask[row, col], blk)
                    tx.wait()

    @ttl.compute()
    def compute():
        for _ in range(Mt):
            # First column: compute masked, output it, init max
            with s_dfb.wait() as s, m_dfb.wait() as m:
                with acc_dfb.reserve() as acc:
                    acc.store(s + m)
                with o_dfb.reserve() as out:
                    out.store(s + m)

            # Remaining columns: compute masked, output it, update max
            for _ in range(Nt - 1):
                with s_dfb.wait() as s, m_dfb.wait() as m, acc_dfb.wait() as prev:
                    with acc_dfb.reserve() as acc:
                        acc.store(ttl.math.max(prev, s + m))
                    with o_dfb.reserve() as out:
                        out.store(s + m)

            # Write final max
            with acc_dfb.wait() as final_max:
                with mx_dfb.reserve() as mx:
                    mx.store(final_max)

    @ttl.datamovement()
    def write():
        for row in range(Mt):
            for col in range(Nt):
                with o_dfb.wait() as blk:
                    tx = ttl.copy(blk, masked_out[row, col])
                    tx.wait()
            with mx_dfb.wait() as blk:
                tx = ttl.copy(blk, max_out[row, 0])
                tx.wait()


@ttl.kernel(grid=(1, 1))
def exp_sum_kernel(masked, row_max, exp_out, sum_out):
    """exp_out = exp(masked - row_max); sum_out = sum of exp across columns.

    masked: [Mt, Nt]
    row_max: [Mt, 1] — per-row max
    exp_out: [Mt, Nt]
    sum_out: [Mt, 1] — per-row sum of exp
    """
    Mt = masked.shape[0] // TILE
    Nt = masked.shape[1] // TILE

    m_dfb = ttl.make_dataflow_buffer_like(masked, shape=(1, 1), buffer_factor=2)
    mx_dfb = ttl.make_dataflow_buffer_like(row_max, shape=(1, 1), buffer_factor=2)
    acc_dfb = ttl.make_dataflow_buffer_like(sum_out, shape=(1, 1), buffer_factor=2)
    e_dfb = ttl.make_dataflow_buffer_like(exp_out, shape=(1, 1), buffer_factor=2)
    sm_dfb = ttl.make_dataflow_buffer_like(sum_out, shape=(1, 1), buffer_factor=2)

    @ttl.datamovement()
    def read():
        for row in range(Mt):
            # Read max once per row, then all column tiles
            with mx_dfb.reserve() as blk:
                tx = ttl.copy(row_max[row, 0], blk)
                tx.wait()
            for col in range(Nt):
                with m_dfb.reserve() as blk:
                    tx = ttl.copy(masked[row, col], blk)
                    tx.wait()

    @ttl.compute()
    def compute():
        for _ in range(Mt):
            with mx_dfb.wait() as max_blk:
                # First column: exp, output it, init sum
                with m_dfb.wait() as masked_blk:
                    with acc_dfb.reserve() as acc:
                        acc.store(ttl.math.exp(masked_blk - max_blk))
                    with e_dfb.reserve() as exp_tile:
                        exp_tile.store(ttl.math.exp(masked_blk - max_blk))

                # Remaining columns: exp, output, accumulate sum
                for _ in range(Nt - 1):
                    with m_dfb.wait() as masked_blk, acc_dfb.wait() as prev:
                        with acc_dfb.reserve() as acc:
                            acc.store(prev + ttl.math.exp(masked_blk - max_blk))
                        with e_dfb.reserve() as exp_tile:
                            exp_tile.store(ttl.math.exp(masked_blk - max_blk))

            # Write sum
            with acc_dfb.wait() as final_sum:
                with sm_dfb.reserve() as sm:
                    sm.store(final_sum)

    @ttl.datamovement()
    def write():
        for row in range(Mt):
            for col in range(Nt):
                with e_dfb.wait() as blk:
                    tx = ttl.copy(blk, exp_out[row, col])
                    tx.wait()
            with sm_dfb.wait() as blk:
                tx = ttl.copy(blk, sum_out[row, 0])
                tx.wait()


@ttl.kernel(grid=(1, 1))
def normalize_kernel(exp_scores, row_sum, Y):
    """Y = exp_scores * recip(row_sum).

    exp_scores: [Mt, Nt]
    row_sum: [Mt, 1]
    Y: [Mt, Nt]
    """
    Mt = exp_scores.shape[0] // TILE
    Nt = exp_scores.shape[1] // TILE

    e_dfb = ttl.make_dataflow_buffer_like(exp_scores, shape=(1, 1), buffer_factor=2)
    s_dfb = ttl.make_dataflow_buffer_like(row_sum, shape=(1, 1), buffer_factor=2)
    y_dfb = ttl.make_dataflow_buffer_like(Y, shape=(1, 1), buffer_factor=2)

    @ttl.datamovement()
    def read():
        for row in range(Mt):
            with s_dfb.reserve() as blk:
                tx = ttl.copy(row_sum[row, 0], blk)
                tx.wait()
            for col in range(Nt):
                with e_dfb.reserve() as blk:
                    tx = ttl.copy(exp_scores[row, col], blk)
                    tx.wait()

    @ttl.compute()
    def compute():
        for _ in range(Mt):
            with s_dfb.wait() as sum_blk:
                for _ in range(Nt):
                    with e_dfb.wait() as exp_blk:
                        with y_dfb.reserve() as out:
                            out.store(exp_blk * ttl.math.recip(sum_blk))

    @ttl.datamovement()
    def write():
        for row in range(Mt):
            for col in range(Nt):
                with y_dfb.wait() as blk:
                    tx = ttl.copy(blk, Y[row, col])
                    tx.wait()


def device_softmax(scores_dev, mask_dev, device):
    """Full device-side softmax: scores_dev + mask_dev → softmax weights.

    All intermediate tensors stay on device. No host transfers.

    Args:
        scores_dev: [Mt, Nt] tiles on device
        mask_dev: [Mt, Nt] tiles on device
        device: TTNN device

    Returns:
        weights_dev: [Mt, Nt] tiles on device
    """
    rows = scores_dev.shape[0]
    cols = scores_dev.shape[1]
    Mt = rows // TILE

    def alloc(shape):
        return ttnn.from_torch(
            torch.zeros(shape, dtype=torch.bfloat16),
            dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT,
            device=device, memory_config=ttnn.DRAM_MEMORY_CONFIG,
        )

    masked = alloc((rows, cols))
    row_max = alloc((rows, TILE))  # [Mt, 1] tiles
    exp_out = alloc((rows, cols))
    row_sum = alloc((rows, TILE))  # [Mt, 1] tiles
    weights = alloc((rows, cols))

    masked_max_kernel(scores_dev, mask_dev, masked, row_max)
    exp_sum_kernel(masked, row_max, exp_out, row_sum)
    normalize_kernel(exp_out, row_sum, weights)

    return weights


# =========================================================================
# Test
# =========================================================================
def _to_device(tensor, device):
    return ttnn.from_torch(
        tensor, dtype=ttnn.bfloat16, layout=ttnn.TILE_LAYOUT,
        device=device, memory_config=ttnn.DRAM_MEMORY_CONFIG,
    )


def test_softmax(device):
    rows, cols = TILE, 512  # decode size: 1x16 tiles
    print(f"  softmax [{rows}x{cols}]...", end="", flush=True)

    scores_t = torch.randn(rows, cols, dtype=torch.bfloat16) * 0.5
    mask_t = torch.full((rows, cols), float("-inf"), dtype=torch.bfloat16)
    mask_t[0, :10] = 0.0  # attend to first 10 positions

    scores = _to_device(scores_t, device)
    mask = _to_device(mask_t, device)

    result_dev = device_softmax(scores, mask, device)
    result = ttnn.to_torch(result_dev)

    expected = torch.nn.functional.softmax(
        (scores_t.float() + mask_t.float()), dim=-1
    ).bfloat16()

    score = torch.corrcoef(
        torch.stack([result[0].float(), expected[0].float()])
    )[0, 1].item()
    print(f" PCC={score:.6f}", end="")
    assert score > 0.98, f" FAIL"
    print(" PASS")


def test_softmax_prefill(device):
    rows, cols = 32, 32  # small prefill: 1x1 tiles
    print(f"  softmax [{rows}x{cols}] (prefill)...", end="", flush=True)

    scores_t = torch.randn(rows, cols, dtype=torch.bfloat16)
    mask_t = torch.triu(torch.full((rows, cols), float("-inf")), diagonal=1).bfloat16()

    scores = _to_device(scores_t, device)
    mask = _to_device(mask_t, device)

    result_dev = device_softmax(scores, mask, device)
    result = ttnn.to_torch(result_dev)

    expected = torch.nn.functional.softmax(
        (scores_t.float() + mask_t.float()), dim=-1
    ).bfloat16()

    score = torch.corrcoef(
        torch.stack([result.float().flatten(), expected.float().flatten()])
    )[0, 1].item()
    print(f" PCC={score:.6f}", end="")
    assert score > 0.98, f" FAIL"
    print(" PASS")


if __name__ == "__main__":
    device = ttnn.open_device(device_id=0)
    try:
        print("Softmax tests:")
        test_softmax_prefill(device)
        test_softmax(device)
        print("All softmax tests passed!")
    finally:
        ttnn.close_device(device)
