...
def test_tt_lang_upsample_nearest_rowwise_interleaved(input_shape, scale_factor):
    ...
    input_tensor = ttnn.rand(input_shape, device=device, layout=ttnn.ROW_MAJOR_LAYOUT, memory_config=ttnn.DRAM_MEMORY_CONFIG)
    output_shape = (input_shape[0], input_shape[1] * scale_factor[0], input_shape[2] * scale_factor[1], input_shape[3])
    output_tensor = ttnn.empty(output_shape, device=device, layout=ttnn.ROW_MAJOR_LAYOUT, memory_config=ttnn.DRAM_MEMORY_CONFIG)

    tt_lang_upsample_nearest_rowwise_interleaved(input_tensor, output_tensor, scale_factor)
