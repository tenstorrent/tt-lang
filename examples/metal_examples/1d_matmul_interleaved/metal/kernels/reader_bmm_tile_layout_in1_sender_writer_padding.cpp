...
void kernel_main() {
    ...
    // WRITER
    // out tensor args
    constexpr uint32_t out_tensor_stride_w = get_compile_time_arg_val(14);
    constexpr uint32_t out_tensor_stride_h = get_compile_time_arg_val(15);
    constexpr uint32_t out_tensor_next_subblock_stride_w = get_compile_time_arg_val(16);
    constexpr uint32_t out_tensor_next_subblock_stride_h = get_compile_time_arg_val(17);
    constexpr uint32_t out_tensor_next_w_dim_block_stride = get_compile_time_arg_val(18);
    constexpr uint32_t out_tensor_next_h_dim_block_stride = get_compile_time_arg_val(19);
    // out subblock args
    ...
    constexpr auto in1_args = TensorAccessorArgs<27>();
...
//  READER
    ...
    *(in1_mcast_receiver_semaphore_addr_ptr) = VALID;
    ...
    cb_wait_front(
        cb_id_out0,
        out_num_nonzero_subblocks_h * out_num_nonzero_subblocks_w * out_subblock_w * out_subblock_h);
#endif
    noc_async_write_barrier();
}
