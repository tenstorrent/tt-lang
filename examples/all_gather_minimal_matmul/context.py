...
"""Context-manager fabric all-gather fused with a block matmul.
    ...
    def all_gather_minimal_matmul(
        ...
        def send_activation():
            def send_activation_tile(pipe):
                for k_transfer_index in range(num_k_transfers):
                    ...
                    with send_activation_dfb.reserve() as activation_block:
                        ttl.copy(
                            activation_shard[0:m_tiles, k_begin:k_end],
                            activation_block,
                        ).wait()
                    with send_activation_dfb.wait() as activation_block:
        ...
        def receive_activation():
            ...
            def receive_activation_tile(pipe):
                ...
                k_begin = source_begin + k_transfer_index * k_tiles_per_transfer
                k_end = k_begin + k_tiles_per_transfer
                with receive_activation_dfb.reserve() as activation_block:
                    ttl.copy(pipe, activation_block).wait()
                with receive_activation_dfb.wait() as activation_block:
                    ttl.copy(
                        activation_block,
            ...
            with output_dfb.wait() as output_block:
                ttl.copy(
                    output_block, output_shard[0:m_tiles, 0:n_tiles_per_device]
                ).wait()
...
def main() -> None:
    ...
    print_config("all_gather_minimal_matmul_context", config)

    with open_configured_mesh(config) as mesh_device:
        host_tensors = make_host_tensors(config)
        mesh_tensors = make_mesh_tensors(config, mesh_device, host_tensors)
        ...
        validate_results(config, mesh_device, host_tensors, mesh_tensors)
