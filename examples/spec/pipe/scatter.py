...
def scatter(src: ttnn.Tensor, out: ttnn.Tensor) -> None:
    ...
    send_dfb = ttl.make_dataflow_buffer_like(src, shape=(1, 1), block_count=2)
    recv_dfb = ttl.make_dataflow_buffer_like(src, shape=(1, 1), block_count=2)
    # spec:begin
    # ---------------------
    ...
    def dm():
        def pipe_src(pipe):
            with send_dfb.reserve() as producer_block:
                ...
                ttl.copy(src[nx : nx + 1, 0:1], producer_block).wait()
                # spec:begin
