...
Computes single-query scaled dot-product attention over K/V chunks distributed
across eight nodes and combines per-node streaming-softmax statistics with a
...
High-level optimized algorithm:
  ...
  for each node's round-robin K/V chunks:
    for each K/V chunk:
...
vDHt = 16  # Last known working: 16; optimized target: 16.
Sk_chunk_t = 4  # Last known working: 4; optimized target: 4.
N_CHUNKS = 2  # Last known working: 2; optimized target: 2.
...
def flash_chain_8node(q, k, v, final_out):
    ...
    def compute():
        ...
        # Nodes that consume a level-0 child either keep the merged state for the
        # next receive level or publish it to their level-1 parent.
        if s1_m.is_dst():
            m_b0 = m_recv.wait()
            l_b0 = l_recv.wait()
            o_b0 = o_recv.wait()
            ...
            a_scale0_bc = ttl.block.broadcast(a_scale0, dims=[1], shape=(PNHt, vDHt))
            b_scale0_bc = ttl.block.broadcast(b_scale0, dims=[1], shape=(PNHt, vDHt))
            o_new = ttl.add(ttl.mul(a_scale0_bc, o_final), ttl.mul(b_scale0_bc, o_b0))
        ...
        if s2_m.is_dst():
            ...
            a_scale1_bc = ttl.block.broadcast(a_scale1, dims=[1], shape=(PNHt, vDHt))
            b_scale1_bc = ttl.block.broadcast(b_scale1, dims=[1], shape=(PNHt, vDHt))
    ...
    def dm_recv_io():
        ...
        # Match Blaze's S-block schedule: node x processes chunks x, x + 8,
        # and so on, preserving DRAM-bank-local work assignment.
        q_dst = q_cb.reserve()
        ttl.copy(q[0:PNHt, 0:DHt], q_dst)
        for chunk_iter in range(N_CHUNKS):
            chunk_index = node_x + chunk_iter * NNODES
            kc = chunk_index * Sk_chunk_t
            k_dst = k_cb.reserve()
...
def _compute_abs_error_metrics(golden, actual):
    abs_error = torch.nan_to_num(torch.abs(golden.float() - actual.float()))
    return abs_error.mean().item(), abs_error.max().item()
...
def main():
    ...
    try:
        ...
        print(f"  mean_abs={mean_abs:.6e}  max_abs={max_abs:.6e}")
