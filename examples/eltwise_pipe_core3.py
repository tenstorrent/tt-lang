...
def eltwise_pipe_node3(
    ...
    # Source: (0,3) (node 3), Destinations: nodes at row 0, columns 0-2
    # NodeRange format: (row_index, slice(start_col, end_col))
    # This expands to nodes (0,0), (0,1), (0,2)
    pipe = ttl.Pipe((0, 3), (0, slice(0, 3)))
...
def main() -> None:
    ...
    # uses only row 0 of the source tile and replicates it across the
    # remaining rows (step 1) before tiling across the grid (step 2); rows
    # 1..31 of the random ``c_in`` tile are not visible in the output.
