...
// Double-buffered reader kernel for 3 input tensors (ternary operation)
// Prefetches next block while compute processes current block
...
void kernel_main() {
  ...
  if (num_blocks > 0) {
    ...
    for (uint32_t t = 0; t < block_size; t++) {
      const uint32_t tile_id = start_id + t;
  ...
  // Process remaining full blocks with double-buffering
  // While compute processes block N, prefetch block N+1
  for (uint32_t block = 1; block < num_blocks; block++) {
    // Reserve space for next block (double-buffer)
    cb_reserve_back(cb_in0, block_size);
    cb_reserve_back(cb_in1, block_size);
    cb_reserve_back(cb_in2, block_size);
  ...
  // Read remaining tiles (partial block) - no double-buffering needed for last
  // partial
