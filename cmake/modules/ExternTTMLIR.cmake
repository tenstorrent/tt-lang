...
if(NOT DEFINED _TTMLIR_BUILD_DIR)
  ...
  # Check if the config file exists at the hint location
  foreach(_hint IN LISTS TTMLIR_HINTS)
    message(STATUS "Checking hint: ${_hint}")
    if(EXISTS "${_hint}/TTMLIRConfig.cmake")
      message(STATUS "  -> Found TTMLIRConfig.cmake!")
      # Try to read first few lines for debugging
      file(STRINGS "${_hint}/TTMLIRConfig.cmake" _config_lines LIMIT_COUNT 5)
      foreach(_line IN LISTS _config_lines)
        message(STATUS "    ${_line}")
      endforeach()
    else()
      message(STATUS "  -> TTMLIRConfig.cmake NOT found at ${_hint}/TTMLIRConfig.cmake")
    endif()
  endforeach()
  # Note: Not using QUIET so we can see why find_package fails
  find_package(TTMLIR CONFIG HINTS ${TTMLIR_HINTS})
  if(TTMLIR_FOUND)
    ...
    message(STATUS "  TTMLIR_FOUND: ${TTMLIR_FOUND}")
