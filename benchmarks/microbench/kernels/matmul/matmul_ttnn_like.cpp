...
//
// TTNN-like single-node matmul compute-feed probe. This mirrors the relevant
// single-core contract from TTNN's large-block matmul kernel: A and B arrive in
// K blocks, subblocks use TTNN's output-subblock order, and `matmul_block` is
// called with `kt_dim = in0_block_w` while A/B offsets advance through the K
// block.
//
