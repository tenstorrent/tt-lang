...
@test "full fabric tests expose every device and have a timeout" {
    TT_VISIBLE_DEVICES=7 run -0 "$SCRIPT" fabric-full

    run cat "$CALLS"
    assert_output --partial "python3 active:1 visible: args:-m pytest"
    assert_output --partial "test/python/fabric -v -x"
    assert_output --partial "--junitxml=build/test/pytest-report-fabric-full.xml"
}
