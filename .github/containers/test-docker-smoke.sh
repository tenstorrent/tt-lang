...
if [ -e /dev/tenstorrent/0 ]; then
    ...
    run_test "Example ran on hardware" "Hardware example failed" \
        sudo docker run --rm \
            ...
            tt-lang-dist-ubuntu-22-04:latest python /opt/ttlang-toolchain/examples/tutorial/multinode_grid_auto.py
else
...
run_test "Examples in /root" "Examples missing" \
    sudo docker run --rm tt-lang-dist-ubuntu-22-04:latest ls /root/examples/tutorial/multinode_grid_auto.py
echo ""
