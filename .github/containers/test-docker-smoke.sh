...
if [ -e /dev/tenstorrent/0 ]; then
    ...
    run_test "Example ran on hardware" "Hardware example failed" \
        sudo docker run --rm \
            ...
            tt-lang-dist-ubuntu-22-04:latest python /opt/ttmlir-toolchain/examples/demo_one.py
else
...
run_test "Examples in /root" "Examples missing" \
    sudo docker run --rm tt-lang-dist-ubuntu-22-04:latest ls /root/examples/demo_one.py
echo ""
