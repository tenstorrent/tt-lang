...
When building locally (`--no-push`), only local tags are created to avoid
shadowing registry images in the Docker cache.
...
sudo .github/containers/build-docker-images.sh --image-type ird --no-push
