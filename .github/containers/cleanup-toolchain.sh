...
# Replace unnecessary LLVM/MLIR binaries with stubs
...
#
# We create stub scripts instead of deleting binaries because CMake's
# LLVMExports.cmake checks that all imported targets exist.

BINARIES_TO_STUB=(
    llc lli lli-child-target opt bugpoint
...
for binary in "${BINARIES_TO_STUB[@]}"; do
    rm -f "$binary"
    printf '#!/bin/sh\necho "Error: %s was removed to save space" >&2\nexit 1\n' "$binary" > "$binary"
    chmod +x "$binary"
done
